...
def ProcessErrorList(_filename):
    ...
    # below.
    if _filename == '-':
        lines = codecs.StreamReaderWriter(sys.stdin, codecs.getreader('utf8'), codecs.getwriter('utf8'),
                                          'replace').readlines()
    else:
...
def fix_whitespace_operators(_lines, _error_list):
    for index in range(len(_error_list), 0, -1):
        ...
        if IsCanAutoFix(operators, _lines, info.linenum):
            _lines[info.linenum] = ReplaceAll(r'([\w\])}"])%s(.)' % (match_re), r'\1 %s\2' % (match_re),
                                              _lines[info.linenum])
            _lines[info.linenum] = ReplaceAll(r'(.)%s([\w\[({"-])' % (match_re), r'\1%s \2' % (match_re),
                                              _lines[info.linenum])
...
def fix_whitespace_parens(_lines, _error_list):
    for index in range(len(_error_list), 0, -1):
        ...
        if IsCanAutoFix(match.group(1) + match.group(2), _lines, info.linenum):
            _lines[info.linenum] = ReplaceAll(r'%s' % (ToHex(match.group(1) + match.group(2))),
                                              match.group(1) + ' ' + match.group(2), _lines[info.linenum])
        del _error_list[index - 1]
