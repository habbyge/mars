...
class cpplint_ex:
    ...
    def __walk(self, start_path, path):
        ...
        if cpplint_custom_rules.is_skip_path(relative_path):
            print
            'skip walk path : %s' % (relative_path)
            return
        ...
        for item in os.listdir(path):
            ...
            if (stat.S_ISDIR(mode)):
                ...
                print
                'add file to list : ' + relative_file_path
    ...
    def check(self, max_err_count=None):
        ...
        for filename in self.allfiles:
            try:
                ...
                print
                "Error: except when check file: %s, err:%s" % (filename, str(e))
        ...
        try:
            ...
            try:
                ...
                print
                'out_file_name xml can not parse.'
                root = None
                dom = None
            ...
            print
            "Error: write CodeRule result err(%s)" % (str(e))
            return -2
