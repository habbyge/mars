...
//           mars_boost::throw_exception(thread_resource_error());
  ...
  return true;
#else
  uintptr_t const new_thread = _beginthreadex(0, 0, &thread_start_function, thread_info.get(), CREATE_SUSPENDED,
                                              &thread_info->id);
  if (!new_thread) {
    return false;
//            mars_boost::throw_exception(thread_resource_error());
...
bool thread::start_thread_noexcept(const attributes& attr) {
  ...
  uintptr_t const new_thread = _beginthreadex(0, static_cast<unsigned int>(attr.get_stack_size()),
                                              &thread_start_function, thread_info.get(), CREATE_SUSPENDED,
                                              &thread_info->id);
  if (!new_thread) {
    return false;
//          mars_boost::throw_exception(thread_resource_error());
...
bool thread::interruption_requested() const BOOST_NOEXCEPT {
  detail::thread_data_ptr local_thread_info = (get_thread_info)();
  return local_thread_info.get() &&
         (detail::win32::WaitForSingleObjectEx(local_thread_info->interruption_handle, 0, 0) == 0);
}
...
typedef BOOL (WINAPI
*setwaitabletimerex_t)(HANDLE, const LARGE_INTEGER *, LONG, PTIMERAPCROUTINE, LPVOID, PREASON_CONTEXT, ULONG);
static inline BOOL WINAPI
SetWaitableTimerEx_emulation(HANDLE
hTimer,
const LARGE_INTEGER* lpDueTime, LONG
lPeriod,
PTIMERAPCROUTINE pfnCompletionRoutine, LPVOID
lpArgToCompletionRoutine,
PREASON_CONTEXT WakeContext, ULONG
TolerableDelay) {
return
SetWaitableTimer(hTimer, lpDueTime, lPeriod, pfnCompletionRoutine, lpArgToCompletionRoutine, FALSE
);
}
#ifdef _MSC_VER
#pragma warning(push)
...
bool interruptible_wait(detail::win32::handle handle_to_wait_for, detail::timeout target_time) {
  ...
  if (!target_time.is_sentinel()) {
    ...
    if (timer_handle != 0) {
      ...
      bool const set_time_succeeded =
          detail_::SetWaitableTimerEx()(timer_handle, &due_time, 0, 0, 0, NULL, tolerable) != 0;
  ...
  do {
    ...
    if (handle_count) {
      unsigned long const notified_index = detail::win32::WaitForMultipleObjectsEx(handle_count, handles, false,
                                                                                   using_timer ? INFINITE
...
bool interruption_requested() BOOST_NOEXCEPT {
  return detail::get_current_thread_data() &&
         (detail::win32::WaitForSingleObjectEx(detail::get_current_thread_data()->interruption_handle, 0, 0) == 0);
}
