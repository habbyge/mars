...
bool file_descriptor_impl::is_open() const { return handle_ != invalid_handle(); }
...
file_descriptor::file_descriptor(handle_type fd, file_descriptor_flags f)
    : pimpl_(new impl_type) { open(fd, f); }
...
file_descriptor::file_descriptor(const std::string& path,
    ...
    : pimpl_(new impl_type) { open(path, mode); }
...
void file_descriptor::open(const std::string& path, BOOST_IOS::openmode mode) { open(detail::path(path), mode); }

void file_descriptor::open(const char* path, BOOST_IOS::openmode mode) { open(detail::path(path), mode); }

bool file_descriptor::is_open() const { return pimpl_->is_open(); }

void file_descriptor::close() { pimpl_->close(); }

std::streamsize file_descriptor::read(char_type* s, std::streamsize n) { return pimpl_->read(s, n); }

std::streamsize file_descriptor::write(const char_type* s, std::streamsize n) { return pimpl_->write(s, n); }

std::streampos file_descriptor::seek(stream_offset off, BOOST_IOS::seekdir way) { return pimpl_->seek(off, way); }
...
file_descriptor_source::file_descriptor_source(
    handle_type fd, file_descriptor_flags f) { open(fd, f); }
...
file_descriptor_source::file_descriptor_source(
    const std::string& path, BOOST_IOS::openmode mode) { open(path, mode); }

file_descriptor_source::file_descriptor_source(
    const char* path, BOOST_IOS::openmode mode) { open(path, mode); }

file_descriptor_source::file_descriptor_source(
    const file_descriptor_source& other)
    : file_descriptor(static_cast<const file_descriptor&>(other)) {}

void file_descriptor_source::open(handle_type fd, file_descriptor_flags f) { file_descriptor::open(fd, f); }
...
void file_descriptor_source::open(
    const std::string& path, BOOST_IOS::openmode mode) { open(detail::path(path), mode); }

void file_descriptor_source::open(
    const char* path, BOOST_IOS::openmode mode) { open(detail::path(path), mode); }
...
void file_descriptor_sink::open(handle_type fd, file_descriptor_flags f) { file_descriptor::open(fd, f); }
