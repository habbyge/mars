...
template<class T>
struct promote_arg { // If T is integral type, then promote to double.
...
struct promote_args_2 { // Promote, if necessary, & pick the wider of the two floating-point types.
  ...
  typedef typename mpl::if_<
      typename mpl::and_<is_floating_point < T1P>,
      is_floating_point < T2P> >::type, // both T1P and T2P are floating-point?
#ifdef BOOST_MATH_USE_FLOAT128
  typename mpl::if_< typename mpl::or_<is_same<__float128, T1P>, is_same<__float128, T2P> >::type, // either long double?
   __float128,
#endif
  typename mpl::if_<typename mpl::or_<is_same < long double, T1P>,
      is_same < long double, T2P> >::type, // either long double?
  ...
  typename mpl::if_<typename mpl::and_<
      mpl::not_ < is_floating_point < T2P>>, ::mars_boost::is_convertible<T1P, T2P> >, T2P, T1P>::type>::type type;
}; // promote_arg2
// These full specialisations reduce mpl::if_ usage and speed up
// compilation:
template<>
