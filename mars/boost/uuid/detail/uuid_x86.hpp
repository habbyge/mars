...
BOOST_FORCEINLINE __m128i

load_unaligned_si128(const uint8_t* p)
...
return _mm_loadu_si128(reinterpret_cast
<const __m128i*>(p)
);
...
uint32_t cmp = static_cast< uint32_t >(_mm_movemask_epi8(mm_cmp)), rcmp = static_cast< uint32_t >(_mm_movemask_epi8(
    mm_rcmp));
