...
class guard { //! Locks the mutex, binding guard<Mutex> to Mutex.
  /*! Example:
...
public:
  explicit guard(Mutex& nmtx)
      : mtx(nmtx) { //! Locks the mutex of the guard class.
    mtx.lock();
  }

  ~guard() { //! destructor unlocks the mutex of the guard class.
    mtx.unlock();
  }
}; // class guard
