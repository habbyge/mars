...
void FUZ_bug976(void) {   /* these constants shall not depend on MIN() macro */
...
static int basicUnitTests(U32 const seed, double compressibility) {
  ...
  }
  {
    ...
    CHECK_Z(
        ZSTD_flushStream(cctx, &output));   /* ensure no possibility to "concatenate" and determine the content size */
  ...
  DISPLAYLEVEL(3, "test%3d : re-use CCtx with expanding block size : ", testNb++);
  {
    ...
    CHECK_Z(
        ZSTD_compressEnd(cctx, compressedBuffer, compressedBufferSize, CNBuffer, 1)); /* creates a block size of 1 */
  ...
  DISPLAYLEVEL(3, "test%3d : re-using a CCtx should compress the same : ", testNb++);
  {
    ...
    for (i = 1; i <= 19; i++) {
      ...
      size2 = ZSTD_compress2(cctx, compressedBuffer, ZSTD_compressBound(sampleSize) - 1, CNBuffer,
                             sampleSize);  /* force streaming, as output buffer is not large enough to guarantee success */
      CHECK_Z(size2);
      CHECK_EQ(size1, size2);

      {
        ...
        outb.pos = 0;
        outb.size = ZSTD_compressBound(sampleSize) -
  ...
  DISPLAYLEVEL(3, "test%3d : btultra2 & 1st block : ", testNb++);
  {
    ...
    CHECK_Z(ZSTD_compressStream2(cctx, &outb, &inb,
                                 ZSTD_e_end));   /* will break internal assert if stats_init is not disabled */
    assert(inb.pos == inb.size);
  ...
  DISPLAYLEVEL(3, "test%3d : overflow protection with large windowLog : ", testNb++);
  {
    ...
    for (cnb = 0; cnb < nbCompressions; ++cnb) {
      ...
      CHECK_Z(
          ZSTD_compressBegin_advanced(cctx, NULL, 0, params, ZSTD_CONTENTSIZE_UNKNOWN));  /* re-use same parameters */
  ...
  DISPLAYLEVEL(3, "test%3d : size down context : ", testNb++);
  {
    ...
    CHECK_Z(ZSTD_compressBegin(largeCCtx,
    ...
    CHECK_Z(ZSTD_compressEnd(largeCCtx, compressedBuffer, compressedBufferSize, CNBuffer, 1));
    {
      size_t const largeCCtxSize = ZSTD_sizeof_CCtx(
          largeCCtx);   /* size of context must be measured after compression */
      {
...
#define STATIC_CCTX_LEVEL 3
  ...
  /* Dictionary and CCtx Duplication tests */
  {
    ...
    DISPLAYLEVEL(3, "test%3i : decompress with static DDict : ", testNb++);
    {
      ...
      {
        const ZSTD_DDict* const ddict = ZSTD_initStaticDDict(ddictBuffer, ddictBufferSize, CNBuffer, dictSize,
    ...
    if ((int) (compressibility * 100 + 0.1) == FUZ_compressibility_default) { /* test only valid with known input */
      ...
      DISPLAYLEVEL(4, "content hash : %016llx;  dict hash : %016llx \n", XXH64(contentStart, contentSize, 0),
                   XXH64(dict, flatdictSize, 0));
  ...
  /* Dictionary and dictBuilder tests */
  {
    ...
    {
      U32 u;
      for (u = 0; u < totalSampleSize; u++) ((BYTE*) decodedBuffer)[u] = (BYTE) u;
    }
    {
      U32 u;
      for (u = 0; u < nbSamples; u++) samplesSizes[u] = sampleUnitSize;
    }
    {
    ...
    DISPLAYLEVEL(3, "test%3i : ZSTD_compress_usingCDict_advanced, no contentSize, no dictID : ", testNb++);
    {
      ...
      ZSTD_compressionParameters const cParams = ZSTD_getCParams(1, CNBuffSize, dictSize);
      ZSTD_CDict* const cdict = ZSTD_createCDict_advanced(dictBuffer, dictSize, ZSTD_dlm_byRef, ZSTD_dct_auto, cParams,
                                                          ZSTD_defaultCMem);
      assert(cdict != NULL);
    ...
    {
      ZSTD_compressionParameters const cParams = ZSTD_getCParams(1, CNBuffSize, dictSize);
      ZSTD_CDict* const cdict = ZSTD_createCDict_advanced(dictBuffer, dictSize, ZSTD_dlm_byRef, ZSTD_dct_fullDict,
                                                          cParams, ZSTD_defaultCMem);
    ...
    {
      ZSTD_compressionParameters const cParams = ZSTD_getCParams(1, CNBuffSize, dictSize);
      ZSTD_CDict* const cdict = ZSTD_createCDict_advanced((const char*) dictBuffer + 1, dictSize - 1, ZSTD_dlm_byRef,
    ...
    DISPLAYLEVEL(3, "test%3i : Loading rawContent starting with dict header w/ ZSTD_dct_rawContent should pass : ",
                 testNb++);
    {
      size_t ret;
      MEM_writeLE32((char*) dictBuffer + 2, ZSTD_MAGIC_DICTIONARY);
         ...
         */
    {
      ...
      dSize = ZSTD_decompress_usingDict(dctx, decodedBuffer, sizeof(data), compressedBuffer, cSize, dictBuffer,
                                        dictSize);
...

    {
      ...
      CHECK(ZSTD_CCtx_setParameter(cctx, ZSTD_c_compressionLevel, 2));
      {
        ...
        if (XXH64(compressedBuffer, result, 0) != xxh64)
          goto _output_error;  /* must result in exactly same content, hence same hash */
  ...
  /* block API tests */
  {
    ...
    CHECK(ZSTD_getBlockSize(cctx) >= blockSize);
    {
      ...
      while (compressed < toCompress) {
        size_t const blockCSize = ZSTD_compressBlock(cctx, compressedBuffer, ZSTD_compressBound(blockSize), CNBuffer,
                                                     blockSize);
        assert(blockCSize != 0);
    ...
    CHECK_VAR(cSize,
              ZSTD_compressBlock(cctx, compressedBuffer, ZSTD_compressBound(blockSize), (char*) CNBuffer + dictSize,
                                 blockSize));
    RDG_genBuffer((char*) CNBuffer + dictSize + blockSize, blockSize, 0.0, 0.0,
                  seed);  /* create a non-compressible second block */
    {
      CHECK_NEWV(r, ZSTD_compressBlock(cctx, (char*) compressedBuffer + cSize, ZSTD_compressBound(blockSize),
                                       (char*) CNBuffer + dictSize + blockSize,
                                       blockSize));  /* for cctx history consistency */
      assert(r == 0); /* non-compressible block */ }
    memcpy((char*) compressedBuffer + cSize, (char*) CNBuffer + dictSize + blockSize,
           blockSize);   /* send non-compressed block (without header) */
    CHECK_VAR(cSize2,
              ZSTD_compressBlock(cctx, (char*) compressedBuffer + cSize + blockSize, ZSTD_compressBound(blockSize),
    ...
    {
      CHECK_NEWV(r, ZSTD_decompressBlock(dctx, (char*) decodedBuffer + 2 * blockSize, blockSize,
                                         (char*) compressedBuffer + cSize + blockSize, cSize2));
      if (r != blockSize) {
        DISPLAYLEVEL(1,
                     "ZSTD_decompressBlock() with _usingDict() and after insertBlock() fails : %u, instead of %u expected \n",
                     (unsigned) r, (unsigned) blockSize);
        goto _output_error;
      }
    }
    assert(memcpy((char*) CNBuffer + dictSize, decodedBuffer,
                  blockSize * 3));  /* ensure regenerated content is identical to origin */
    DISPLAYLEVEL(3, "OK \n");
    ...
    DISPLAYLEVEL(3, "test%3i : Block compression with CDict : ", testNb++);
    {
      ...
      CHECK(ZSTD_compressBlock(cctx, compressedBuffer, ZSTD_compressBound(blockSize), (char*) CNBuffer + dictSize,
                               blockSize));
      ZSTD_freeCDict(cdict);
    }
    DISPLAYLEVEL(3, "OK \n");
...
#define NB3BYTESSEQMASK (NB3BYTESSEQ-1)
  ...
  if (strcmp("No error detected", ZSTD_getErrorName((ZSTD_ErrorCode)(0 - ZSTD_error_no_error))) != 0)
    goto _output_error;
  if (strcmp("No error detected", ZSTD_getErrorString(ZSTD_error_no_error)) != 0) goto _output_error;
  if (strcmp("Unspecified error code", ZSTD_getErrorString((ZSTD_ErrorCode)(0 - ZSTD_error_GENERIC))) != 0)
    goto _output_error;
...
static int fuzzerTests(U32 seed, unsigned nbTests, unsigned startTest, U32 const maxDurationS, double compressibility,
                       int bigTests) {
  ...
  for (; (testNb <= nbTests) || (UTIL_clockSpanMicro(startClock) < maxClockSpan); testNb++) {
    ...
    /* compression tests */
    {
      ...
      assert(cSize > 3);
      {
                     ...
                     testNb, (unsigned) tooSmallSize, (unsigned) missing);
        {
          ...
          CHECK(!ZSTD_isError(errorCode), "ZSTD_compressCCtx should have failed ! (buffer too small : %u < %u)",
                (unsigned) tooSmallSize, (unsigned) cSize);
        }
        {
          unsigned endCheck;
          memcpy(&endCheck, dstBuffer + tooSmallSize, sizeof(endCheck));
          CHECK(endCheck != endMark, "ZSTD_compressCCtx : dst buffer overflow  (check.%08X != %08X.mark)", endCheck,
                endMark);
        }
      }
    }
    ...
    DISPLAYLEVEL(5, "fuzzer t%u: simple decompression test \n", testNb);
    {
      ...
      CHECK(dSize != sampleSize, "ZSTD_decompress failed (%s) (srcSize : %u ; cSize : %u)", ZSTD_getErrorName(dSize),
            (unsigned) sampleSize, (unsigned) cSize);
      {
        U64 const crcDest = XXH64(dstBuffer, sampleSize, 0);
        CHECK(crcOrig != crcDest, "decompression result corrupted (pos %u / %u)",
              (unsigned) findDiff(sampleBuffer, dstBuffer, sampleSize), (unsigned) sampleSize);
      }
    }
    ...
    if (sampleSize > 3) {
      size_t const missing =
          (FUZ_rand(&lseed) % (sampleSize - 2)) + 1;   /* no problem, as cSize > 4 (frameHeaderSizer) */
      size_t const tooSmallSize = sampleSize - missing;
      static const BYTE token = 0xA9;
      dstBuffer[tooSmallSize] = token;
      {
        size_t const errorCode = ZSTD_decompress(dstBuffer, tooSmallSize, cBuffer, cSize);
        CHECK(!ZSTD_isError(errorCode), "ZSTD_decompress should have failed : %u > %u (dst buffer too small)",
      ...
      DISPLAYLEVEL(5, "fuzzer t%u: decompress noisy source \n", testNb);
      {
        ...
        memcpy(dstBuffer + sampleSize, &endMark, 4);
        {
          ...
          CHECK((!ZSTD_isError(decompressResult)) && (decompressResult > sampleSize),
                "ZSTD_decompress on noisy src : result is too large : %u > %u (dst buffer)",
...

    {
      ...
      for (totalTestSize = 0, cSize = 0, n = 0; n < nbChunks; n++) {
        ...
        {
          size_t const compressResult = ZSTD_compressContinue(ctx, cBuffer + cSize, cBufferSize - cSize,
                                                              srcBuffer + segmentStart, segmentSize);
          CHECK (ZSTD_isError(compressResult), "multi-segments compression error : %s",
    ...
    while (totalCSize < cSize) {
      ...
      size_t const genSize = ZSTD_decompressContinue(dctx, dstBuffer + totalGenSize, dstBufferSize - totalGenSize,
