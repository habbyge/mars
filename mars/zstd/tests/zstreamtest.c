...
static int basicUnitTests(U32 seed, double compressibility) {
  ...
  DISPLAYLEVEL(3, "test%3i : estimate CStream size : ", testNb++);
  {
    ...
    size_t const cdictSize = ZSTD_estimateCDictSize_advanced(dictSize, cParams,
  ...
  CHECK_Z(ZSTD_DCtx_setParameter(zd, ZSTD_d_windowLogMax, ZSTD_WINDOWLOG_LIMIT_DEFAULT + 1));  /* large limit */
  {
    ...
    if (remaining != 0) goto _output_error;
  ...
  {   /* skippable frame */
    ...
    while (r) {   /* skippable frame */
      ...
      if (ZSTD_isError(r))
        DISPLAYLEVEL(4, "ZSTD_decompressStream on skippable frame error : %s \n", ZSTD_getErrorName(r));
      if (ZSTD_isError(r)) goto _output_error;
    }
    /* normal frame */
    ZSTD_initDStream_usingDict(zd, CNBuffer, dictSize);
    ...
    while (r) {
      ...
      size_t const outSize = (FUZ_rand(&coreSeed) & 15) +
                             (!inSize);   /* avoid having both sizes at 0 => would trigger a no_forward_progress error */
      inBuff.size = inBuff.pos + inSize;
      outBuff.size = outBuff.pos + outSize;
      r = ZSTD_decompressStream(zd, &outBuff, &inBuff);
  ...
  if (outBuff.pos != CNBufferSize)
    DISPLAYLEVEL(4, "outBuff.pos != CNBufferSize : should have regenerated same amount ! \n");
  if (outBuff.pos != CNBufferSize) goto _output_error;   /* should regenerate the same amount */
  ...
  /* use 1 */
  {
    ...
    DISPLAYLEVEL(5, "use1 ");
    ZSTD_initCStream_advanced(zc, NULL, 0, ZSTD_getParams(19, inSize, 0),
  ...
  DISPLAYLEVEL(3, "test%3i : ZSTD_initCStream_usingCDict_advanced with masked dictID : ", testNb++);
  {
    ...
    ZSTD_CDict* const cdict = ZSTD_createCDict_advanced(dictionary.start, dictionary.filled, ZSTD_dlm_byRef,
  ...
  DISPLAYLEVEL(3, "test%3i : ZSTD_initCStream_advanced with pledgedSrcSize=0 and dict : ", testNb++);
  {
    ...
    CHECK_Z(ZSTD_initCStream_advanced(zc, dictionary.start, dictionary.filled, params,
  ...
  {
    ZSTD_compressionParameters const cParams = ZSTD_getCParams(1, 4 KB,
                                                               dictionary.filled);   /* intentionally lies on estimatedSrcSize, to push cdict into targeting a small window size */
    ZSTD_CDict* const cdict = ZSTD_createCDict_advanced(dictionary.start, dictionary.filled, ZSTD_dlm_byRef,
  ...
  DISPLAYLEVEL(3, "test%3i : check dictionary FSE tables can represent every code : ", testNb++);
  {
    ...
    cdict = ZSTD_createCDict_advanced(dictionary.start, dictionary.filled, ZSTD_dlm_byRef, ZSTD_dct_fullDict, cParams,
                                      ZSTD_defaultCMem);
  ...
  DISPLAYLEVEL(3, "test%3i : dictionary + uncompressible block + reusing tables checks offset table validity: ",
               testNb++);
  {
  ...
  DISPLAYLEVEL(3, "test%3i : dictionary + small blocks + reusing tables checks offset table validity: ", testNb++);
  {
    ...
    CHECK_Z(ZSTD_decompress_usingDict(zd, decodedBuffer, CNBufferSize, out.dst, out.pos, dictionary.start,
                                      dictionary.filled));
  ...
  for (; (testNb <= nbTests) || (UTIL_clockSpanMicro(startClock) < g_clockTime); testNb++) {
    ...
    /* compression init */
    {
      ...
      int const cLevelCandidate = (FUZ_rand(&lseed)
                                  ...
                                  + 1;
      int const cLevelThreadAdjusted =
          cLevelCandidate - (nbThreads * 2) + 2;  /* reduce cLevel when multiple threads to reduce memory consumption */
      ...
      if (FUZ_rand(&lseed) & 1) {   /* simple init */
        ...
        }
        {
          ...
          CHECK_Z(ZSTDMT_setMTCtxParameter(zc, ZSTDMT_p_jobSize,
  ...
  for (; (testNb <= nbTests) || (UTIL_clockSpanMicro(startClock) < g_clockTime); testNb++) {
    ...
    if ((FUZ_rand(&lseed) & 1) /* at beginning, to keep same nb of rand */
      ...
      }
      {
        ...
        if (FUZ_rand(&lseed) & 1) CHECK_Z(
            setCCtxParameter(zc, cctxParams, ZSTD_c_searchLog, cParams.searchLog, opaqueAPI));
        if (FUZ_rand(&lseed) & 1) CHECK_Z(
            setCCtxParameter(zc, cctxParams, ZSTD_c_minMatch, cParams.minMatch, opaqueAPI));
        if (FUZ_rand(&lseed) & 1) CHECK_Z(
            setCCtxParameter(zc, cctxParams, ZSTD_c_targetLength, cParams.targetLength, opaqueAPI));

        /* mess with long distance matching parameters */
        if (bigTests) {
          if (FUZ_rand(&lseed) & 1) CHECK_Z(
              setCCtxParameter(zc, cctxParams, ZSTD_c_enableLongDistanceMatching, FUZ_rand(&lseed) & 63, opaqueAPI));
          if (FUZ_rand(&lseed) & 3) CHECK_Z(
              setCCtxParameter(zc, cctxParams, ZSTD_c_ldmHashLog, FUZ_randomClampedLength(&lseed, ZSTD_HASHLOG_MIN, 23),
                               opaqueAPI));
          if (FUZ_rand(&lseed) & 3) CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_c_ldmMinMatch,
                                                             FUZ_randomClampedLength(&lseed, ZSTD_LDM_MINMATCH_MIN,
                                                                                     ZSTD_LDM_MINMATCH_MAX),
                                                             opaqueAPI));
          if (FUZ_rand(&lseed) & 3) CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_c_ldmBucketSizeLog,
                                                             FUZ_randomClampedLength(&lseed, ZSTD_LDM_BUCKETSIZELOG_MIN,
                                                                                     ZSTD_LDM_BUCKETSIZELOG_MAX),
                                                             opaqueAPI));
          if (FUZ_rand(&lseed) & 3) CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_c_ldmHashRateLog,
                                                             FUZ_randomClampedLength(&lseed, ZSTD_LDM_HASHRATELOG_MIN,
                                                                                     ZSTD_LDM_HASHRATELOG_MAX),
                                                             opaqueAPI));
          if (FUZ_rand(&lseed) & 3) CHECK_Z(setCCtxParameter(zc, cctxParams, ZSTD_c_srcSizeHint,
                                                             FUZ_randomClampedLength(&lseed, ZSTD_SRCSIZEHINT_MIN,
                                                                                     ZSTD_SRCSIZEHINT_MAX), opaqueAPI));
        }
        ...
        if (FUZ_rand(&lseed) & 1) CHECK_Z(
            setCCtxParameter(zc, cctxParams, ZSTD_c_dictIDFlag, FUZ_rand(&lseed) & 1, opaqueAPI));
        if (FUZ_rand(&lseed) & 1) CHECK_Z(
            setCCtxParameter(zc, cctxParams, ZSTD_c_contentSizeFlag, FUZ_rand(&lseed) & 1, opaqueAPI));
        if (FUZ_rand(&lseed) & 1) {
        ...
        if (FUZ_rand(&lseed) & 1) CHECK_Z(
            setCCtxParameter(zc, cctxParams, ZSTD_c_forceMaxWindow, FUZ_rand(&lseed) & 1, opaqueAPI));
    ...
    XXH64_reset(&xxhState, 0);
    {
      ...
      for (cSize = 0, totalTestSize = 0; (totalTestSize < maxTestSize);) {
        ...
        DISPLAYLEVEL(6, "t%u: compress consumed %u bytes (total : %u) ; flush: %u (total : %u) \n",
                     testNb, (unsigned) inBuff.pos, (unsigned) (totalTestSize + inBuff.pos), (unsigned) flush,
                     (unsigned) outBuff.pos);
    ...
    }
    {
      ...
      for (totalGenSize = 0; decompressionResult;) {
        ...
        CHECK (ZSTD_isError(decompressionResult), "decompression error : %s", ZSTD_getErrorName(decompressionResult));
        CHECK (inBuff.pos > cSize, "ZSTD_decompressStream consumes too much input : %u > %u ", (unsigned) inBuff.pos,
               (unsigned) cSize);
      }
      CHECK (inBuff.pos != cSize, "compressed data should be fully read (%u != %u)", (unsigned) inBuff.pos,
             (unsigned) cSize);
      CHECK (outBuff.pos != totalTestSize, "decompressed data : wrong size (%u != %u)", (unsigned) outBuff.pos,
...
typedef enum {
  simple_api, mt_api, advanced_api
} e_api;
