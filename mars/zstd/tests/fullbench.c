...
static int benchMem(unsigned benchNb,
  ...
  switch (benchNb) {
    ...
    case 42:
      ...
      break;
    case 43:
      benchFunction = local_ZSTD_compressStream_freshCCtx;
      benchName = "compressStream_freshCCtx";
      break;
    case 51:
  ...
  switch (benchNb) {
    ...
    case 31:  /* ZSTD_decodeLiteralsBlock : starts literals block in dstBuff2 */
    {
      ...
      srcSize = srcSize > 128
      KB ? 128 KB : srcSize;    /* speed relative to block */
      ZSTD_decompressBegin(g_zdc);
      break;
    }
  ...
  {
    size_t i;
    for (i = 0; i < dstBuffSize; i++) dstBuff[i] = (BYTE) i;
  }
  ...
  /* benchmark loop */
  {
    ...
    bestResult.nanoSecPerRun = (double) TIMELOOP_NANOSEC *
  ...
  for (argNb = 1; argNb < argc; argNb++) {
    ...
    if (longCommandWArg(&argument, "--zstd=")) {
      for (;;) {
        ...
        if (longCommandWArg(&argument, "chainLog=") || longCommandWArg(&argument, "clog=")) {
        ...
        if (longCommandWArg(&argument, "hashLog=") || longCommandWArg(&argument, "hlog=")) {
        ...
        if (longCommandWArg(&argument, "searchLog=") || longCommandWArg(&argument, "slog=")) {
        ...
        if (longCommandWArg(&argument, "minMatch=") || longCommandWArg(&argument, "mml=")) {
        ...
        if (longCommandWArg(&argument, "targetLength=") || longCommandWArg(&argument, "tlen=")) {
        ...
        if (longCommandWArg(&argument, "strategy=") || longCommandWArg(&argument, "strat=")) {
          cparams.strategy = (ZSTD_strategy)(readU32FromChar(&argument));
          if (argument[0] == ',') {
            argument++;
            continue;
          }
