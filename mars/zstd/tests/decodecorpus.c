...
static size_t writeLiteralsBlockCompressed(U32* seed, frame_t* frame, size_t contentSize) {
  ...
  switch (sizeFormat) {
    ...
    case 1: {
      U32
      const header = hType | (sizeFormat << 2) | ((U32)
      litSize << 4) |
      ((U32)
      compressedSize << 14);
...
static U32 generateSequences(U32* seed, frame_t* frame, seqStore_t* seqStore,
  ...
  if (literalsSize != contentSize) {
    ...
    U32
    const maxSequences = (U32)
    remainingMatch / MIN_SEQ_LEN;
  ...
  for (i = 0; i < numSequences; i++) {
    ...
    if (i + 1 == numSequences) matchLen = MIN_SEQ_LEN + (U32)
    excessMatch;
...
static size_t writeSequences(U32* seed, frame_t* frame, seqStore_t* seqStorePtr,
  ...
  {
    unsigned max = MaxLL;
    size_t const mostFrequent = HIST_countFast_wksp(count, &max, llCodeTable, nbSeq, WKSP,
    ...
    if (frame->stats.fseInit && !(RAND(seed) & 3) &&
      ...
      *op++ = llCodeTable[0];
      ...
      FSE_buildCTable_wksp(CTable_LitLength, LL_defaultNorm, MaxLL, LL_defaultNormLog, scratchBuffer,
  ...
  {
    unsigned max = MaxOff;
    size_t const mostFrequent = HIST_countFast_wksp(count, &max, ofCodeTable, nbSeq, WKSP,
                                                    sizeof(WKSP));   /* cannot fail */
    assert(!HIST_isError(mostFrequent));
    if (frame->stats.fseInit && !(RAND(seed) & 3) &&
      ...
      *op++ = ofCodeTable[0];
      ...
      FSE_buildCTable_wksp(CTable_OffsetBits, OF_defaultNorm, DefaultMaxOff, OF_defaultNormLog, scratchBuffer,
                           sizeof(scratchBuffer));
      Offtype = set_basic;
    } else {
      size_t nbSeq_1 = nbSeq;
  ...
  {
    unsigned max = MaxML;
    size_t const mostFrequent = HIST_countFast_wksp(count, &max, mlCodeTable, nbSeq, WKSP,
                                                    sizeof(WKSP));   /* cannot fail */
    assert(!HIST_isError(mostFrequent));
    if (frame->stats.fseInit && !(RAND(seed) & 3) &&
      ...
      *op++ = *mlCodeTable;
      ...
      FSE_buildCTable_wksp(CTable_MatchLength, ML_defaultNorm, MaxML, ML_defaultNormLog, scratchBuffer,
                           sizeof(scratchBuffer));
      MLtype = set_basic;
    } else {
      /* fall back on table */
...
static U32 generateCompressedBlock(U32 seed, frame_t* frame, dictInfo info) {
  ...
  while (!blockWritten) {
    ...
    if (cSize >=
        blockContentSize) {  /* compressed size must be strictly smaller than decompressed size : https://github.com/facebook/zstd/blob/dev/doc/zstd_compression_format.md#blocks */
