...
static const size_t maxMemory = (sizeof(size_t) == 4) ? (2 GB - 64
MB) : (size_t)(1ULL << ((sizeof(size_t)*8)-31));
...
static const U32 mintable[NUM_PARAMS] =
    {ZSTD_WINDOWLOG_MIN, ZSTD_CHAINLOG_MIN, ZSTD_HASHLOG_MIN, ZSTD_SEARCHLOG_MIN, ZSTD_MINMATCH_MIN,
     ZSTD_TARGETLENGTH_MIN, ZSTD_STRATEGY_MIN, FADT_MIN};

/* maximum value of parameters */
static const U32 maxtable[NUM_PARAMS] =
    {ZSTD_WINDOWLOG_MAX, ZSTD_CHAINLOG_MAX, ZSTD_HASHLOG_MAX, ZSTD_SEARCHLOG_MAX, ZSTD_MINMATCH_MAX,
     ZSTD_TARGETLENGTH_MAX, ZSTD_STRATEGY_MAX, FADT_MAX};
...
static const ZSTD_cParameter cctxSetParamTable[NUM_PARAMS] =
    {ZSTD_c_windowLog, ZSTD_c_chainLog, ZSTD_c_hashLog, ZSTD_c_searchLog, ZSTD_c_minMatch, ZSTD_c_targetLength,
...
static int
compareResultLT(const BMK_benchResult_t result1, const BMK_benchResult_t result2, const constraint_t target,
                size_t srcSize) {
...
redundantParams(const paramValues_t paramValues, const constraint_t target, const size_t maxBlockSize) {
  return
      (ZSTD_estimateCStreamSize_usingCParams(pvalsToCParams(paramValues)) >
       (size_t) target.cMem) /* Uses too much memory */
      || ((1ULL << (paramValues.vals[wlog_ind] - 1)) >= maxBlockSize &&
          paramValues.vals[wlog_ind] != mintable[wlog_ind]) /* wlog too much bigger than src size */
      || (paramValues.vals[clog_ind] > (paramValues.vals[wlog_ind] + (paramValues.vals[strt_ind] >
...
static void
BMK_printWinner(FILE* f, const int cLevel, const BMK_benchResult_t result, const paramValues_t params,
...
static void
BMK_printWinnerOpt(FILE* f, const U32 cLevel, const BMK_benchResult_t result, const paramValues_t params,
...
static int createBuffersFromMemory(buffers_t* buff, void* srcBuffer, const size_t nbFiles,
  ...
  if (!buff->srcPtrs || !buff->srcSizes || !buff->dstPtrs || !buff->dstCapacities || !buff->dstSizes ||
      !buff->resPtrs || !buff->resSizes) {
    DISPLAY("alloc error\n");
...
static int createBuffers(buffers_t* buff, const char* const* const fileNamesTable,
  ...
  for (n = 0; n < nbFiles; n++) {
    ...
    if (fileSize + pos > benchedSize)
      fileSize = benchedSize - pos, nbFiles = n;   /* buffer too small - stop after this file */
    {
...
static size_t
sanitizeVarArray(varInds_t* varNew, const size_t varLength, const varInds_t* varArray, const ZSTD_strategy strat) {
  size_t i, j = 0;
...
static void memoTableSet(const memoTable_t* memoTableArray, const paramValues_t p, const BYTE value) {
  ...
  switch (mt.tableType) {
    case directMap:
      ...
      break;
    case xxhashMap:
      mt.table[(XXH64(&p.vals, sizeof(U32) * NUM_PARAMS, 0) >> 3) % mt.tableLen] = value;
      break;
    case noMemo:
      break;
  }
...
createMemoTableArray(const paramValues_t p,
  ...
  for (i = stratMin; i <= stratMax; i++) {
    ...
    if (memoTableLog != PARAM_UNSET &&
        mtl > (1ULL << memoTableLog)) { /* use hash table */ /* provide some option to only use hash tables? */
...
static void BMK_init_level_constraints(int bytePerSec_level1) {
  ...
  /* establish speed objectives (relative to level 1) */
  {
    ...
    for (l = 2; l <= NB_LEVELS_TRACKED; l++) {
      ...
      g_level_constraint[l].windowLog_max = (l < 20) ? 23 : l +
...
static int BMK_seed(winnerInfo_t* winners,
  ...
  for (cLevel = 1; cLevel <= NB_LEVELS_TRACKED; cLevel++) {
    ...
    if ((double) testResult.cSize <= ((double) winners[cLevel].result.cSize * (1. + (0.02 / cLevel)))) {
      ...
      size_t
      W_DMemUsed = (1 << params.vals[wlog_ind]) + (16
      KB);
      size_t
      O_DMemUsed = (1 << winners[cLevel].params.vals[wlog_ind]) + (16
      KB);
      ...
      size_t O_CMemUsed = (1 << winners[cLevel].params.vals[wlog_ind]) +
                          ZSTD_estimateCCtxSize_usingCParams(pvalsToCParams(winners[cLevel].params));
      ...
      if (W_CSpeed_note < O_CSpeed_note) {
        ...
        if (W_ratio > O_ratio)
          DISPLAYLEVEL(3,
                       "Compression Speed : %5.3f @ %4.1f MB/s  vs  %5.3f @ %4.1f MB/s   : not enough for level %i\n",
      ...
      if (W_DSpeed_note < O_DSpeed_note) {
        ...
        if (W_ratio > O_ratio)
          DISPLAYLEVEL(3,
                       "Decompression Speed : %5.3f @ %4.1f MB/s  vs  %5.3f @ %4.1f MB/s   : not enough for level %i\n",
...
BMK_selectRandomStart(FILE* f,
  ...
  if ((id == 0) || (winners[id].params.vals[wlog_ind] == 0)) {
    ...
    paramValues_t const p = adjustParams(
        cParamsToPVals(pvalsToCParams(randomParams())), /* defaults nonCompression parameters */
        buf.srcSize, 0);
    playAround(f, winners, p, buf, ctx);
  } else {
...
benchOnce(const buffers_t buf, const contexts_t ctx, const int cLevel) {
  ...
  g_params = adjustParams(
      overwriteParams(cParamsToPVals(ZSTD_getCParams(cLevel, buf.maxBlockSize, ctx.dictSize)), g_params),
...
optimizeForSize(const char* const* const fileNamesTable, const size_t nbFiles,
  ...
  if (g_singleRun) {
    ...
    g_params = adjustParams(
        overwriteParams(cParamsToPVals(ZSTD_getCParams(cLevelRun, buf.maxBlockSize, ctx.dictSize)), g_params),
        buf.maxBlockSize, ctx.dictSize);
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (longCommandWArg(&argument, "--optimize=")) {
      ...
      for (;;) {
        ...
        if (longCommandWArg(&argument, "level=") || longCommandWArg(&argument, "lvl=")) {
          cLevelOpt = (int) readU32FromChar(&argument);
          g_optmode = 1;
          if (argument[0] == ',') {
            argument++;
            continue;
        ...
        if (longCommandWArg(&argument, "speedForRatio=") || longCommandWArg(&argument, "speedRatio=")) {
  ...
  if (filenamesStart == 0) {
    ...
    if (seperateFiles) {
      for (i = 0; i < argc - filenamesStart; i++) {
        if (g_optimizer) {
          result = optimizeForSize(argv + filenamesStart + i, 1, dictFileName, target, paramTarget, cLevelOpt,
                                   cLevelRun, memoTableLog);
          if (result) {
      ...
      if (g_optimizer) {
        ...
        result = optimizeForSize(argv + filenamesStart, (size_t)(argc - filenamesStart), dictFileName, target,
