...
static int basicUnitTests(U32 seed, double compressibility, ZSTD_customMem customMem) {
  ...
  genSize = CNBufferSize;
  {
    size_t const r = ZBUFF_decompressContinue(zd, decodedBuffer, &genSize, ((char*) compressedBuffer) + readSkipSize,
                                              &readSize);
    if (r != 0) goto _output_error;
  ...
  for (; (testNb <= nbTests) || (UTIL_clockSpanMicro(startClock) < g_clockTime); testNb++) {
    ...
    for (n = 0, cSize = 0, totalTestSize = 0; (n < nbChunks) && (totalTestSize < maxTestSize); n++) {
      /* compress random chunk into random size dst buffer */
      {
        ...
        size_t const compressionError = ZBUFF_compressContinue(zc, cBuffer + cSize, &dstBuffSize, srcBuffer + srcStart,
                                                               &readChunkSize);
    ...
    /* final frame epilogue */
    {
      ...
      while (remainingToFlush) {
        ...
        CHECK (enoughDstSize && remainingToFlush,
               "ZBUFF_compressEnd() not fully flushed (%u remaining), but enough space available",
    ...
    for (totalCSize = 0, totalGenSize = 0; errorCode;) {
      ...
      errorCode = ZBUFF_decompressContinue(zd, dstBuffer + totalGenSize, &dstBuffSize, cBuffer + totalCSize,
                                           &readCSrcSize);
    ...
    while ((totalCSize < cSize) && (totalGenSize < dstBufferSize)) {
      ...
      size_t const randomDstSize = FUZ_randomLength(&lseed, maxSampleLog);
      size_t dstBuffSize = MIN(dstBufferSize - totalGenSize, randomDstSize);
      size_t const decompressError = ZBUFF_decompressContinue(zd, dstBuffer + totalGenSize, &dstBuffSize,
