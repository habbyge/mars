...
/* ===   Compression   === */
typedef enum {
  ZWRAP_useInit, ZWRAP_useReset, ZWRAP_streamEnd
} ZWRAP_state_t;
...
ZWRAP_initializeCStream(ZWRAP_CCtx* zwc, const void* dict, size_t dictSize, unsigned long long pledgedSrcSize) {
  ...
  if (!pledgedSrcSize) pledgedSrcSize = zwc->pledgedSrcSize;
  {
    ...
    LOG_WRAPPERC("pledgedSrcSize=%d windowLog=%d chainLog=%d hashLog=%d searchLog=%d minMatch=%d strategy=%d\n",
                 (int) pledgedSrcSize, params.cParams.windowLog, params.cParams.chainLog, params.cParams.hashLog,
                 params.cParams.searchLog, params.cParams.minMatch, params.cParams.strategy);
...
z_deflateInit_ OF((z_streamp strm, int level,
...
strm->
state = (struct internal_state*) zwc; /* use state which in not used by user */
strm->
total_in = 0;
strm->
total_out = 0;
...
z_deflateInit2_ OF((z_streamp strm, int level, int method,
...
z_deflateReset OF((z_streamp strm)
...
z_deflateSetDictionary OF((z_streamp strm,
...
if (zwc->zbc == NULL) {
zwc->
zbc = ZSTD_createCStream_advanced(zwc->customMem);
if (zwc->zbc == NULL) return
ZWRAPC_finishWithError(zwc, strm,
0);
}
{
int res = ZWRAP_initializeCStream(zwc, dictionary, dictLength, ZSTD_CONTENTSIZE_UNKNOWN);
if (res != Z_OK) return
ZWRAPC_finishWithError(zwc, strm, res
); }
zwc->
comprState = ZWRAP_useReset;
}
...
z_deflate OF((z_streamp strm, int flush)
...
0);
{
int const initErr = ZWRAP_initializeCStream(zwc, NULL, 0,
                                            (flush == Z_FINISH) ? strm->avail_in : ZSTD_CONTENTSIZE_UNKNOWN);
if (initErr != Z_OK) return
ZWRAPC_finishWithError(zwc, strm, initErr
); }
if (flush != Z_FINISH) zwc->
comprState = ZWRAP_useReset;
} else {
...
strm->next_out += zwc->outBuffer.
pos;
strm->total_out += zwc->outBuffer.
pos;
strm->avail_out -= zwc->outBuffer.
pos;
strm->total_in += zwc->inBuffer.
pos;
zwc->totalInBytes += zwc->inBuffer.
pos;
strm->next_in += zwc->inBuffer.
pos;
strm->avail_in -= zwc->inBuffer.
pos;
}
...
|| flush == Z_BLOCK)
return
ZWRAPC_finishWithErrorMsg(strm,
"Z_FULL_FLUSH, Z_BLOCK and Z_TREES are not supported!");

if (flush == Z_FINISH) {
size_t bytesLeft;
if (zwc->streamEnd) return
Z_STREAM_END;
zwc->outBuffer.
dst = strm->next_out;
zwc->outBuffer.
...
z_deflateEnd OF((z_streamp strm)
...
ZWRAP_CCtx* zwc = (ZWRAP_CCtx*) strm->state;
if (zwc == NULL) return
Z_OK;  /* structures are already freed */
strm->
state = NULL;
...
return
ZSTD_compressBound(sourceLen);
}


ZEXTERN int ZEXPORT

z_deflateParams OF((z_streamp strm,
                    int level,
                    int strategy)
...
typedef enum {
  ZWRAP_ZLIB_STREAM, ZWRAP_ZSTD_STREAM, ZWRAP_UNKNOWN_STREAM
} ZWRAP_stream_type;
...
z_inflateInit_ OF((z_streamp strm,
                   const char* version, int stream_size)
...
if (zwd == NULL) return
ZWRAPD_finishWithError(zwd, strm,
0);

zwd->
version = (char*) ZSTD_malloc(strlen(version) + 1, zwd->customMem);
if (zwd->version == NULL) return
ZWRAPD_finishWithError(zwd, strm,
0);
strcpy(zwd
->version, version);
...
totalInBytes = 0;
strm->
state = (struct internal_state*) zwd;
strm->
total_in = 0;
strm->
total_out = 0;
strm->
reserved = ZWRAP_UNKNOWN_STREAM;
strm->
adler = 0;
}
...
z_inflateInit2_ OF((z_streamp strm, int windowBits,
                    const char* version, int stream_size)
...
z_inflateReset OF((z_streamp strm)
...
z_inflateSetDictionary OF((z_streamp strm,
                           const Bytef* dictionary,
                           uInt dictLength)
...
{
ZWRAP_DCtx* const zwd = (ZWRAP_DCtx*) strm->state;
if (zwd == NULL || zwd->zbd == NULL) return
Z_STREAM_ERROR;
{
size_t const initErr = ZSTD_initDStream_usingDict(zwd->zbd, dictionary, dictLength);
if (
ZSTD_isError(initErr)
) return
ZWRAPD_finishWithError(zwd, strm,
0); }
zwd->
decompState = ZWRAP_useReset;
...
z_inflate OF((z_streamp strm, int flush)
...
if (zwd == NULL) return
Z_STREAM_ERROR;
if (zwd->decompState == ZWRAP_streamEnd) return
Z_STREAM_END;
...
if (initErr != Z_OK) return
ZWRAPD_finishWithError(zwd, strm, initErr
);
}
...
memcpy(zwd
->headerBuf+zwd->totalInBytes, strm->next_in, srcSize);
strm->total_in +=
srcSize;
zwd->totalInBytes +=
srcSize;
...
strm->avail_in -=
srcSize;
if (zwd->totalInBytes < ZLIB_HEADERSIZE) return
Z_OK;

if (
MEM_readLE32(zwd
->headerBuf) != ZSTD_MAGICNUMBER) {
z_stream strm2;
strm2.
...
/* inflate header */
strm->
next_in = (unsigned char*) zwd->headerBuf;
strm->
avail_in = ZLIB_HEADERSIZE;
strm->
avail_out = 0;
{
...
if (dErr != Z_OK)
return
ZWRAPD_finishWithError(zwd, strm, dErr
);
}
if (strm->avail_in > 0) goto
error;

strm->
next_in = strm2.next_in;
strm->
avail_in = strm2.avail_in;
strm->
next_out = strm2.next_out;
strm->
avail_out = strm2.avail_out;

strm->
reserved = ZWRAP_ZLIB_STREAM; /* mark as zlib stream */
{
size_t const freeErr = ZWRAP_freeDCtx(zwd);
if (
ZSTD_isError(freeErr)
...
strm->
reserved = ZWRAP_ZSTD_STREAM; /* mark as zstd steam */

if (flush == Z_INFLATE_SYNC) {
strm->
msg = "inflateSync is not supported!";
goto
error;
}
...
if (!zwd->zbd) {
zwd->
zbd = ZSTD_createDStream_advanced(zwd->customMem);
if (zwd->zbd == NULL) {
...
if (zwd->inBuffer.pos != zwd->inBuffer.size) goto
error; /* not consumed */
}
...
strm->next_out += zwd->outBuffer.
pos;
strm->total_out += zwd->outBuffer.
pos;
strm->avail_out -= zwd->outBuffer.
pos;
strm->total_in += zwd->inBuffer.
pos;
zwd->totalInBytes += zwd->inBuffer.
pos;
strm->next_in += zwd->inBuffer.
pos;
strm->avail_in -= zwd->inBuffer.
pos;
if (dErr == 0) {
LOG_WRAPPERD("inflate Z_STREAM_END1 avail_in=%d avail_out=%d total_in=%d total_out=%d\n",
             (int) strm->avail_in, (int) strm->avail_out, (int) strm->total_in, (int) strm->total_out);
zwd->
decompState = ZWRAP_streamEnd;
return
Z_STREAM_END;
}
}  /* dErr lifetime */
...
z_inflateEnd OF((z_streamp strm)
...
z_inflateSync OF((z_streamp strm)
...
z_deflateCopy OF((z_streamp dest,
                  z_streamp source)
...
return
ZWRAPC_finishWithErrorMsg(source,
"deflateCopy is not supported!");
}


ZEXTERN int ZEXPORT

z_deflateTune OF((z_streamp strm,
                  int good_length,
                  int max_lazy,
                  int nice_length,
                  int max_chain)
...
z_deflatePrime OF((z_streamp strm,
                   int bits,
                   int value)
...
return
ZWRAPC_finishWithErrorMsg(strm,
"deflatePrime is not supported!");
}


ZEXTERN int ZEXPORT

z_deflateSetHeader OF((z_streamp strm,
                       gz_headerp head)
...
z_inflateCopy OF((z_streamp dest,
                  z_streamp source)
...
return
ZWRAPD_finishWithErrorMsg(source,
"inflateCopy is not supported!");
}
...
z_inflatePrime OF((z_streamp strm,
                   int bits,
                   int value)
...
return
ZWRAPD_finishWithErrorMsg(strm,
"inflatePrime is not supported!");
}


ZEXTERN int ZEXPORT

z_inflateGetHeader OF((z_streamp strm,
                       gz_headerp head)
...
z_inflateBackInit_ OF((z_streamp strm, int windowBits,
                       unsigned char FAR
...
return
ZWRAPD_finishWithErrorMsg(strm,
"inflateBackInit is not supported!");
}


ZEXTERN int ZEXPORT

z_inflateBack OF((z_streamp strm,
                  in_func in, void FAR
...
return
ZWRAPD_finishWithErrorMsg(strm,
"inflateBack is not supported!");
}


ZEXTERN int ZEXPORT

z_inflateBackEnd OF((z_streamp strm)
...
return
ZWRAPD_finishWithErrorMsg(strm,
"inflateBackEnd is not supported!");
}


ZEXTERN uLong
ZEXPORT z_zlibCompileFlags
OF((void)) {
return
...
z_compress OF((Bytef* dest, uLongf* destLen,
...
z_compress2 OF((Bytef* dest, uLongf* destLen,
                const Bytef* source, uLong sourceLen,
                int level)
...
z_uncompress OF((Bytef* dest, uLongf* destLen,
                 const Bytef* source, uLong sourceLen)

)
{
if (!
ZSTD_isFrame(source, sourceLen
))
return
