...
static int BMK_benchMem(z_const void* srcBuffer, size_t srcSize,
  ...
  /* checks */
  if (!compressedBuffer || !resultBuffer || !blockTable || !ctx || !dctx) EXM_THROW(31,
...
#       define NB_MARKS 4
    ...
    while (!cCompleted | !dCompleted) {
      ...
      if (!cCompleted) {   /* still some time to do compression tests */
        ...
        if (compressor == BMK_ZSTD) {
          ...
          ZSTD_CDict* const cdict = ZSTD_createCDict_advanced(dictBuffer, dictBufferSize, ZSTD_dlm_byRef, ZSTD_dct_auto,
                                                              zparams.cParams, cmem);
        ...
        } else if (compressor == BMK_ZWRAP_ZLIB_REUSE || compressor == BMK_ZWRAP_ZSTD_REUSE ||
                   compressor == BMK_ZLIB_REUSE) {
          z_stream def;
          int ret;
          ...
          do {
            ...
            for (blockNb = 0; blockNb < nbBlocks; blockNb++) {
              ...
              def.next_in = (z_const
              z_Bytef *) blockTable[blockNb].srcPtr;
              ...
              if (ret != Z_STREAM_END) EXM_THROW(1, "deflate failure ret=%d srcSize=%d", ret,
                                                 (int) blockTable[blockNb].srcSize);
      ...
      (void) fastestD;
      (void) crcOrig;   /*  unused when decompression disabled */
#if 1
      /* Decompression */
          ...
          do {
            ...
            for (blockNb = 0; blockNb < nbBlocks; blockNb++) {
              size_t const regenSize = ZSTD_decompress_usingDDict(dctx,
                                                                  blockTable[blockNb].resPtr,
                                                                  blockTable[blockNb].srcSize,
          ...
          do {
            ...
            for (blockNb = 0; blockNb < nbBlocks; blockNb++) {
              if (ZWRAP_isUsingZSTDdecompression(&inf))
                ret = ZWRAP_inflateReset_keepDict(
                    &inf); /* reuse dictionary to make decompression faster; inflate will return Z_NEED_DICT only for the first time */
              else
                ret = inflateReset(&inf);
              if (ret != Z_OK) EXM_THROW(1, "inflateReset failure");
              inf.next_in = (z_const
              z_Bytef *) blockTable[blockNb].cPtr;
    ...
    if (g_displayLevel == 1) {
      ...
      if (g_additionalParam)
        DISPLAY("-%-3i%11i (%5.3f) %6.2f MB/s %6.1f MB/s  %s (param=%d)\n", cLevel, (int) cSize, ratio, cSpeed, dSpeed,
...
static void BMK_loadFiles(void* buffer, size_t bufferSize,
  ...
  for (n = 0; n < nbFiles; n++) {
    ...
    if (fileSize > bufferSize - pos)
      fileSize = bufferSize - pos, nbFiles = n;   /* buffer too small - stop after this file */
    {
...
static void BMK_benchFileTable(const char** fileNamesTable, unsigned nbFiles,
  ...
  if (dictFileName != NULL) {
    ...
    if (dictBuffer == NULL) EXM_THROW(11, "not enough memory for dictionary (%u bytes)", (unsigned) dictBufferSize);
...
int main(int argCount, char** argv) {
  ...
  {
    size_t pos;
    for (pos = (int) strlen(programName); pos > 0; pos--) {
      if (programName[pos] == '/') {
        pos++;
        break;
      }
  ...
  for (argNb = 1; argNb < argCount; argNb++) {
    ...
    if (nextArgumentIsFile == 0) {
      ...
      if (!strcmp(argument, "--help")) {
        displayOut = stdout;
        CLEAN_RETURN(usage(programName));
      }
