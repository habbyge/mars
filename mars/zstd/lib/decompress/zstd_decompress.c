...
size_t ZSTD_decompressContinue(ZSTD_DCtx* dctx, void* dst, size_t dstCapacity, const void* src, size_t srcSize) {
  ...
  switch (dctx->stage) {
    ...
    case ZSTDds_decompressLastBlock:
    case ZSTDds_decompressBlock: DEBUGLOG(5, "ZSTD_decompressContinue: case ZSTDds_decompressBlock");
      {
        size_t rSize;
        switch (dctx->bType) {
...
size_t ZSTD_decompressStream(ZSTD_DStream* zds, ZSTD_outBuffer* output, ZSTD_inBuffer* input) {
  ...
  while (someMoreWork) {
    switch (zds->streamStage) {
      ...
      case zdss_loadHeader : DEBUGLOG(5, "stage zdss_loadHeader (srcSize : %u)", (U32) (iend - ip));
#if defined(ZSTD_LEGACY_SUPPORT) && (ZSTD_LEGACY_SUPPORT >= 1)
        ...
        if (zds->fParams.frameContentSize && zds->fParams.windowSize /* skippable frame if == 0 */
          ...
          if (cSize <= (size_t) (iend - istart)) {
            ...
            size_t const decompressedSize = ZSTD_decompress_usingDDict(zds, op, oend - op, istart, cSize,
                                                                       ZSTD_getDDict(zds));
        ...
        /* Adapt buffer sizes to frame header instructions */
        {
          ...
          size_t const neededOutBuffSize = ZSTD_decodingBufferSize_min(zds->fParams.windowSize,
