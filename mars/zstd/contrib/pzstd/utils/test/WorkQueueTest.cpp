...
i < 5; ++i) {
threads.
emplace_back(Popper{&queue, results.data(), &mutex}
);
}
...
std::mutex mutex;
std::cerr << "Creating popperThreads" <<
std::endl;
std::vector<std::thread> popperThreads;
for (
int i = 0;
...
std::cerr << "Creating pusherThreads" <<
std::endl;
std::vector<std::thread> pusherThreads;
for (
int i = 0;
...
std::cerr << "Joining pusherThreads" <<
std::endl;
for (
auto& thread
: pusherThreads) {
...
std::cerr << "Finishing queue" <<
std::endl;
queue.

finish();

std::cerr << "Joining popperThreads" <<
std::endl;
for (
auto& thread
: popperThreads) {
