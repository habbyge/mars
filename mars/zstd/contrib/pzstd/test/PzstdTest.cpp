...
inputFiles = {inputFile};
options.
numThreads = std::min(numThreads, options.numThreads);
options.
compressionLevel = level;
options.
verbosity = 1;
...
unsigned seed = std::random_device{}();
std::fprintf(stderr,
"Pzstd.ExtremelyLargeSize seed: %u\n", seed);
std::mt19937 gen(seed);
...
if (written != kLength) {
std::fprintf(stderr,
"Failed to write file, skipping test\n");
return;
}
}
}
