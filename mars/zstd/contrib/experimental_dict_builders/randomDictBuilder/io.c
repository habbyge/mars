...
static const size_t g_maxMemory = (sizeof(size_t) == 4) ?
                                  (2 GB - 64
MB) : ((size_t)(512 MB) << sizeof(size_t));
...
static unsigned loadFiles(void* buffer, size_t* bufferSizePtr, size_t* sampleSizes,
  ...
  for (fileIndex = 0; fileIndex < nbFiles; fileIndex++) {
    ...
    size_t
    const maxChunkSize = (size_t) MIN(chunkSize, SAMPLESIZE_MAX);
    U32 cnb;
...
static fileStats getFileStats(const char** fileNamesTable, unsigned nbFiles,
  ...
  for (n = 0; n < nbFiles; n++) {
    ...
    size_t
    const cappedChunkSize = (size_t) MIN(chunkToLoad, SAMPLESIZE_MAX);
...
sampleInfo* getSampleInfo(const char** fileNamesTable, unsigned nbFiles, size_t chunkSize,
  ...
  /* Checks */
  if ((!sampleSizes) || (!srcBuffer)) EXM_THROW(12, "not enough memory for trainFromFiles");   /* should not happen */
