...
struct WorkspaceDeleter {
  ...
  template<typename T>
  void operator()(T const*) { free(memory); }
};
...
std::string
decompress(ZSTD_DCtx& dctx, std::string const& compressed, size_t decompressedSize, std::string const& dict = "") {
...
dctx.
reset(ZSTD_initDCtx(d.memory, ZSTD_DCtxWorkspaceBound())
);
// Repeat
{
auto cctx = createCCtx(1);
...
{
compressed.
resize(ZSTD_compressBound(kData.size())
);
