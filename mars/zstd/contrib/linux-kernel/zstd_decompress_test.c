...
static int zstd_decompress_test_release(struct inode* i, struct file* f) {
  ...
  printk(KERN_INFO
  DEVICE_NAME ": uncompressed_len = %llu\n", uncompressed_len);
  printk(KERN_INFO
  DEVICE_NAME ": compressed_len   = %llu\n", compressed_len);
  printk(KERN_INFO
  DEVICE_NAME ": closed\n");
  return 0;
}
...
if (
copy_from_user(buffer_in, buf, copy_size
))
return -
EFAULT;
buf +=
...
uncompressed_len += sizeof(buffer_out) - state.
avail_out;
if (ret != Z_OK && ret != Z_STREAM_END) {
printk(KERN_INFO
DEVICE_NAME ": zlib decompress error %d: %s\n", ret, state.msg);
return -
EIO;
}
}
...
zstd_decompress_test_init(void) {
  ...
  printk(KERN_INFO
  DEVICE_NAME ": decompression requires %zu bytes of memory\n", workspace_size);
  printk(KERN_INFO
  ...
  fail:
  printk(KERN_INFO
  DEVICE_NAME ": failed to load module\n");
  if (workspace) {
    vfree(workspace);
    workspace = NULL;
  }
