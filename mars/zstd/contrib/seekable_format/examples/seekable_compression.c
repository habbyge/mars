...
static void compressFile_orDie(const char* fname, const char* outName, int cLevel, unsigned frameSize) {
  ...
  while ((read = fread_orDie(buffIn, toRead, fin))) {
    ...
    while (input.pos < input.size) {
      ...
      toRead = ZSTD_seekable_compressStream(cstream, &output,
      ...
      if (toRead > buffInSize)
        toRead = buffInSize;   /* Safely handle case when `buffInSize` is manually changed to a value < ZSTD_CStreamInSize()*/
