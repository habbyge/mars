...
BMK_benchMemAdvancedNoAlloc(
    ...
    const BMK_advancedParams_t* adv) {
  size_t const blockSize = ((adv->blockSize >= 32 && (adv->mode != BMK_decodeOnly)) ? adv->blockSize : srcSize) +
  ...
  /* Init data blocks  */
  {
    ...
    for (nbBlocks = 0, fileNb = 0; fileNb < nbFiles; fileNb++) {
      ...
      U32 const nbBlocksforThisFile = (adv->mode == BMK_decodeOnly) ? 1 : (U32)(
          (remaining + (blockSize - 1)) / blockSize);
      ...
      for (; nbBlocks < blockEnd; nbBlocks++) {
        ...
        resSizes[nbBlocks] = (adv->mode == BMK_decodeOnly) ? (size_t) ZSTD_findDecompressedSize(srcPtr, thisBlockSize)
                                                           : thisBlockSize;
    ...
    if (displayLevel == 1) {   /* hidden display mode -q, used by python speed benchmark */
      ...
      if (adv->additionalParam) {
        DISPLAY("-%-3i%11i (%5.3f) %6.2f MB/s %6.1f MB/s  %s (param=%d)\n", cLevel, (int) cSize, ratio, cSpeed, dSpeed,
                displayName, adv->additionalParam);
      } else {
...
static int BMK_loadFiles(void* buffer, size_t bufferSize,
  ...
  for (n = 0; n < nbFiles; n++) {
    ...
    }
    {
      ...
      if (fileSize > bufferSize - pos) fileSize = bufferSize -
