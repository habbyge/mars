...
static unsigned parseCoverParameters(const char* stringPtr, ZDICT_cover_params_t* params) {
  ...
  DISPLAYLEVEL(4, "cover: k=%u\nd=%u\nsteps=%u\nsplit=%u\nshrink%u\n", params->k, params->d, params->steps,
               (unsigned) (params->splitPoint * 100), params->shrinkDictMaxRegression);
  return 1;
}
...
static unsigned parseFastCoverParameters(const char* stringPtr, ZDICT_fastCover_params_t* params) {
  ...
  if (stringPtr[0] != 0) return 0;
  DISPLAYLEVEL(4, "cover: k=%u\nd=%u\nf=%u\nsteps=%u\nsplit=%u\naccel=%u\nshrink=%u\n", params->k, params->d, params->f,
               params->steps, (unsigned) (params->splitPoint * 100), params->accel, params->shrinkDictMaxRegression);
  return 1;
}
...
static unsigned parseAdaptParameters(const char* stringPtr, int* adaptMinPtr, int* adaptMaxPtr) {
  for (;;) {
    if (longCommandWArg(&stringPtr, "min=")) {
      *adaptMinPtr = (int) readU32FromChar(&stringPtr);
      if (stringPtr[0] == ',') {
        stringPtr++;
        continue;
      }
    ...
    if (longCommandWArg(&stringPtr, "max=")) {
      *adaptMaxPtr = (int) readU32FromChar(&stringPtr);
      if (stringPtr[0] == ',') {
        stringPtr++;
        continue;
      }
...
static unsigned parseCompressionParameters(const char* stringPtr, ZSTD_compressionParameters* params) {
  for (;;) {
    ...
    if (longCommandWArg(&stringPtr, "minMatch=") || longCommandWArg(&stringPtr, "mml=")) {
    ...
    if (longCommandWArg(&stringPtr, "ldmHashLog=") || longCommandWArg(&stringPtr, "lhlog=")) {
    ...
    if (longCommandWArg(&stringPtr, "ldmMinMatch=") || longCommandWArg(&stringPtr, "lmml=")) {
    ...
    if (longCommandWArg(&stringPtr, "ldmBucketSizeLog=") || longCommandWArg(&stringPtr, "lblog=")) {
    ...
    if (longCommandWArg(&stringPtr, "ldmHashRateLog=") || longCommandWArg(&stringPtr, "lhrlog=")) {
  ...
  DISPLAYLEVEL(4, "windowLog=%d, chainLog=%d, hashLog=%d, searchLog=%d \n", params->windowLog, params->chainLog,
               params->hashLog, params->searchLog);
  DISPLAYLEVEL(4, "minMatch=%d, targetLength=%d, strategy=%d \n", params->minMatch, params->targetLength,
               params->strategy);
  if (stringPtr[0] != 0) return 0; /* check the end of string */
  return 1;
}
...
typedef enum {
  zom_compress, zom_decompress, zom_test, zom_bench, zom_train, zom_list
} zstd_operation_mode;
...
int main(int argCount, const char* argv[]) {
    ...
    if (nextArgumentsAreFiles == 0) {
      ...
      if (argument[0] == '-') {
        ...
        if (argument[1] == '-') {
          ...
          if (!strcmp(argument, "--output-dir-flat")) {
            nextArgumentIsOutDirName = 1;
            lastCommand = 1;
            continue;
          }
          ...
          if (longCommandWArg(&argument, "--adapt=")) {
            adapt = 1;
            if (!parseAdaptParameters(argument, &adaptMin, &adaptMax)) CLEAN_RETURN(badusage(programName));
            continue;
          }
          ...
          if (!strcmp(argument, "--format=zstd")) {
            ...
            FIO_setCompressionType(prefs, FIO_zstdCompression);
            continue;
          }
#ifdef ZSTD_GZCOMPRESS
          ...
          if (!strcmp(argument, "--compress-literals")) {
            literalCompressionMode = ZSTD_lcm_huffman;
            continue;
          }
          if (!strcmp(argument, "--no-compress-literals")) {
            literalCompressionMode = ZSTD_lcm_uncompressed;
            continue;
          }
          ...
          if (!strcmp(argument, "--exclude-compressed")) {
          ...
          if (longCommandWArg(&argument, "--zstd=")) {
            if (!parseCompressionParameters(argument, &compressionParams)) CLEAN_RETURN(badusage(programName));
            continue;
          }
          if (longCommandWArg(&argument, "--stream-size=")) {
            streamSrcSize = readU32FromChar(&argument);
            continue;
          }
          if (longCommandWArg(&argument, "--target-compressed-block-size=")) {
            targetCBlockSize = readU32FromChar(&argument);
            continue;
          }
          if (longCommandWArg(&argument, "--size-hint=")) {
            srcSizeHint = readU32FromChar(&argument);
            continue;
          }
  ...
  if (operation == zom_bench) {
    ...
    if (cLevelLast > cLevel) DISPLAYLEVEL(3, "Benchmarking levels from %d to %d\n", cLevel, cLevelLast);
    if (filenameIdx) {
      if (separateFiles) {
        unsigned i;
        for (i = 0; i < filenameIdx; i++) {
          ...
          for (c = cLevel; c <= cLevelLast; c++) {
            BMK_benchFilesAdvanced(&filenameTable[i], 1, dictFileName, c, &compressionParams, g_displayLevel,
                                   &benchParams);
          }
        }
      } else {
        for (; cLevel <= cLevelLast; cLevel++) {
          BMK_benchFilesAdvanced(filenameTable, filenameIdx, dictFileName, cLevel, &compressionParams, g_displayLevel,
                                 &benchParams);
        }
      }
    } else {
  ...
  if (operation == zom_train) {
    ...
    if (dict == cover) {
      ...
      operationResult = DiB_trainFromFiles(outFileName, maxDictSize, filenameTable, filenameIdx, blockSize, NULL,
                                           &coverParams, NULL, optimize);
    } else if (dict == fastCover) {
      int const optimize = !fastCoverParams.k || !fastCoverParams.d;
      fastCoverParams.nbThreads = (unsigned) nbWorkers;
      fastCoverParams.zParams = zParams;
      operationResult = DiB_trainFromFiles(outFileName, maxDictSize, filenameTable, filenameIdx, blockSize, NULL, NULL,
                                           &fastCoverParams, optimize);
    } else {
      ZDICT_legacy_params_t dictParams;
      memset(&dictParams, 0, sizeof(dictParams));
      dictParams.selectivityLevel = dictSelect;
      dictParams.zParams = zParams;
      operationResult = DiB_trainFromFiles(outFileName, maxDictSize, filenameTable, filenameIdx, blockSize, &dictParams,
                                           NULL, NULL, 0);
    }
#else
  ...
  if (outFileName && !strcmp(outFileName, stdoutmark)
      ...
      && !forceStdout
      && operation != zom_decompress) CLEAN_RETURN(badusage(programName));
  ...
  if (!strcmp(filenameTable[0], stdinmark) && outFileName && !strcmp(outFileName, stdoutmark) &&
      (g_displayLevel == 2))
    g_displayLevel = 1;
  ...
  if (operation == zom_compress) {
    ...
    if ((filenameIdx == 1) && outFileName)
      operationResult = FIO_compressFilename(prefs, outFileName, filenameTable[0], dictFileName, cLevel,
                                             compressionParams);
    else
      operationResult = FIO_compressMultipleFilenames(prefs, filenameTable, filenameIdx, outDirName, outFileName,
                                                      suffix, dictFileName, cLevel, compressionParams);
#else
    ...
    if (filenameIdx == 1 && outFileName)
      ...
      operationResult = FIO_decompressMultipleFilenames(prefs, filenameTable, filenameIdx, outDirName, outFileName,
                                                        dictFileName);
#else
