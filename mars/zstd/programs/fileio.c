...
void FIO_setCompressionType(FIO_prefs_t* const prefs,
                            FIO_compressionType_t compressionType) { prefs->compressionType = compressionType; }
...
void FIO_setExcludeCompressedFile(FIO_prefs_t* const prefs,
                                  int excludeCompressedFiles) { prefs->excludeCompressedFiles = excludeCompressedFiles; }

void FIO_setBlockSize(FIO_prefs_t* const prefs, int blockSize) {
  if (blockSize && prefs->nbWorkers == 0) DISPLAYLEVEL(2, "Setting block size is useless in single-thread mode \n");
  prefs->blockSize = blockSize;
}

void FIO_setOverlapLog(FIO_prefs_t* const prefs, int overlapLog) {
  if (overlapLog && prefs->nbWorkers == 0) DISPLAYLEVEL(2, "Setting overlapLog is useless in single-thread mode \n");
  prefs->overlapLog = overlapLog;
}

void FIO_setAdaptiveMode(FIO_prefs_t* const prefs, unsigned adapt) {
  if ((adapt > 0) && (prefs->nbWorkers == 0)) EXM_THROW(1,
                                                        "Adaptive mode is not compatible with single thread mode \n");
  prefs->adaptiveMode = adapt;
}

void FIO_setRsyncable(FIO_prefs_t* const prefs, int rsyncable) {
  if ((rsyncable > 0) && (prefs->nbWorkers == 0)) EXM_THROW(1,
...
static size_t FIO_createDictBuffer(void** bufferPtr, const char* fileName) {
  ...
  *bufferPtr = malloc((size_t) fileSize);
  if (*bufferPtr == NULL) EXM_THROW(34, "%s", strerror(errno));
  {
    ...
    if (readSize != fileSize) EXM_THROW(35, "Error reading dictionary file %s : %s",
...
static cRess_t FIO_createCResources(FIO_prefs_t* const prefs,
  ...
  if (ress.cctx == NULL) EXM_THROW(30, "allocation error (%s): can't create ZSTD_CCtx",
                                   strerror(errno));
  ...
  /* Advanced parameters, including dictionary */
  {
    ...
    if (dictFileName && (dictBuffer == NULL)) EXM_THROW(32, "allocation error : can't create dictBuffer");
    ...
    CHECK(ZSTD_CCtx_setParameter(ress.cctx, ZSTD_c_contentSizeFlag,
                                 1));  /* always enable content size when available (note: supposed to be default) */
    ...
    *readsize += inSize;
    ...
    while ((inBuff.pos != inBuff.size)   /* input buffer must be entirely ingested */
      ...
      if (oldIPos == inBuff.pos)
        inputBlocked++;  /* input buffer is full and can't take any more : input speed is faster than consumption rate */
      ...
      if (outBuff.pos) {
        ...
        if (sizeCheck != outBuff.pos) EXM_THROW(25, "Write error : %s (cannot write compressed block)",
                                                strerror(errno));
      ...
      if (READY_FOR_UPDATE()) {
        ...
        if (g_display_prefs.displayLevel >= 3) {
          ...
          if (fileSize != UTIL_FILESIZE_UNKNOWN) DISPLAYLEVEL(2, "/ %u ", (unsigned) (fileSize >> 20));
        ...
        if (prefs->adaptiveMode) {
          ...
          if (zfp.currentJobID > 1) {  /* only possible if nbWorkers >= 1 */
            ...
            if ((zfp.consumed ==
                 previous_zfp_update.consumed)   /* no data compressed : no data available, or no more buffer to compress to, OR compression is really slow (compression of a single block is slower than update rate)*/
            ...
            if ((newlyProduced > (newlyFlushed * 9 /
                                  8))   /* compression produces more data than output can flush (though production can be spiky, due to work unit : (N==4)*block sizes) */
                && (flushWaiting ==
                    0)                        /* flush speed was never slowed by lack of production, so it's operating at max capacity */
                ) {
              DISPLAYLEVEL(6,
                           "compression faster than flush (%llu > %llu), and flushed was never slowed down by lack of production => slow down \n",
          ...
          if (zfp.currentJobID > lastJobID) {
            ...
            if (zfp.currentJobID > (unsigned) (prefs->nbWorkers + 1)) {   /* warm up period, to fill all workers */
              if (inputBlocked <= 0) {
                ...
                if ((inputBlocked > inputPresented /
                                    8)     /* input is waiting often, because input buffers is full : compression or output too slow */
...
FIO_compressFilename_internal(FIO_prefs_t* const prefs,
  ...
  switch (prefs->compressionType) {
    ...
    case FIO_lzmaCompression:
      ...
      (void) compressionLevel;
      EXM_THROW(20,
                "zstd: %s: file cannot be compressed as xz/lzma (zstd compiled without ZSTD_LZMACOMPRESS) -- ignored \n",
                srcFileName);
#endif
      break;
...
FIO_compressFilename_srcFile(FIO_prefs_t* const prefs,
  ...
  */
  ...
  if (prefs->removeSrcFile   /* --rm */
     ...
     */
    clearHandler();
    if (FIO_remove(srcFileName)) EXM_THROW(1, "zstd: %s: %s", srcFileName, strerror(errno));
  }
  return result;
}
...
int FIO_compressMultipleFilenames(FIO_prefs_t* const prefs,
  ...
  if (outFileName != NULL) {   /* output into a single destination (stdout typically) */
    ...
    if (ress.dstFile == NULL) {  /* could not open outFileName */
      ...
      if (fclose(ress.dstFile)) EXM_THROW(29, "Write error (%s) : cannot properly close %s",
...
static dRess_t FIO_createDResources(FIO_prefs_t* const prefs, const char* dictFileName) {
  ...
  if (ress.dctx == NULL) EXM_THROW(60, "Error: %s : can't create ZSTD_DStream", strerror(errno));
...
FIO_fwriteSparse(const FIO_prefs_t* const prefs,
  ...
  if (!prefs->sparseFileSupport) {  /* normal write */
    ...
    if (sizeCheck != bufferSize) EXM_THROW(70, "Write error : cannot write decoded block : %s",
                                           strerror(errno));
    return 0;
  }
  ...
  if (storedSkips > 1 GB) {
    ...
    if (seekResult != 0) EXM_THROW(91, "1 GB skip error (sparse file support)");
    storedSkips -= 1 GB;
  }
  ...
  while (ptrT < bufferTEnd) {
    ...
    if (nb0T != seg0SizeT) {   /* not all 0s */
      ...
      ptrT += nb0T;
      {
        ...
        if (sizeCheck != seg0SizeT) EXM_THROW(93, "Write error : cannot write decoded block : %s",
                                              strerror(errno));
      }
    }
    ptrT += seg0SizeT;
...

  {
    ...
    if (bufferSize & maskT) {
      ...
      if (restPtr != restEnd) {
        ...
        if (seekResult) EXM_THROW(94, "Sparse skip error ; try --no-sparse");
        storedSkips = 0;
        {
          size_t const sizeCheck = fwrite(restPtr, 1, (size_t)(restEnd - restPtr), file);
          if (sizeCheck != (size_t)(restEnd - restPtr)) EXM_THROW(95,
                                                                  "Write error : cannot write decoded end of block : %s",
                                                                  strerror(errno));
        }
      }
    }
...
FIO_fwriteSparseEnd(const FIO_prefs_t* const prefs, FILE* file, unsigned storedSkips) {
  ...
  if (storedSkips > 0) {
    ...
    if (LONG_SEEK(file, storedSkips - 1, SEEK_CUR) != 0) EXM_THROW(69, "Final skip error (sparse file support)");
    /* last zero must be explicitly written,
     * so that skipped ones get implicitly translated as zero by FS */
    {
      const char lastZeroByte[1] = {0};
      if (fwrite(lastZeroByte, 1, 1, file) != 1) EXM_THROW(69, "Write error : cannot write last zero : %s",
                                                           strerror(errno));
    }
  }
}
...
*/
static int
FIO_decompressSrcFile(FIO_prefs_t* const prefs, dRess_t ress, const char* dstFileName, const char* srcFileName) {
  FILE* srcFile;
  int result;
...
FIO_determineDstName(const char* srcFileName, const char* outDirName) {
  ...
  if (dfnbCapacity + srcSuffixLen <= sfnSize + 1 + dstSuffixLen) {
    ...
    dstFileNameBuffer = (char*) malloc(dfnbCapacity);
    if (dstFileNameBuffer == NULL) EXM_THROW(74, "%s : not enough memory for dstFileName",
                                             strerror(errno));
  }
...
FIO_decompressMultipleFilenames(FIO_prefs_t* const prefs,
  ...
  if (outFileName) {
    ...
    if ((!prefs->testMode) && (fclose(ress.dstFile))) EXM_THROW(72,
