...
public:
  ...
  AsyncResult1(const T& _func)
      : m_function(m_function_holder), m_callback_function(m_callback_function_holder), m_result(m_result_holder),
        m_result_valid(m_result_valid_holder), m_function_holder(_func), m_result_valid_holder(false) {
    BOOST_STATIC_ASSERT(boost::is_same<boost::result_of<T()>::type, R>::value);
  }

  template<typename T>
  AsyncResult1(const T& _func, R& _return_holder)
      : m_function(m_function_holder), m_callback_function(m_callback_function_holder), m_result(_return_holder),
        m_result_valid(m_result_valid_holder), m_function_holder(_func), m_result_valid_holder(false) {
    BOOST_STATIC_ASSERT(boost::is_same<boost::result_of<T()>::type, R>::value);
  }
  ...
  AsyncResult1(const T& _func, const C& _callback)
      : m_function(m_function_holder), m_callback_function(m_callback_function_holder), m_result(m_result_holder),
        m_result_valid(m_result_valid_holder), m_function_holder(_func), m_callback_function_holder(_callback),
        m_result_valid_holder(false) {
    BOOST_STATIC_ASSERT(boost::is_same<boost::result_of<T()>::type, R>::value);
  }
  ...
  AsyncResult1(const AsyncResult1& _ref)
      : m_function(_ref.m_function), m_callback_function(_ref.m_callback_function), m_result(_ref.m_result),
        m_result_valid(_ref.m_result_valid) {}
...
int ret = 0;
MessageQueue::AsyncResult<int> result(&test_0, ret, &callback);
EXPECT_NE(MessageQueue::KNullPost, MessageQueue::AsyncInvoke(result, MessageQueue::GetDefAsyncInvokeHandler(
    MessageQueue::GetDefTaskQueue()))
);
ThreadUtil::sleep(2);
...
MessageQueue::MessagePost_t post = MessageQueue::AsyncInvoke(result, MessageQueue::GetDefAsyncInvokeHandler(
    MessageQueue::GetDefTaskQueue()));
...
EXPECT_NE(result,
true);
EXPECT_NE(sg_callback_false,
106);
}
{
sg_callback_false = 0;
MessageQueue::MessagePost_t post = MessageQueue::AsyncInvoke(
    MessageQueue::AsyncResult<int&>(boost::bind(test_2, 107), &callback_false),
    MessageQueue::GetDefAsyncInvokeHandler(MessageQueue::GetDefTaskQueue()));
MessageQueue::CancelMessage(post);

EXPECT_EQ(sg_callback_false,
-1);
}
...
{
{
SingletonMessage(true,

MessageQueue::GetDefAsyncInvokeHandler(MessageQueue::GetDefTaskQueue()),
    MessageQueue::Message(123, WaitMessage_SingletonMessage_test), MessageQueue::MessageTiming(2 * 1000)

);
MessageQueue::WaitMessage(MessageQueue::SingletonMessage(false, MessageQueue::GetDefAsyncInvokeHandler(
    MessageQueue::GetDefTaskQueue()),
                                                         MessageQueue::Message(123, WaitMessage_SingletonMessage_test),
                                                         MessageQueue::MessageTiming(2 * 1000))
);
}
