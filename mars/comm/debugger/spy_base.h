...
public:
  ...
  template<class var>
  void Attach(const char* _key, var _var) {
  ...
  template<class var>
  var* Variable(const char* _key) const { return boost::any_cast<var>(m_variablemap.find(_key)->second); }
...
public:
  ...
  Spy* GetSpy(const void* _this) const { return m_thismap.find(_this)->second; }

  Spy* GetSpy(const char* _name) const { return m_strmap.find(_name)->second; }
