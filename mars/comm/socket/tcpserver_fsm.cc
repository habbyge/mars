...
TcpServerFSM::TcpServerFSM(SOCKET _socket)
  ...
  xerror2_if(0 > getpeername(sock_, (sockaddr*) &addr_, &addr_len), TSF"getpeername:%_, %_", socket_errno,
...
void TcpServerFSM::Close(bool _notify) {
  char ip[16];
  xinfo2(TSF"sock:%_, (%_:%_), onclose local socket close, notify:%_", sock_,
         socket_inet_ntop(AF_INET, &(addr_.sin_addr), ip, sizeof(ip)), ntohs(addr_.sin_port), _notify);
...
TcpServerFSM::TSocketStatus TcpServerFSM::AfterReadWriteSelect(const SocketSelect& _sel, XLogger& _log) {
  ...
  xinfo2(TSF"sock:%_, (%_:%_), ", sock_, socket_inet_ntop(AF_INET, &(addr_.sin_addr), ip, sizeof(ip)),
         ntohs(addr_.sin_port)) >> _log;
  ...
  if (!_sel.Write_FD_ISSET(sock_) && !_sel.Read_FD_ISSET(sock_) && 0 >= timeout) {
    xwarn2(TSF"onclose readwrite timeout:(%_, %_), (%_, %_)", ReadWriteAbsTimeout(), -timeout, SOCKET_ERRNO(ETIMEDOUT),
