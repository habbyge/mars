...
ComplexConnect::ComplexConnect(unsigned int _timeout, unsigned int _interval)
    : timeout_(_timeout), interval_(_interval), error_interval_(_interval), max_connect_(3), trycount_(0), index_(-1),
      errcode_(0), index_conn_rtt_(0), index_conn_totalcost_(0), totalcost_(0), is_interrupted_(false),
      each_IP_timeout_mode_(EachIPConnectTimoutMode::MODE_FIXED) {}

ComplexConnect::ComplexConnect(unsigned int _timeout /*ms*/, unsigned int _interval /*ms*/,
                               unsigned int _error_interval /*ms*/, unsigned int _max_connect)
    : timeout_(_timeout), interval_(_interval), error_interval_(_error_interval), max_connect_(_max_connect),
      trycount_(0), index_(-1), errcode_(0), index_conn_rtt_(0), index_conn_totalcost_(0), totalcost_(0),
      is_interrupted_(false), each_IP_timeout_mode_(EachIPConnectTimoutMode::MODE_FIXED) {}


ComplexConnect::ComplexConnect(unsigned int _timeout /*ms*/, unsigned int _interval /*ms*/,
                               EachIPConnectTimoutMode _mode)
    : timeout_(_timeout), interval_(_interval), error_interval_(_interval), max_connect_(3), trycount_(0), index_(-1),
      errcode_(0), index_conn_rtt_(0), index_conn_totalcost_(0), totalcost_(0), is_interrupted_(false),
      each_IP_timeout_mode_(_mode) {}
...
public:
  ...
  ConnectCheckFSM(const socket_address& _addr, unsigned int _connect_timeout, unsigned int _index,
                  MComplexConnect* _observer)
      : TcpClientFSM(_addr.address()), connect_timeout_(_connect_timeout), index_(_index), observer_(_observer),
        checkfintime_(0) {
...
public:
  ConnectHttpTunelCheckFSM(const socket_address& _destaddr, const socket_address& _proxy_addr,
                           const std::string& _proxy_username,
                           const std::string& _proxy_pwd, unsigned int _connect_timeout, unsigned int _index,
                           MComplexConnect* _observer)
      : ConnectCheckFSM(_proxy_addr, _connect_timeout, _index, _observer), destaddr_(_destaddr),
        username_(_proxy_username), password_(_proxy_pwd) {
  ...
  virtual void _OnRecv(AutoBuffer& _recv_buff, ssize_t _recv_len) {
    ...
    if (check_status_ == EProxyHttpTunelConnSvrReq) {
      ...
      if (200 == parser.Status().StatusCode()) {
        ...
        xwarn2(TSF"proxy error, proxy status code:%_, proxy info:%_:%_ resp:%_", parser.Status().StatusCode(),
               addr_.ip(), addr_.port(), xdump(_recv_buff.Ptr(), _recv_buff.Length()));
        check_status_ = ECheckFail;
      }

    } else if (check_status_ == EProxyConnSvrSucc) {
      check_status_ = (observer_ && observer_->OnVerifyRecv(index_, destaddr_, sock_, _recv_buff)) ? ECheckOK
                                                                                                   : ECheckFail;
      checkfintime_ = gettickcount();
    } else {
...
public:
  ...
  ConnectSocks5CheckFSM(const socket_address& _destaddr, const socket_address& _proxy_addr,
                        const std::string& _proxy_username,
                        const std::string& _proxy_pwd, unsigned int _connect_timeout, unsigned int _index,
                        MComplexConnect* _observer)
      : ConnectCheckFSM(_proxy_addr, _connect_timeout, _index, _observer), destaddr_(_destaddr),
  ...
  virtual void _OnRequestSend(AutoBuffer& _send_buff) {
    if (check_status_ == EConnProxySucc) {
      uint8_t socks5_greet_data[] = {kSocks5Version, kSockstNAuthMethos,
                                     (username_.empty() || password_.empty()) ? kSocks5NoAuth : kSocks5UsrNamePwdAuth};
      _send_buff.Length(0, 0);
...
SOCKET ComplexConnect::ConnectImpatient(const std::vector<socket_address>& _vecaddr,
  ...
  if (_vecaddr.empty()) {
    xwarn2(TSF"_vecaddr size:%_, m_timeout:%_, m_interval:%_, m_error_interval:%_, m_max_connect:%_, @%_",
           _vecaddr.size(), timeout_, interval_, error_interval_, max_connect_, this);
    return INVALID_SOCKET;
  }

  xinfo2(TSF"_vecaddr size:%_, m_timeout:%_, m_interval:%_, m_error_interval:%_, m_max_connect:%_, @%_",
         _vecaddr.size(), timeout_, interval_, error_interval_, max_connect_, this);
  ...
  do {
    ...
    for (unsigned int i = 0; i < index; ++i) {
      ...
      xgroup2_if(!group.Empty(), TSF"index:%_, @%_, status:%_,%_", i, this, vecsocketfsm[i]->Status(),
                 vecsocketfsm[i]->CheckStatus()) << group;

      if (TcpClientFSM::EEnd == vecsocketfsm[i]->Status()) {
        if (_observer)
          _observer->OnFinished(i, socket_address(&vecsocketfsm[i]->Address()), vecsocketfsm[i]->Socket(),
                                vecsocketfsm[i]->Error(),
                                vecsocketfsm[i]->Rtt(), vecsocketfsm[i]->TotalRtt(),
      ...
      if (TcpClientFSM::EReadWrite == vecsocketfsm[i]->Status() &&
          ConnectCheckFSM::ECheckFail == vecsocketfsm[i]->CheckStatus()) {
        if (_observer)
          _observer->OnFinished(i, socket_address(&vecsocketfsm[i]->Address()), vecsocketfsm[i]->Socket(),
                                vecsocketfsm[i]->Error(),
                                vecsocketfsm[i]->Rtt(), vecsocketfsm[i]->TotalRtt(),
      ...
      if (TcpClientFSM::EReadWrite == vecsocketfsm[i]->Status() &&
        ...
        xinfo2(TSF"index:%_, sock:%_, suc ConnectImpatient:%_:%_, RTT:(%_, %_), @%_", i, vecsocketfsm[i]->Socket(),
               vecsocketfsm[i]->IP(), vecsocketfsm[i]->Port(), vecsocketfsm[i]->Rtt(), vecsocketfsm[i]->TotalRtt(),
               this);
  ...
  xinfo2(TSF"retsocket:%_, connrtt:%_, conntotalrtt:%_, totalcost:%_, @%_", retsocket, index_conn_rtt_,
         index_conn_totalcost_, totalcost_, this);
