  ...
  if (error == 0) {
    for (res = res0; res; res = res->ai_next) {
      ...
      if (AF_INET6 == res->ai_family) {
        ...
        const char* ip_str = socket_inet_ntop(AF_INET, &(((sockaddr_in*) res->ai_addr)->sin_addr), ip_buf,
                                              sizeof(ip_buf));
...
#else
        memcpy((char*) &(_nat64_prefix_in6.s6_addr16), (char*) &((((sockaddr_in6*) res->ai_addr)->sin6_addr).s6_addr16),
               12);
#endif
        ret = true;
        break;
