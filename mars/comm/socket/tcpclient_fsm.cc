...
void TcpClientFSM::PreConnectSelect(SocketSelect& _sel, XLogger& _log) {
  ...
  if (0 != ret && !IS_NOBLOCK_CONNECT_ERRNO(socket_errno)) {
    ...
    error_ = socket_errno;
    xwarn2(TSF"close connect err:(%_, %_), localip:%_", error_, socket_strerror(error_),
           socket_address::getsockname(sock_).ip()) >> _log;
  } else {
    xinfo2("connect") >> _log;
    _sel.Write_FD_SET(sock_);
    _sel.Exception_FD_SET(sock_);
...
void TcpClientFSM::AfterConnectSelect(const SocketSelect& _sel, XLogger& _log) {
  ...
  if (0 >= timeout) {
    end_connecttime_ = gettickcount();
    xwarn2(TSF"close connect timeout:(%_, %_), (%_, %_)", ConnectAbsTimeout(), -timeout, SOCKET_ERRNO(ETIMEDOUT),
           socket_strerror(SOCKET_ERRNO(ETIMEDOUT))) >> _log;
...
void TcpClientFSM::AfterReadWriteSelect(const SocketSelect& _sel, XLogger& _log) {
  ...
  if (_sel.Write_FD_ISSET(sock_)) {
    ...
    if (0 < ret) {
      ...
      xwarn2(TSF"buffer full wait for next select, send err:(%_, %_, %_)", ret, socket_errno,
             socket_strerror(socket_errno)) >> _log;
    } else {
      error_ = socket_errno;
      last_status_ = status_;
      status_ = EEnd;
      xwarn2(TSF"close send err:(%_, %_, %_), localip:%_", ret, error_, socket_strerror(error_),
             socket_address::getsockname(sock_).ip()) >> _log;
      return;
    }
  }
  ...
  if (_sel.Read_FD_ISSET(sock_)) {
    ...
    ssize_t ret = recv(sock_, ((char*) recv_buf_.Ptr() + recv_buf_.Length()), recv_buf_.Capacity() - recv_buf_.Length(),
                       0);
    ...
    if (0 < ret) {
      ...
      xwarn2(TSF"buffer empty wait for next select, recv err:(%_, %_, %_)", ret, socket_errno,
             socket_strerror(socket_errno)) >> _log;
    } else {
      error_ = socket_errno;
      last_status_ = status_;
      status_ = EEnd;
      xwarn2(TSF"close recv %_:(%_, %_, %_), localip:%_", "err", ret, error_, socket_strerror(error_),
             socket_address::getsockname(sock_).ip()) >> _log;
      return;
    }
  }
  ...
  if (!_sel.Write_FD_ISSET(sock_) && !_sel.Read_FD_ISSET(sock_) && 0 >= timeout) {
    xwarn2(TSF"close readwrite timeout:(%_, %_), (%_, %_)", ReadWriteAbsTimeout(), -timeout, SOCKET_ERRNO(ETIMEDOUT),
           socket_strerror(SOCKET_ERRNO(ETIMEDOUT))) >> _log;
