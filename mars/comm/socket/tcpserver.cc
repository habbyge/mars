...
TcpServer::TcpServer(const char* _ip, uint16_t _port, MTcpServer& _observer, int _backlog)
    : observer_(_observer), thread_(boost::bind(&TcpServer::__ListenThread, this)), listen_sock_(INVALID_SOCKET),
      backlog_(_backlog) {
...
TcpServer::TcpServer(const sockaddr_in& _bindaddr, MTcpServer& _observer, int _backlog)
    : observer_(_observer), thread_(boost::bind(&TcpServer::__ListenThread, this)), listen_sock_(INVALID_SOCKET),
      bind_addr_(_bindaddr), backlog_(_backlog) {}
  ...
  do {
    ...
    while (true) {
      ...
      if (sel.Exception_FD_ISSET(listen_sock_)) {
        xerror2(TSF"socket exception err:(%_, %_)", socket_error(listen_sock_),
                socket_strerror(socket_error(listen_sock_))) >> break_group;
        break;
      }
      ...
      if (INVALID_SOCKET == client) {
        xerror2(TSF"accept return client invalid:%_, err:(%_, %_)", client, socket_errno, socket_strerror(socket_errno))
          >> break_group;
        break;
      }
      ...
      xinfo2(TSF"listen accept sock:(%_, %_:%_) cli:(%_, %_:%_)", listen_sock_, ip, ntohs(bind_addr_.sin_port), client,
