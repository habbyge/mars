...
static void __register_anr(uintptr_t _ptr, const char* _file, const char* _func, int _line, int _timeout, int _call_id,
                           void* extra_info) {
  ScopedLock lock(sg_mutex);
  __unregister_anr_impl(_ptr);
  ...
  check_content ch = {_ptr, _file, _func, _line, _timeout, xlogger_tid(), clock_app_monotonic(), 0, gettickcount(),
                      0/*init cpu_time*/, _call_id, extra_info};
...
static void __anr_checker_thread() {
  while (true) {
    ...
    if (sg_check_heap.empty()) {
      ...
      wait_timeout = iOS_style ? (sg_check_heap.front().timeout - sg_check_heap.front().used_cpu_time) : (
          sg_check_heap.front().end_time - clock_app_monotonic());
      if (wait_timeout < 0) {
        xwarn2("@%p", (void*) sg_check_heap.front().ptr)(TSF
                                                         "wait_timeout:%_, end_time:%_, used_cpu_time:%_, timeout:%_ now:%_, iOS_style：%_, anr_checker_size:%_",
                                                         wait_timeout,
                                                         sg_check_heap.front().end_time,
                                                         sg_check_heap.front().used_cpu_time,
                                                         sg_check_heap.front().timeout, iOS_style,
                                                         clock_app_monotonic(), sg_check_heap.size());
        wait_timeout = 0;
      }
    ...
    if (is_wait_timeout && round_tick_elapse > (wait_timeout + kTimeDeviation)) {
      xwarn2("@%p", (void*) sg_check_heap.front().ptr)(TSF
                                                       "now:%_, round_tick_start:%_, round_tick_elapse:%_, wait_timeout:%_, round cputime:%_, anr_checker_size:%_",
                                                       clock_app_monotonic(), round_tick_start, round_tick_elapse,
                                                       wait_timeout, use_cpu_time_2 - use_cpu_time_1,
                                                       sg_check_heap.size());
      iOS_style = true;
    }
    for (std::vector<check_content>::iterator it = sg_check_heap.begin(); it != sg_check_heap.end(); ++it) {
      if (use_cpu_time_2 >= use_cpu_time_1) {
        ...
        xassert2(false,
                 TSF"use_cpu_time_2:%_, use_cpu_time_1:%_, use_cpu_clock_2:%_, use_cpu_clock_1:%_, CLOCKS_PER_SEC:%_",
    ...
    if (iOS_style) {
      if (!sg_check_heap.empty() && (uint64_t) sg_check_heap.front().timeout <= sg_check_heap.front().used_cpu_time) {
        ...
        xassert2(sg_check_heap.front().end_time <= clock_app_monotonic(),
                 "end_time:%" PRIu64", now:%" PRIu64", anr_checker_size:%d, @%p", sg_check_heap.front().end_time,
                 clock_app_monotonic(), (int) sg_check_heap.size(),
                 (void*) sg_check_heap.front().ptr); //old logic is strict than new logic
      }
    } else {
