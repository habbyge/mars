...
private:
  ...
  public:
    RunnableReference(Runnable* _target)
        : target(_target), count(0), tid(0), isjoined(false), isended(true), aftertime(LONG_MAX),
          periodictime(LONG_MAX), iscanceldelaystart(false), condtime(), splock(), isinthread(false), killsig(0) {
...
public:
  ...
  int start_after(long after) {
    ...
    int ret = pthread_create(reinterpret_cast<thread_tid*>(&runable_ref_->tid), &attr_, start_routine_after,
                             runable_ref_);
    ASSERT(0 == ret);
  ...
  int start_periodic(long after, long periodic) { // ms
    ...
    int ret = pthread_create(reinterpret_cast<thread_tid*>(&runable_ref_->tid), &attr_, start_routine_periodic,
                             runable_ref_);
    ASSERT(0 == ret);
