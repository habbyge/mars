...
RequestLine::RequestLine(THttpMethod _httpMethod, const char* _url, THttpVersion _httpVersion)
    : http_method_(_httpMethod), req_url_(_url), http_version_(_httpVersion) {}
...
StatusLine::StatusLine()
    : http_version_(kVersion_1_0), statuscode_(0) {}
...
uint32_t HeaderFields::KeepAliveTimeout() const {
  ...
  std::string aliveConfig = (NULL == HeaderField(HeaderFields::KStringKeepalive) ? "" : HeaderField(
      HeaderFields::KStringKeepalive));
...
const std::string HeaderFields::ToString() const {
  ...
  for (std::map<const std::string, std::string, less>::const_iterator iter = headers_.begin();
...
Builder::Builder(TCsMode _csmode)
    : csmode_(_csmode), blockbody_(NULL), streambody_(NULL), is_manage_body_(false) {}
...
Parser::Parser(BodyReceiver* _body, bool _manage)
    : recvstatus_(kStart), response_header_ready_(false), csmode_(kRespond), headfields_(), bodyreceiver_(_body),
      is_manage_body_(_manage), firstlinelength_(0), headerlength_(0) {
}
...
Parser::TRecvStatus
Parser::Recv(const void* _buffer, size_t _length, size_t* consumed_bytes, bool only_parse_header/* = false*/) {
        ...
        if (bodyreceiver_) {
          ...
          if (headfields_.IsTransferEncodingChunked()) {
            ...
            if (Fields().IsConnectionClose() && 0 == contentLength) {
              ...
              xwarn2(TSF"recv len bigger than contentlen, (%_, %_, %_)", recvbuf_.Length(), bodyreceiver_->Length(),
                     contentLength);
        ...
        if (bodyreceiver_) {
          ...
          if (headfields_.IsTransferEncodingChunked()) {
            ...
            if (int64_t(_recv_buffer.Length() + bodyreceiver_->Length()) <= contentLength)
              ...
              xwarn2(TSF"contentLength:%_, body.len:%_, recv len:%_", contentLength, int64_t(bodyreceiver_->Length()),
                     _recv_buffer.Length());
              appendlen = contentLength - int64_t(bodyreceiver_->Length());
            }
