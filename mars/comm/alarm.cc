...
bool Alarm::Start(int _after, bool _needWake) {
  ...
  broadcast_msg_id_ = MessageQueue::BroadcastMessage(MessageQueue::GetDefMessageQueue(),
                                                     MessageQueue::Message(KALARM_MESSAGETITLE, (int64_t) seq,
  ...
  after_ = _after;
  seq_ = seq;
  xinfo2(TSF"alarm id:%_, after:%_, seq:%_, po.reg.q:%_,po.reg.s:%_,po.s:%_, MQ:%_", (uintptr_t) this, _after, seq,
         broadcast_msg_id_.reg.queue, broadcast_msg_id_.reg.seq, broadcast_msg_id_.seq,
         MessageQueue::GetDefMessageQueue());
...
void Alarm::OnAlarm(const MessageQueue::MessagePost_t& _id, MessageQueue::Message& _message) {
  ...
  if (MessageQueue::CurrentThreadMessageQueue() != MessageQueue::Handler2Queue(reg_async_.Get())) {
    MessageQueue::AsyncInvoke(boost::bind(&Alarm::OnAlarm, this, _id, _message), (MessageQueue::MessageTitle_t) this,
                              reg_async_.Get(), "Alarm::OnAlarm");
    return;
  }
  ...
  if (seq_ != boost::any_cast<int64_t>(_message.body1) || fromMQ != MessageQueue::GetDefMessageQueue()) {
    if (fromMQ != MessageQueue::GetDefMessageQueue()) {
      xinfo2(TSF"not match:(%_, %_), (%_, %_)", seq_, boost::any_cast<int64_t>(_message.body1),
             MessageQueue::GetDefMessageQueue(), fromMQ);
    }
    return;
  }
  ...
  xinfo2(TSF"OnAlarm id:%_, seq:%_, elapsed:%_, after:%_, miss:%_, android alarm:%_, MQ:%_", (uintptr_t) this, seq_,
         elapseTime, after_, -missTime, isSystemAlarm, MessageQueue::GetDefMessageQueue()) >> group;
