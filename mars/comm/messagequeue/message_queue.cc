...
struct MessageWrapper {
  MessageWrapper(const MessageHandler_t& _handlerid, const Message& _message, const MessageTiming& _timing,
                 unsigned int _seq)
...
struct HandlerWrapper {
  HandlerWrapper(const MessageHandler& _handler, bool _recvbroadcast, const MessageQueue_t& _messagequeueid,
                 unsigned int _seq)
...
static std::string DumpMessage(const std::list<MessageWrapper*>& _message_lst) {
  ...
  for (auto msg : _message_lst) {
    xmsg(TSF "postid:%_, timing:%_, record_time:%_, message:%_\n", msg->postid.ToString(), msg->timing.ToString(),
         msg->record_time, msg->message.ToString());
    if (++index > 50)
      break;
  }
  return xmsg.String();
...
MessageHandler_t
InstallMessageHandler(const MessageHandler& _handler, bool _recvbroadcast, const MessageQueue_t& _messagequeueid) {
...
MessagePost_t SingletonMessage(bool _replace, const MessageHandler_t& _handlerid, const Message& _message,
  ...
  MessageWrapper* messagewrapper = new MessageWrapper(_handlerid, _message, _timing,
                                                      0 != post_id.seq ? post_id.seq : __MakeSeq());
  content.lst_message.push_back(messagewrapper);
  content.breaker->Notify(lock);
  return messagewrapper->postid;
}
...
MessagePost_t
BroadcastMessage(const MessageQueue_t& _messagequeueid, const Message& _message, const MessageTiming& _timing) {
  ScopedLock lock(sg_messagequeue_map_mutex);
  const MessageQueue_t& id = _messagequeueid;
...
static void __ANRAssert(bool _iOS_style, const mars::comm::check_content& _content, MessageHandler_t _mq_id) {
  ...
  __ASSERT2(_content.file.c_str(), _content.line, _content.func.c_str(), "anr dead lock",
            "timeout:%d, tid:%" PRIu64 ", runing time:%" PRIu64 ", real time:%" PRIu64 ", used_cpu_time:%" PRIu64 ", iOS_style:%d",
            _content.timeout, _content.tid, clock_app_monotonic() - _content.start_time,
            gettickcount() - _content.start_tickcount, _content.used_cpu_time, _iOS_style);
#ifdef ANDROID
...
static void __ANRCheckCallback(bool _iOS_style, const mars::comm::check_content& _content) {
  ...
  MessageQueue::AsyncInvoke([=]() {
    if (thread->isruning()) {
      xinfo2(TSF"misjudge anr, timeout:%_, tid:%_, runing time:%_, real time:%_, used_cpu_time:%_, handler:(%_,%_)",
             _content.timeout,
             _content.tid, clock_app_monotonic() - _content.start_time, gettickcount() - _content.start_tickcount,
             _content.used_cpu_time, mq_id.queue, mq_id.seq);
      thread->cancel_after();
    }
  ...
  while (true) {
    ...
    for (std::list<HandlerWrapper*>::iterator it = content.lst_handler.begin(); it != content.lst_handler.end(); ++it) {
      if (messagewrapper->postid.reg == (*it)->reg ||
          ((*it)->recvbroadcast && messagewrapper->postid.reg.isbroadcast())) {
...
#endif
        ASSERT2(0 >= anr_timeout || anr_timeout >= (int64_t) (timeend - timestart),
                "anr_timeout:%" PRId64 " < cost:%" PRIu64", timestart:%" PRIu64", timeend:%" PRIu64, anr_timeout,
...
MessageQueueCreater::MessageQueueCreater(boost::shared_ptr<RunloopCond> _breaker, bool _iscreate,
...
MessageQueue_t
MessageQueueCreater::CreateNewMessageQueue(boost::shared_ptr<RunloopCond> _breaker, const char* _messagequeue_name) {
...
const MessageHandler_t& ScopeRegister::Get() const { return *m_reg; }
