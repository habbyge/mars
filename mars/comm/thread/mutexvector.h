...
public:
  ScopedMutexVector(MutexVector& _mutex, int _vector, bool _initiallyLocked = true)
      : mutex_vector_(_mutex), vec_(_vector), islocked_(false) { if (_initiallyLocked) Lock(); }
  ...
  void Lock() {
    ...
    while (mutex_vector_.m_count > 0 && mutex_vector_.m_vec != vec_) { mutex_vector_.m_cond.wait(lock); }
