...
std::string GetDetailNetInfo() {
  ...
  switch (::getNetInfo()) {
    ...
    case kWifi: {
      WifiInfo info;
      getCurWifiInfo(info);
      detail_net_info << "current network:wifi, ssid:" << info.ssid << ",ipstack:"
                      << TLocalIPStackStr[local_ipstack_detect()] << "\n";
    }
      break;
    ...
    case kMobile: {
      ...
      detail_net_info << ", ran:" << raninfo.radio_access_network << ",ipstack:"
                      << TLocalIPStackStr[local_ipstack_detect()] << "\n";
    }
      break;
  ...
  if (getifaddrs_ipv4_filter(v4_addrs, 0)) {
    for (size_t i = 0; i < v4_addrs.size(); ++i) {
      detail_net_info << "interface name:" << v4_addrs[i].ifa_name << ", "
                      << (v4_addrs[i].ifa_family == AF_INET ? "AF_INET" : "XX_INET")
                      << ", ip:" << v4_addrs[i].ip << "\n";
    }
  } else {
