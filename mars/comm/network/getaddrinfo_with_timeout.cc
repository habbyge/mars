...
struct DnsItem {
  ...
  std::string ToString() const {
    ...
    xmsg(TSF "node:%_, service:%_, hints:%_, res:%_, tid:%_, error_code:%_, status:%_",
         NULL == node ? "NULL" : node, NULL == service ? "NULL" : service, NULL == hints ? (void*) 0 : hints,
         NULL == res ? (void*) 0 : res, threadid, error_code, status);
    return xmsg.String();
  }
};
...
int getaddrinfo_with_timeout(const char* node, const char* service, const struct addrinfo* hints, struct addrinfo** res,
                             bool& _is_timeout, unsigned long long _timeout_msec) {
  xverbose_function();
  //Check param
