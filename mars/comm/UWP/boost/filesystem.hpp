...
std::time_t last_write_time(const path& p) {
  ...
  handle_wrapper hw(
      CreateFile2(ws.c_str(), GENERIC_READ, FILE_SHARE_DELETE | FILE_SHARE_READ | FILE_SHARE_WRITE, OPEN_EXISTING,
                  &para));
...
public:
  ...
  file_status status() const {
    m_status = mars_boost::filesystem::status(m_path);
    return m_status;
  }
...
private:
  ...
  bool dir_itr_first(void*& handle, const path& dir, std::string& target, file_status& sf, file_status& symlink_sf) {
    ...
    if ((handle = FindFirstFileExW(dirpath.c_str(), FindExInfoStandard, &data, FindExSearchNameMatch, NULL, 0)) ==
