...
static DWORD __SO_RCVTIMEO(SOCKET _sock) {
  ...
  ASSERT2(0 == getsockopt(_sock, SOL_SOCKET, SO_RCVTIMEO, &optval, &optlen), "%d, %s", WSAGetLastError(),
...
static void __SO_RCVTIMEO(SOCKET _sock, DWORD _ms) {
  DWORD optval = _ms;
  int optlen = sizeof(optval);
  ASSERT2(0 == setsockopt(_sock, SOL_SOCKET, SO_RCVTIMEO, (const char*) &optval, optlen), "%d, %s", socket_errno,
          gai_strerror(socket_errno));
}
...
SocketBreaker::SocketBreaker()
    : m_broken(false), m_create_success(true), m_exception(0) {
  ReCreate();
}
...
int SocketSelect::Select(int _msec) {
  ...
  for (std::map<SOCKET, int>::iterator it = m_filter_map.begin(); it != m_filter_map.end(); ++it) {
    ...
    ASSERT2(SOCKET_ERROR != WSAEventSelect(it->first, eventarray[index], it->second), "%d, %s", WSAGetLastError(),
  ...
  if (WSA_WAIT_FAILED != ret && WSA_WAIT_TIMEOUT != ret && 0 < ret - WSA_WAIT_EVENT_0) {
    ...
    int event_index = ret;
    ret = WSAEnumNetworkEvents(socketarray[event_index - WSA_WAIT_EVENT_0], eventarray[event_index - WSA_WAIT_EVENT_0],
                               &networkevents);
