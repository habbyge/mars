...
jfieldID VarCache::GetStaticFieldId(JNIEnv* _env, const char* const _class_path, const char* const _name,
...
bool LoadStaticMethod(JNIEnv* _env) {
  ...
  for (std::set<JniMethodInfo>::iterator iter = methoNameSet.begin(); iter != methoNameSet.end(); ++iter) {
    jmethodID mid = VarCache::Singleton()->GetStaticMethodId(_env, (*iter).classname.c_str(),
                                                             (*iter).methodname.c_str(), (*iter).methodsig.c_str());
...
bool LoadMethod(JNIEnv* env) {
  ...
  for (std::set<JniMethodInfo>::iterator iter = methoNameSet.begin(); iter != methoNameSet.end(); ++iter) {
    jmethodID mid = VarCache::Singleton()->GetMethodId(env, (*iter).classname.c_str(), (*iter).methodname.c_str(),
