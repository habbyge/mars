...
public:
  ...
  explicit Alarm(const T& _op, bool _inthread = true)
      : target_(detail::transform(_op)),
        ...
        broadcast_msg_id_(MessageQueue::KNullPost), runthread_(boost::bind(&Alarm::__Run, this), "alarm"),
        inthread_(_inthread), seq_(0), status_(kInit), after_(0), starttime_(0), endtime_(0),
  ...
  explicit Alarm(const T& _op, const MessageQueue::MessageQueue_t& _id)
      : target_(detail::transform(_op)), reg_async_(MessageQueue::InstallAsyncHandler(_id)),
        broadcast_msg_id_(MessageQueue::KNullPost), runthread_(boost::bind(&Alarm::__Run, this), "alarm"),
        inthread_(false), seq_(0), status_(kInit), after_(0), starttime_(0), endtime_(0),
        reg_(MessageQueue::InstallMessageHandler(boost::bind(&Alarm::OnAlarm, this, _1, _2), true))
#ifdef ANDROID
  , wakelock_(NULL)
  , type_(-1)
