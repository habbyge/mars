...
public:
  ...
  template<class V>
  bool Set(const std::string& key, const V& value) { return Set(currentsection, key, value); }
  ...
  const std::string& Get(const std::string& key, const std::string& def) { return Get(currentsection, key, def); }
...
template<typename V>
V INI::Get(const std::string& key, const V& def) { return Get(currentsection, key, def); }
