...
void Packer_Pack(const char* _url, unsigned int _sequence, const void* _data, size_t _datalen, AutoBuffer& _outbuf,
                 bool _dohash) {
  ASSERT(_url);
  ...
  if (_dohash) {
    ...
    if (NULL != _data && 0 < _datalen) {
      st.hash = (unsigned int) adler32(st.hash, (const unsigned char*) _data, (unsigned int) _datalen);
    }
  }
...
int Packer_Unpack(const void* _rawbuf, size_t _rawlen, std::string& _url, unsigned int& _sequence, size_t& _packlen,
                  AutoBuffer& _data) {
  ...
  if (0 != st.hash &&
      st.hash != adler32(0, (const unsigned char*) _rawbuf + st.head_length, st.total_length - st.head_length))
    return __LINE__;

  _data.Write((const char*) _rawbuf + st.head_length + st.url_length,
              st.total_length - (st.head_length + st.url_length));
...
int Packer_Unpack(const void* _rawbuf, size_t _rawlen, std::string& _url, unsigned int& _sequence, size_t& _packlen,
  ...
  if (0 != st.hash && st.hash != adler32(0, (const unsigned char*) _rawbuf + st.head_length,
                                         st.total_length - st.head_length)) { return __LINE__; }
