...
SmartHeartbeat::SmartHeartbeat() : report_smart_heart_(NULL), is_wait_heart_response_(false), success_heart_count_(0),
                                   last_heart_(MinHeartInterval),
                                   pre_heart_(MinHeartInterval), cur_heart_(MinHeartInterval),
                                   ini_(mars::app::GetAppFilePath() + "/" + kFileName, false), doze_mode_count_(0),
                                   normal_mode_count_(0), noop_start_tick_(false) {
  xinfo_function();
  ini_.Parse();
}
...
void SmartHeartbeat::OnHeartResult(bool _sucess, bool _fail_of_timeout) {
  ...
  if (report_smart_heart_ && !_sucess && success_heart_count_ >= NetStableTestCount &&
      current_net_heart_info_.is_stable_) {
  ...
  if (_sucess && current_net_heart_info_.is_stable_) {
    ...
    if ((cur_time - current_net_heart_info_.last_modify_time_) >= 7 * ONE_DAY_SECONEDS &&
      ...
      xinfo2(TSF"tryProbeBiggerHeart. curHeart=%_, last modify:%_", current_net_heart_info_.cur_heart_,
             current_net_heart_info_.last_modify_time_);
  ...
  if (_sucess) {
    if (current_net_heart_info_.succ_heart_count_ >= BaseSuccCount) {
      if (current_net_heart_info_.cur_heart_ >= (MaxHeartInterval - SuccessStep)) {
        ...
        xinfo2(TSF"%0 find the smart heart interval = %1", current_net_heart_info_.net_detail_,
               current_net_heart_info_.cur_heart_);
        if (report_smart_heart_)
        ...
        if (__IsDozeStyle()) {
          ...
          current_net_heart_info_.cur_heart_ = std::min((unsigned int) (MaxHeartInterval - SuccessStep),
                                                        current_net_heart_info_.cur_heart_);
        }
...
unsigned int SmartHeartbeat::GetNextHeartbeatInterval() {  //
  ...
  if (__IsDozeStyle() && current_net_heart_info_.heart_type_ != kDozeModeHeart &&
      last_heart_ != (MaxHeartInterval - SuccessStep)) {
...
void SmartHeartbeat::__LoadINI() {
  ...
  if (ini_.Select(net_info)) {
    ...
    xassert2(net_type == current_net_heart_info_.net_type_, "cur:%d, INI:%d", net_type,
             current_net_heart_info_.net_type_);

    if (current_net_heart_info_.cur_heart_ < MinHeartInterval) {
      xerror2(TSF"current_net_heart_info_.cur_heart_:%_ < MinHeartInterval:%_", current_net_heart_info_.cur_heart_,
              MinHeartInterval);
      current_net_heart_info_.cur_heart_ = MinHeartInterval;
    }

    if (current_net_heart_info_.cur_heart_ > MaxHeartInterval) {
      xerror2(TSF"current_net_heart_info_.cur_heart_:%_ > MaxHeartInterval:%_", current_net_heart_info_.cur_heart_,
              MaxHeartInterval);
      current_net_heart_info_.cur_heart_ = MaxHeartInterval - SuccessStep;
    }
    ...
    if (current_net_heart_info_.last_modify_time_ > cur_time) {
      xerror2(TSF"current_net_heart_info_.last_modify_time_:%_ > cur_time:%_",
...
void SmartHeartbeat::__DumpHeartInfo() {
  xinfo2(TSF"SmartHeartbeat Info last_heart_:%0,successHeartCount:%1, currSuccCount:%2", last_heart_,
         success_heart_count_, current_net_heart_info_.succ_heart_count_);
