...
NetCore::NetCore() : messagequeue_creater_(true, XLOGGER_TAG),
                     ...
                     shortlink_task_manager_(new ShortLinkTaskManager(*net_source_, *dynamic_timeout_,
                                                                      messagequeue_creater_.GetMessageQueue())),
                     shortlink_error_count_(0), shortlink_try_flag_(false) {
  ...
  xinfo2(TSF"netcore messagequeue_id=%_, handler:(%_,%_)",
         messagequeue_creater_.GetMessageQueue(), asyncreg_.Get().queue,
         asyncreg_.Get().seq);
...
void NetCore::__InitShortLink() {
  // async
  shortlink_task_manager_->fun_callback_ = boost::bind(&NetCore::__CallBack, this, (int) kCallFromShort, _1, _2, _3, _4,
                                                       _5);

  // sync
  shortlink_task_manager_->fun_notify_retry_all_tasks = boost::bind(&NetCore::RetryTasks, this, _1, _2, _3, _4, _5);
  shortlink_task_manager_->fun_notify_network_err_ = boost::bind(&NetCore::__OnShortLinkNetworkError, this, _1, _2, _3,
                                                                 _4, _5, _6);
...
void NetCore::__InitLongLink() {
  ...
  zombie_task_manager_->fun_callback_ = boost::bind(&NetCore::__CallBack, this, (int) kCallFromZombie, _1, _2, _3, _4,
                                                    _5);

  timing_sync_ = new TimingSync(*ActiveLogic::Instance());

  longlink_task_manager_ = new LongLinkTaskManager(*net_source_, *ActiveLogic::Instance(), *dynamic_timeout_,
                                                   GetMessageQueueId());
  ...
  // async
  longlink_task_manager_->fun_callback_ = boost::bind(&NetCore::__CallBack, this, (int) kCallFromLong, _1, _2, _3, _4,
                                                      _5);

  // sync
  longlink_task_manager_->fun_notify_retry_all_tasks = boost::bind(&NetCore::RetryTasks, this, _1, _2, _3, _4, _5);
  longlink_task_manager_->fun_notify_network_err_ = boost::bind(&NetCore::__OnLongLinkNetworkError, this, _1, _2, _3,
                                                                _4, _5, _6);
...
void NetCore::__Release(NetCore* _instance) {
  if (MessageQueue::CurrentThreadMessageQueue() != MessageQueue::Handler2Queue(_instance->asyncreg_.Get())) {
    WaitMessage(AsyncInvoke((MessageQueue::AsyncInvokeFunction) boost::bind(&NetCore::__Release, _instance),
                            _instance->asyncreg_.Get(), "NetCore::__Release"));
    return;
  }
...
void NetCore::StartTask(const Task& _task) {
  ...
  ASYNC_BLOCK_START
        ...
        xinfo2(
            TSF"task start long short taskid:%0, cmdid:%1, need_authed:%2, cgi:%3, channel_select:%4, limit_flow:%5, channel_name:%6",
            _task.taskid, _task.cmdid, _task.need_authed, _task.cgi.c_str(), _task.channel_select, _task.limit_flow,
            _task.channel_name) >> group;
        xinfo2(
            TSF"host:%_, send_only:%_, cmdid:%_, server_process_cost:%_, retrycount:%_,  channel_strategy:%_, channel_name:%_",
            ...
            _task.channel_name) >> group;
        xinfo2(TSF" total_timeout:%_, network_status_sensitive:%_, priority:%_, report_arg:%_", _task.total_timeout,
               _task.network_status_sensitive, _task.priority, _task.report_arg) >> group;
        ...
        switch (task.channel_select) {
          case Task::kChannelBoth: {
            ...
            if (bUseLongLink && task.channel_strategy == Task::kChannelFastStrategy) {
              ...
              bUseLongLink = bUseLongLink && (longlink_task_manager_->GetTaskCount(task.channel_name) <=
  ...
  switch (::getNetInfo()) {
    ...
    case kMobile: {
      ...
      xinfo2(TSF"task network change current network:mobile, ispname:%_, ispcode:%_, ran:%_, ip_stack:%_, log:%_",
             info.isp_name, info.isp_code, raninfo.radio_access_network, TLocalIPStackStr[ip_stack], ip_stack_log);
    }
      break;
...
void
NetCore::RetryTasks(ErrCmdType _err_type, int _err_code, int _fail_handle, uint32_t _src_taskid, std::string _user_id) {
...
int NetCore::__CallBack(int _from, ErrCmdType _err_type, int _err_code, int _fail_handle, const Task& _task,
...
void NetCore::__OnShortLinkResponse(int _status_code) {
  if (_status_code == 301 || _status_code == 302 || _status_code == 307) {
    ...
    xinfo2(TSF"status code:%0, long link status:%1, longlink task continue fail count:%2", _status_code,
           longlink_status, continues_fail_count);
...
void NetCore::__OnPush(const std::string& _channel_id, uint32_t _cmdid, uint32_t _taskid, const AutoBuffer& _body,
...
void NetCore::__OnLongLinkNetworkError(const std::string& _name, int _line, ErrCmdType _err_type, int _err_code,
...
void NetCore::__OnShortLinkNetworkError(int _line, ErrCmdType _err_type, int _err_code, const std::string& _ip,
...
std::shared_ptr<LongLink> NetCore::CreateLongLink(const LonglinkConfig& _config) {
  ...
  if (_config.IsMain()) {
    longlink_channel->fun_network_report_ = boost::bind(&NetCore::__OnLongLinkNetworkError, this, _config.name, _1, _2,
                                                        _3, _4, _5);
