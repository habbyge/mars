...
static constexpr unsigned long kLimitTimeSpan = 60 * 60 * 1000;  // 60 min
static constexpr size_t kLimitCount = 1;

static constexpr long long kMinCheckTimeSpan = 5 * 60 * 1000; //5 min
static constexpr long long kCheckTimeSpanIncrementStep = 10 * 60 * 1000; //10 min

static constexpr uint32_t kValidBitsFilter = 0xFFFFFFFF;  //32 bits stands for recent 32 tasks status
static constexpr uint32_t kMostRecentTaskStartN[2] = {24, 8};
static constexpr uint32_t kSecondRecentTaskStartN[2] = {16, 8};
//static const uint32_t MOST_OLD_TASK_START_N[2]				={1, 16};

static constexpr uint32_t kCheckifBelowCount = 3;
static constexpr uint32_t kCheckifAboveCount = 5;
...
NetCheckLogic::NetCheckLogic() : frequency_limit_(new CommFrequencyLimit(kLimitCount, kLimitTimeSpan)),
...
void NetCheckLogic::UpdateLongLinkInfo(unsigned int _continues_fail_count, bool _task_succ) {
  ...
  xdebug2("shortlink:_continueFailCount=%d, _isTaskSucc=%d, records=0x%x",
          _continues_fail_count, _task_succ, longlink_taskstatus_item_.records);
...
void NetCheckLogic::UpdateShortLinkInfo(unsigned int _continues_fail_count, bool _task_succ) {
  ...
  xdebug2("shortlink:_continues_fail_count: %d, _task_succ: %d, records=0x%x",
          _continues_fail_count, _task_succ, shortlink_taskstatus_item_.records);
...
bool NetCheckLogic::__ShouldNetCheck() {
  ...
  EXTRACT_N_BITS(shortlink_taskstatus_item_.records, kMostRecentTaskStartN[0],
                 kMostRecentTaskStartN[1], most_recent_shorttasks_status);
  ...
  if (is_shortlink_bad) { // 最近八次坏，前八次好，就进入netcheck
    ...
    xinfo2(TSF"netcheck: shortlink succ_count: %_, is most recent %_ times. valid_record_taskcount: %_.",
           succ_count, kMostRecentTaskStartN[1], valid_record_taskcount);

    succ_count = 0;
    uint32_t second_recent_shorttasks_status = 0;

    EXTRACT_N_BITS(shortlink_taskstatus_item_.records, kSecondRecentTaskStartN[0],
                   kSecondRecentTaskStartN[1], second_recent_shorttasks_status);

    CAL_BIT_COUNT(second_recent_shorttasks_status, succ_count);
    shortlink_shouldcheck = succ_count > kCheckifAboveCount;
    xinfo2(TSF"netcheck: shortlink_shouldcheck=%_, shortlink succ_count=%_, in sub-recent %_ times. ",
           shortlink_shouldcheck,
  ...
  EXTRACT_N_BITS(longlink_taskstatus_item_.records, kMostRecentTaskStartN[0],
                 kMostRecentTaskStartN[1], most_recent_longtasks_status);
  ...
  if (is_longlink_bad) { // 最近八次坏，前八次好，就进入netcheck
    unsigned int valid_record_taskcount = 0;
    CAL_BIT_COUNT(kValidBitsFilter, valid_record_taskcount);
    xinfo2(TSF"netcheck: longlink succ_count: %_, in most recent %_ times. valid_record_taskcount: %_.",
           succ_count, kMostRecentTaskStartN[1], valid_record_taskcount);
    succ_count = 0;
    uint32_t second_recent_longtasks_status = 0;

    EXTRACT_N_BITS(longlink_taskstatus_item_.records, kSecondRecentTaskStartN[0],
                   kSecondRecentTaskStartN[1], second_recent_longtasks_status);

    CAL_BIT_COUNT(second_recent_longtasks_status, succ_count);
    longlink_shouldcheck = succ_count > kCheckifAboveCount;
    xinfo2(TSF"netcheck: longlink_shouldcheck: %_, longlink succ_count: %_, in sub-recent %_ times. ",
           longlink_shouldcheck,
           succ_count, kSecondRecentTaskStartN[1]);
  }
  ...
  if (ret) {
    if (::gettickspan(last_netcheck_time_) <
        (kMinCheckTimeSpan + increment_steps * kCheckTimeSpanIncrementStep)) {
...
void NetCheckLogic::__StartNetCheck() {
  ...
  for (std::vector<std::string>::iterator host_iter = longlink_hosts.begin();
       host_iter != longlink_hosts.end(); ++host_iter) {
    ...
    for (std::vector<uint16_t>::iterator port_iter = longlink_portlist.begin();
         port_iter != longlink_portlist.end(); ++port_iter) {
      for (std::vector<std::string>::iterator ip_iter = longlink_iplist.begin();
           ip_iter != longlink_iplist.end(); ++ip_iter) {
    ...
    if (!check_ipport_list.empty()) {
      longlink_check_items.insert(std::pair<std::string,
          std::vector<CheckIPPort>>(*host_iter, check_ipport_list));
    }
  }
  ...
  for (std::vector<std::string>::iterator iter = shortlink_hostlist.begin();
       iter != shortlink_hostlist.end(); ++iter) {
    ...
    std::vector<CheckIPPort> check_ipport_list;
    for (std::vector<std::string>::iterator ip_iter = shortlink_iplist.begin();
         ip_iter != shortlink_iplist.end(); ++ip_iter) {
    ...
    if (!check_ipport_list.empty())
      shortlink_check_items.insert(std::pair<std::string, std::vector<CheckIPPort>>(
          *iter, check_ipport_list));
  }
