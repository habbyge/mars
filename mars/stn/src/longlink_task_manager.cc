...
boost::function<void(const std::string& _user_id,
    ...
    LongLinkTaskManager::get_real_host_;

static int longlink_id = 0;

LongLinkTaskManager::LongLinkTaskManager(NetSource& _netsource,
    ...
    , dynamic_timeout_(_dynamictimeout), netsource_(_netsource), active_logic_(_activelogic)
#ifdef ANDROID
...
void LongLinkTaskManager::__RedoTasks(const std::string& _name) {
  ...
  while (first != last) {
    ...
    if (longlink && first->running_id) {
      ...
      xerror2(TSF"didn't find longlink or running id is empty, task channel name:%_, running id:%_",
              first->task.channel_name, first->running_id);
...
void LongLinkTaskManager::__RunOnTimeout() {
  ...
  while (first != last) {
    ...
    if (first->running_id && 0 < first->transfer_profile.start_send_time) {
      if (0 == first->transfer_profile.last_receive_pkg_time &&
        ...
        xerror2(TSF"task first-pkg timeout taskid:%_,  nStartSendTime=%_, nfirstpkgtimeout=%_",
                first->task.taskid, first->transfer_profile.start_send_time / 1000,
      ...
      if (0 < first->transfer_profile.last_receive_pkg_time &&
        ...
        xerror2(TSF"task pkg-pkg timeout, taskid:%_, nLastRecvTime=%_, pkg-pkg timeout=%_",
                first->task.taskid, first->transfer_profile.last_receive_pkg_time / 1000,
    ...
    if (longlink && cur_time - first->start_task_time >= first->task_timeout) {
      ...
      xerror2(TSF"task timeout, taskid:%_, nStartSendTime=%_, cur_time=%_, timeout:%_",
              first->task.taskid, first->transfer_profile.start_send_time / 1000, cur_time / 1000,
  ...
  for (auto item : batchMap) {
    if (item.second.first == kEctLongTaskTimeout) {
      __BatchErrorRespHandle(item.first, kEctNetMsgXP, kEctLocalTaskTimeout, kTaskFailHandleDefault,
                             item.second.second);
    } else {
      xassert2(fun_notify_network_err_);
      auto longlink_channel = GetLongLink(item.first)->Channel();
      if (longlink_channel)
        fun_notify_network_err_(item.first, __LINE__, kEctNetMsgXP, item.second.first, longlink_channel->Profile().ip,
                                longlink_channel->Profile().port);
...
void LongLinkTaskManager::__RunOnStartTask() {
  ...
  while (first != last) {
    ...
    if (first->task.need_authed) {
      ...
      xinfo2(TSF"makesureauth host:%_, auth result:%_, cgi:%_, channal name:%_", host, ismakesureauthsuccess,
             first->task.cgi, first->task.channel_name);
    ...
    if (!first->antiavalanche_checked) {
      if (!Req2Buf(first->task.taskid, first->task.user_context, first->task.user_id, bufreq, buffer_extension,
                   error_code, Task::kChannelLong, host)) {
    ...
    if (!first->running_id) {
      xwarn2(TSF"task add into longlink readwrite fail cgi:%_, cmdid:%_, taskid:%_", first->task.cgi, first->task.cmdid,
             first->task.taskid);
      first = next;
      continue;
    }
    ...
    xinfo2(
        TSF"task add into longlink readwrite suc cgi:%_, cmdid:%_, taskid:%_, size:%_, channel name:%_, timeout(firstpkg:%_, rw:%_, task:%_), retry:%_, curtime:%_, start_send_time:%_, sendonly:%_",
        first->task.cgi, first->task.cmdid, first->task.taskid, first->transfer_profile.send_data_size,
        first->task.channel_name, first->transfer_profile.first_pkg_timeout / 1000,
        first->transfer_profile.read_write_timeout / 1000, first->task_timeout / 1000, first->remain_retry_count,
        curtime, first->start_task_time, first->task.send_only);
...
bool LongLinkTaskManager::__SingleRespHandle(std::list<TaskProfile>::iterator _it, ErrCmdType _err_type, int _err_code,
                                             int _fail_handle, const ConnectProfile& _connect_profile) {
  xinfo_function();
  ...
  if (0 >= _it->remain_retry_count || kEctOK == _err_type || kTaskFailHandleTaskEnd == _fail_handle ||
      kTaskFailHandleTaskTimeout == _fail_handle) {
    xlog2(kEctOK == _err_type ? kLevelInfo : kLevelWarn, TSF"task end callback  long cmdid:%_, err(%_, %_, %_), ",
      ...
      (TSF "svr(%_:%_, %_, %_), ", _connect_profile.ip, _connect_profile.port,
       IPSourceTypeString[_connect_profile.ip_type], _connect_profile.host)
          ...
          (TSF "cost(s:%_, r:%_%_%_, c:%_, rw:%_), all:%_, retry:%_, ", _it->transfer_profile.send_data_size,
           ...
           receive_data_size - received_size ? "/" : "", receive_data_size, _connect_profile.conn_rtt,
          ...
          (TSF "cgi:%_, taskid:%_, tid:%_, context id:%_", _it->task.cgi, _it->task.taskid, _connect_profile.tid,
           _it->task.user_id);
  ...
  xlog2(kEctOK == _err_type ? kLevelInfo : kLevelWarn, TSF"task end retry  long cmdid:%_, err(%_, %_, %_), ",
        _it->task.cmdid, _err_type, _err_code, _fail_handle)
    (TSF "svr(%_:%_, %_, %_), ", _connect_profile.ip, _connect_profile.port,
     IPSourceTypeString[_connect_profile.ip_type], _connect_profile.host)
        (TSF "cli(%_, %_, n:%_, sig:%_), ", _it->transfer_profile.external_ip, _connect_profile.local_ip,
...
void
LongLinkTaskManager::__BatchErrorRespHandleByUserId(const std::string& _user_id, ErrCmdType _err_type, int _err_code,
                                                    int _fail_handle, uint32_t _src_taskid,
  ...
  while (first != temp.end()) {
    ...
    __BatchErrorRespHandle(first->task.channel_name, _err_type, _err_code, _fail_handle, _src_taskid,
                           _callback_runing_task_only);
    ++first;
  }
}

void LongLinkTaskManager::__BatchErrorRespHandle(const std::string _name, ErrCmdType _err_type, int _err_code,
                                                 int _fail_handle, uint32_t _src_taskid,
                                                 bool _callback_runing_task_only) {
  xinfo2(TSF"batch error, channel name:%_, src id:%_ callback:%_, errcode:%_, errtype:%_, fail handle:%_", _name,
         _src_taskid, _callback_runing_task_only, _err_code, _err_type, _fail_handle);
...
void
LongLinkTaskManager::__OnResponse(const std::string& _name, ErrCmdType _error_type, int _error_code, uint32_t _cmdid,
                                  uint32_t _taskid, AutoBuffer& _body, AutoBuffer& _extension,
                                  const ConnectProfile& _connect_profile) {
  move_wrapper<AutoBuffer> body(_body);
  move_wrapper<AutoBuffer> extension(_extension);
  RETURN_LONKLINK_SYNC2ASYNC_FUNC_TITLE(
      boost::bind(&LongLinkTaskManager::__OnResponse, this, _name, _error_type, _error_code, _cmdid, _taskid, body,
                  extension, _connect_profile), longlink_id_[_name]);
  // svr push notify
  ...
  if (kEctOK != _error_type) {
    xwarn2(TSF"task error, taskid:%_, cmdid:%_, error_type:%_, error_code:%_", _taskid, _cmdid, _error_type,
           _error_code);
  ...
  int err_code = 0;
  int handle_type = Buf2Resp(it->task.taskid, it->task.user_context, it->task.user_id, body, extension, err_code,
                             Task::kChannelLong);
  ...
  switch (handle_type) {
    ...
    case kTaskFailHandleDefault: {
      xerror2(TSF"task decode error taskid:%_, handle_type:%_, err_code:%_, body dump:%_", it->task.taskid, handle_type,
              err_code, xdump(body->Ptr(), body->Length()));
    ...
    case kTaskSlientHandleTaskEnd: {
      xinfo2(TSF"task slient error taskid:%_, svr(%_:%_, %_, %_), handle_type:%_, err_code:%_, body dump:%_",
             it->task.taskid, _connect_profile.ip,
             _connect_profile.port, IPSourceTypeString[_connect_profile.ip_type], _connect_profile.host, handle_type,
             err_code,
...
void
LongLinkTaskManager::__SignalConnection(LongLink::TLongLinkStatus _connect_status, const std::string& _channel_id) {
...
bool LongLinkTaskManager::AddLongLink(const LonglinkConfig& _config) {
  ...
  longlink_metas_[_config.name] = std::make_shared<LongLinkMetaData>(_config, netsource_, active_logic_,
                                                                     asyncreg_.Get().queue);
  ...
  longlink->Channel()->OnResponse = boost::bind(&LongLinkTaskManager::__OnResponse, this, _1, _2, _3, _4, _5, _6, _7,
                                                _8);
...
void LongLinkTaskManager::ReleaseLongLink(const std::string _name) {
  ...
  while (task != lst_cmd_.end()) {
    if (task->task.channel_name == _name) {
      if (__SingleRespHandle(task, kEctLocal, kEctLocalLongLinkReleased, kTaskFailHandleTaskEnd,
                             longlink->Channel()->Profile())) {
        // task erased
        task = lst_cmd_.begin();
      } else {
        task++;
