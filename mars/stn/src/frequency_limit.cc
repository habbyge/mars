...
FrequencyLimit::FrequencyLimit() : itime_record_clear_(::gettickcount()) {}
...
bool FrequencyLimit::Check(const mars::stn::Task& _task,
  ...
  if (0 <= find_index) {
    ...
    if (!__CheckRecord(find_index)) {
      xerror2(
          TSF"Anti-Avalanche had Catch Task, Task Info: ptr=%0, cmdid=%1, need_authed=%2, cgi:%3, channel_select=%4, limit_flow=%5",
      ...
      xerror2(TSF"apBuffer Len=%0, Hash=%1, Count=%2, timeLastUpdate=%3",
              _len, iarr_record_[find_index].hash_, iarr_record_[find_index].count_,
