...
bool ProxyTest::ProxyIsAvailable(const mars::comm::ProxyInfo _proxy_info, const std::string& _host,
  ...
  xinfo_function(TSF"type:%_ host:%_ ip:%_:%_ username:%_ test_host:%_ hardcode_ip:%_", _proxy_info.type,
                 _proxy_info.host, _proxy_info.ip, _proxy_info.port, _proxy_info.username, _host,
                 _hardcode_ips.empty() ? "" : _hardcode_ips.front());
...
SOCKET ProxyTest::__Connect(const mars::comm::ProxyInfo& _proxy_info, const std::string& _host,
  ...
  SOCKET sock = com_connect.ConnectImpatient(vecaddr, testproxybreak_, NULL, _proxy_info.type, proxy_addr,
                                             _proxy_info.username, _proxy_info.password);
  delete proxy_addr;
...
int ProxyTest::__ReadWrite(SOCKET _sock, const mars::comm::ProxyInfo& _proxy_info, const std::string& _host) {
  ...
  if (_proxy_info.IsValid() && mars::comm::kProxyHttp == _proxy_info.type && !_proxy_info.username.empty() &&
      !_proxy_info.password.empty()) {
  ...
  if (mars::comm::kProxyHttp == _proxy_info.type) {
    url = std::string("http://") + _host +
          "/"; // the "/" is necessary, otherwise some proxy server will response: 400 bad request
  } else {
    url = "/";
  }
  ...
  int err_code = 0;
  int send_ret = block_socket_send(_sock, (const unsigned char*) out_buff.Ptr(), (unsigned int) out_buff.Length(),
                                   testproxybreak_, err_code);

  if (send_ret < 0) {
    xerror2(TSF"test proxy Error, ret:%0, errno:%1, nread:%_, nwrite:%_", send_ret, strerror(err_code),
            socket_nread(_sock), socket_nwrite(_sock));
    return -1;
  }
  ...
  while (true) {
    ...
    if (recv_ret == 0 && SOCKET_ERRNO(ETIMEDOUT) == err_code) {
      xerror2(TSF"read timeout error:(%_,%_), nread:%_, nwrite:%_ ", err_code, strerror(err_code), socket_nread(_sock),
              socket_nwrite(_sock));
      continue;
    }
    if (recv_ret == 0) {
      xerror2(TSF"remote disconnect, nread:%_, nwrite:%_", err_code, strerror(err_code), socket_nread(_sock),
              socket_nwrite(_sock));
      break;
    }
