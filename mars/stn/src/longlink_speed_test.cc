...
LongLinkSpeedTestItem::LongLinkSpeedTestItem(const std::string& _ip, uint16_t _port)
    : ip_(_ip), port_(_port), socket_(-1), state_(kLongLinkSpeedTestConnecting),
      before_connect_time_(0), after_connect_time_(0) {
  ...
  gDefaultLongLinkEncoder.longlink_pack(gDefaultLongLinkEncoder.longlink_noop_cmdid(),
                                        Task::kNoopTaskID, body,
                                        extension, req_ab_, NULL);
...
int LongLinkSpeedTestItem::__HandleSpeedTestResp() {
  ...
  if (nrecv <= 0) {
    xerror2(TSF"recv nrecv <= 0, errno:%0, resp_ab_.Capacity():%1,resp_ab_.Pos():%2", strerror(errno),
            resp_ab_.Capacity(), resp_ab_.Pos());
    return kLongLinkSpeedTestFail;
  } else {
    ...
    int nRet = gDefaultLongLinkEncoder.longlink_unpack(resp_ab_, anCmdID,
                                                       anSeq, pacLength,
                                                       body, extension, NULL);
    ...
    } else if (gDefaultLongLinkEncoder.longlink_noop_isresp(Task::kNoopTaskID,
                                                            anCmdID, anSeq,
                                                            body, extension)) {
...
LongLinkSpeedTest::LongLinkSpeedTest(const boost::shared_ptr<NetSource>& _netsource)
    : netsource_(_netsource), selector_(breaker_) {
...
bool LongLinkSpeedTest::GetFastestSocket(int& _fdSocket, std::string& _strIp,
