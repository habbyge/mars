...
public:
  LongLinkConnectObserver(LongLink& _longlink, const std::vector<IPPortItem>& _iplist) : longlink_(_longlink),
                                                                                         ip_items_(_iplist) {
  ...
  virtual void OnConnected(unsigned int _index, const socket_address& _addr, SOCKET _socket, int _error, int _rtt) {
    if (0 == _error) {
      ...
      xwarn2(TSF"index:%_, connnet fail host:%_, iptype:%_", _index, ip_items_[_index].str_host,
             ip_items_[_index].source_type);
  ...
  virtual bool
  OnVerifySend(unsigned int _index, const socket_address& _addr, SOCKET _socket, AutoBuffer& _buffer_send) {
    AutoBuffer body;
    AutoBuffer extension;
    longlink_.Encoder().longlink_noop_req_body(body, extension);
    longlink_.Encoder().longlink_pack(longlink_.Encoder().longlink_noop_cmdid(), Task::kNoopTaskID, body, extension,
                                      _buffer_send, NULL);
    return true;
  }

  virtual bool
  OnVerifyRecv(unsigned int _index, const socket_address& _addr, SOCKET _socket, const AutoBuffer& _buffer_recv) {
    ...
    if (LONGLINK_UNPACK_OK != ret) {
      xerror2(TSF"0>ret, index:%_, sock:%_, %_, ret:%_, cmdid:%_, taskid:%_, pack_len:%_, recv_len:%_", _index, _socket,
              _addr.url(), ret, cmdid, taskid, pack_len, _buffer_recv.Length());
      if (longlink_.fun_network_report_) {
    ...
    if (!longlink_.Encoder().longlink_noop_isresp(taskid, cmdid, taskid, bufferbody, extension)) {
      xwarn2(TSF"index:%_, sock:%_, %_, ret:%_, cmdid:%_, taskid:%_, pack_len:%_, recv_len:%_", _index, _socket,
             _addr.url(), ret, cmdid, taskid, pack_len, _buffer_recv.Length());
    }
...
LongLink::LongLink(const mq::MessageQueue_t& _messagequeueid, NetSource& _netsource, const LonglinkConfig& _config,
                   LongLinkEncoder& _encoder)
    : asyncreg_(MessageQueue::InstallAsyncHandler(_messagequeueid)), netsource_(_netsource), config_(_config),
      thread_(boost::bind(&LongLink::__Run, this), XLOGGER_TAG "::lonklink"), connectstatus_(kConnectIdle),
      disconnectinternalcode_(kNone), identifychecker_(_encoder, _config.name)
#ifdef ANDROID
    , smartheartbeat_(new SmartHeartbeat)
    , wakelock_(new WakeUpLock)
#else
    , smartheartbeat_(NULL), wakelock_(NULL)
#endif
    , encoder_(_encoder) {
...
bool
LongLink::SendWhenNoData(const AutoBuffer& _body, const AutoBuffer& _extension, uint32_t _cmdid, uint32_t _taskid) {
  ScopedLock lock(mutex_);
...
bool LongLink::__NoopResp(uint32_t _cmdid, uint32_t _taskid, AutoBuffer& _buf, AutoBuffer& _extension, Alarm& _alarm,
                          bool& _nooping, ConnectProfile& _profile) {
  bool is_noop = false;
...
void
LongLink::__RunResponseError(ErrCmdType _error_type, int _error_code, ConnectProfile& _profile, bool _networkreport) {
  ...
  if (_networkreport && fun_network_report_)
    fun_network_report_(__LINE__, _error_type, _error_code, _profile.ip, _profile.port);
}
...
void LongLink::__Run() {
  ...
  xinfo_function(TSF"LongLink Rebuild span:%_, net:%_, channel name:%_",
                 conn_profile_.disconn_time != 0 ? cur_time - conn_profile_.disconn_time : 0, getNetInfo(),
                 config_.name);
...
SOCKET LongLink::__RunConnect(ConnectProfile& _conn_profile) {
  ...
  bool use_proxy =
      proxy_info.IsValid() && mars::comm::kProxyNone != proxy_info.type && mars::comm::kProxyHttp != proxy_info.type &&
      netsource_.GetLongLinkDebugIP().empty();
  ...
  SOCKET sock = com_connect.ConnectImpatient(vecaddr, connectbreak_, &connect_observer, proxy_info.type, proxy_addr,
  ...
  xinfo2(
      TSF"task socket connect suc sock:%_, host:%_, ip:%_, port:%_, local_ip:%_, local_port:%_, iptype:%_, costtime:%_, rtt:%_, totalcost:%_, index:%_, net:%_",
      sock, _conn_profile.host, _conn_profile.ip, _conn_profile.port, _conn_profile.local_ip, _conn_profile.local_port,
      IPSourceTypeString[_conn_profile.ip_type], com_connect.TotalCost(), com_connect.IndexRtt(),
      com_connect.IndexTotalCost(), com_connect.Index(), ::getNetInfo());
  __ConnectStatus(kConnected);
  __UpdateProfile(_conn_profile);

  xerror2_if(0 != socket_disable_nagle(sock, 1), TSF"socket_disable_nagle sock:%0, %1(%2)", sock, socket_errno,
...
void LongLink::__RunReadWrite(SOCKET _sock, ErrCmdType& _errtype, int& _errcode, ConnectProfile& _profile) {
  ...
  while (true) {
    ...
    if (kNone != disconnectinternalcode_) {
      xwarn2(TSF"task socket close sock:%0, user disconnect:%1, nread:%_, nwrite:%_", _sock, disconnectinternalcode_,
             socket_nread(_sock), socket_nwrite(_sock)) >> close_log;
    ...
    if (0 > retsel) {
      xfatal2(TSF"task socket close sock:%0, 0 > retsel, errno:%_, nread:%_, nwrite:%_", _sock, sel.Errno(),
              socket_nread(_sock), socket_nwrite(_sock)) >> close_log;
      _errtype = kEctSocket;
      _errcode = sel.Errno();
      goto End;
    ...
    if (sel.IsException()) {
      xerror2(TSF"task socket close sock:%0, socketselect excptoin:%1(%2), nread:%_, nwrite:%_", _sock, socket_errno,
              socket_strerror(socket_errno), socket_nread(_sock), socket_nwrite(_sock)) >> close_log;
      _errtype = kEctSocket;
    ...
    if (sel.Exception_FD_ISSET(_sock)) {
      ...
      xerror2(TSF"task socket close sock:%0, excptoin:%1(%2), nread:%_, nwrite:%_", _sock, error,
              socket_strerror(error), socket_nread(_sock), socket_nwrite(_sock)) >> close_log;
      _errtype = kEctSocket;
      _errcode = error;
      goto End;
    }
    ...
    if (nooping && alarmnooptimeout.Status() == Alarm::kOnAlarm) {
      xerror2(TSF"task socket close sock:%0, noop timeout, nread:%_, nwrite:%_", _sock, socket_nread(_sock),
              socket_nwrite(_sock)) >> close_log;
//            __NotifySmartHeartbeatJudgeDozeStyle();
      _errtype = kEctSocket;
      _errcode = kEctSocketRecvErr;
      goto End;
...
ssize_t writelen = ::send(_sock, lstsenddata_.begin()->second->PosPtr(), lstsenddata_.begin()->second->PosLength(), 0);
      ...
      while (it != lstsenddata_.end() && 0 < writelen) {
        ...
        if ((size_t) writelen >= it->second->PosLength()) {
          xinfo2(TSF"sub send taskid:%_, cmdid:%_, %_, len(S:%_, %_/%_), ", it->first.taskid, it->first.cmdid,
                 it->first.cgi, it->second->PosLength(), it->second->PosLength(), it->second->Length()) >> xlog_group;
          ...
          xinfo2(TSF"sub send taskid:%_, cmdid:%_, %_, len(S:%_, %_/%_), ", it->first.taskid, it->first.cmdid,
                 it->first.cgi, writelen, it->second->PosLength(), it->second->Length()) >> xlog_group;
    ...
    if (sel.Read_FD_ISSET(_sock)) {
      ...
      if (0 > recvlen && !IS_NOBLOCK_READ_ERRNO(socket_errno)) {
        _errtype = kEctSocket;
        _errcode = socket_errno;
        xerror2(TSF"task socket close sock:%0, recv len: %1 errno:%2(%3)", _sock, recvlen, socket_errno,
      ...
      while (0 < bufrecv.Length()) {
        ...
        xinfo2(TSF"task socket recv sock:%_, pack recv %_ taskid:%_, cmdid:%_, %_, packlen:(%_/%_)", _sock,
               LONGLINK_UNPACK_CONTINUE == unpackret ? "continue" : "finish", taskid, cmdid, stream_resp.task.cgi,
               LONGLINK_UNPACK_CONTINUE == unpackret ? bufrecv.Length() : packlen, packlen);
        ...
        } else if (!__NoopResp(cmdid, taskid, stream_resp.stream, stream_resp.extension, alarmnooptimeout, nooping,
                               _profile)) {
          if (OnResponse)
  ...
  if (nread_size > 0 && _errtype != kEctNetMsgXP && _errcode != kEctNetMsgXPHandleBufferErr) {
    ...
    xinfo2_if(recvlen <= 0, TSF", recvlen:%_ error:%_ %_", recvlen, socket_errno, socket_strerror(socket_errno))
      >> close_log;
    if (recvlen > 0) {
      bufrecv.Length(bufrecv.Pos() + recvlen, bufrecv.Length() + recvlen);
