...
DynamicTimeout::DynamicTimeout() : dyntime_status_(kEValuating),
...
void DynamicTimeout::CgiTaskStatistic(std::string _cgi_uri,
                                      unsigned int _total_size,
                                      uint64_t _cost_time) {

  int task_status = (_total_size == kDynTimeTaskFailedPkgLen || _cost_time == 0)
                    ? kDynTimeTaskFailedTag : KDynTimeTaskNormalTag;

  if (task_status == KDynTimeTaskNormalTag) {
    unsigned int small_pkg_costtime = kMobile != getNetInfo()
                                      ? kDynTimeSmallPackageWifiCosttime : kDynTimeSmallPackageGPRSCosttime;

    unsigned int middle_pkg_costtime = kMobile != getNetInfo()
                                       ? kDynTimeMiddlePackageWifiCosttime : kDynTimeMiddlePackageGPRSCosttime;

    unsigned int big_pkg_costtime = kMobile != getNetInfo()
                                    ? kDynTimeBigPackageWifiCosttime : kDynTimeBigPackageGPRSCosttime;

    unsigned int bigger_pkg_costtime = kMobile != getNetInfo()
                                       ? kDynTimeBiggerPackageWifiCosttime : kDynTimeBiggerPackageGPRSCosttime;
...
void DynamicTimeout::__StatusSwitch(std::string _cgi_uri, int _task_status) {
  if (dyntime_fncount_latstmodify_time_ == 0 ||
      (gettickcount() - dyntime_fncount_latstmodify_time_) > kDynTimeCountExpireTime) {
  ...
  dyntime_fncount_pos_ = ++dyntime_fncount_pos_ >= dyntime_failed_normal_count_.size()
                         ? 0 : dyntime_fncount_pos_;
  ...
  switch (dyntime_status_) {
    case kEValuating: {
      if (dyntime_continuous_good_count_ >= kDynTimeMaxContinuousExcellentCount &&
          (gettickcount() - dyntime_latest_bigpkg_goodtime_) <= kDynTimeCountExpireTime) {

        xassert2(kDynTimeMaxContinuousExcellentCount >= 10,
                 TSF"max_continuous_good_count:%_",
                 kDynTimeMaxContinuousExcellentCount);

        dyntime_status_ = kExcellent;
      } else if (dyntime_failed_normal_count_.count() <= kDynTimeMinNormalPkgCount) {
        xassert2(kDynTimeMinNormalPkgCount < dyntime_failed_normal_count_.size(),
                 TSF"DYNTIME_MIN_NORMAL_PKG_COUNT:%_, dyntime_failed_normal_count_:%_",
                 kDynTimeMinNormalPkgCount,
                 dyntime_failed_normal_count_.size());
  ...
  xdebug2(
      TSF"task_status:%_, good_count:%_, good_time:%_, dyntime_status:%_, dyntime_failed_normal_count_NORMAL:%_, cgi:%_",
      _task_status,
      dyntime_continuous_good_count_,
      dyntime_latest_bigpkg_goodtime_,
      dyntime_status_,
      dyntime_failed_normal_count_.count(),
      _cgi_uri);
}
