...
//            xassert2(shortlink_.func_network_report);
      ...
      if (_index < shortlink_.Profile().ip_items.size() && shortlink_.func_network_report)
        shortlink_.func_network_report(__LINE__, kEctSocket, _error, _addr.ip(),
                                       shortlink_.Profile().ip_items[_index].str_host, _addr.port());
    }
...
ShortLink::ShortLink(MessageQueue::MessageQueue_t _messagequeueid, NetSource& _netsource, const Task& _task,
                     bool _use_proxy)
    : asyncreg_(MessageQueue::InstallAsyncHandler(_messagequeueid)), net_source_(_netsource), task_(_task),
      thread_(boost::bind(&ShortLink::__Run, this), XLOGGER_TAG "::shortlink"), use_proxy_(_use_proxy),
      tracker_(shortlink_tracker::Create()), is_keep_alive_(CheckKeepAlive(_task)) {
...
SOCKET ShortLink::__RunConnect(ConnectProfile& _conn_profile) {
  ...
  if (use_proxy && mars::comm::kProxyHttp == _conn_profile.proxy_info.type) {
    ...
    for (size_t i = 0; i < _conn_profile.ip_items.size(); ++i) {
      if (!use_proxy || mars::comm::kProxyNone == _conn_profile.proxy_info.type) {
        vecaddr.push_back(
            socket_address(_conn_profile.ip_items[i].str_ip.c_str(), _conn_profile.ip_items[i].port).v4tov6_address(
                isnat64));
      } else {
  ...
  if (use_proxy && (mars::comm::kProxyHttpTunel == _conn_profile.proxy_info.type ||
  ...
  xinfo2(TSF"task socket dns sock %_ proxy:%_, host:%_, ip list:%_, is_keep_alive:%_", message.String(),
         kIPSourceProxy == _conn_profile.ip_type, _conn_profile.host, NetSource::DumpTable(_conn_profile.ip_items),
         is_keep_alive_);
  ...
  SOCKET sock = conn.ConnectImpatient(vecaddr, breaker_, &connect_observer, _conn_profile.proxy_info.type, proxy_addr,
                                      _conn_profile.proxy_info.username, _conn_profile.proxy_info.password);
  delete proxy_addr;
  ...
  for (int i = 0; i < conn.Index(); ++i) {
    if (1 == connect_observer.ConnectingIndex[i] && func_network_report)
      func_network_report(__LINE__, kEctSocket, SOCKET_ERRNO(ETIMEDOUT), _conn_profile.ip_items[i].str_ip,
                          _conn_profile.ip_items[i].str_host, _conn_profile.ip_items[i].port);
  }
  ...
  xinfo2(
      TSF"task socket connect success sock:%_, %_ host:%_, ip:%_, port:%_, local_ip:%_, local_port:%_, iptype:%_, net:%_",
      sock, message.String(), _conn_profile.host, _conn_profile.ip, _conn_profile.port, _conn_profile.local_ip,
      _conn_profile.local_port, IPSourceTypeString[_conn_profile.ip_type], _conn_profile.net_type);
...
void ShortLink::__RunReadWrite(SOCKET _socket, int& _err_type, int& _err_code, ConnectProfile& _conn_profile) {
  ...
  int send_ret = block_socket_send(_socket, (const unsigned char*) out_buff.Ptr(), (unsigned int) out_buff.Length(),
                                   breaker_, _err_code);

  if (send_ret < 0) {
    xerror2(TSF"Send Request Error, ret:%0, errno:%1, nread:%_, nwrite:%_", send_ret, strerror(_err_code),
            socket_nread(_socket), socket_nwrite(_socket)) >> group_send;
  ...
  if (breaker_.IsBreak()) {
    xwarn2(TSF"Send Request break, sent:%_ nread:%_, nwrite:%_", send_ret, socket_nread(_socket),
           socket_nwrite(_socket)) >> group_send;
    return;
  }
  ...
  while (true) {
    ...
    if (recv_ret < 0) {
      xerror2(TSF"read block socket return false, error:%0, nread:%_, nwrite:%_", strerror(_err_code),
              socket_nread(_socket), socket_nwrite(_socket)) >> group_close;
    ...
    if (recv_ret == 0 && SOCKET_ERRNO(ETIMEDOUT) == _err_code) {
      xerror2(TSF"read timeout error:(%_,%_), nread:%_, nwrite:%_ ", _err_code, strerror(_err_code),
              socket_nread(_socket), socket_nwrite(_socket)) >> group_close;
      continue;
    }
    if (recv_ret == 0) {
      xerror2(TSF"remote disconnect, nread:%_, nwrite:%_", _err_code, strerror(_err_code), socket_nread(_socket),
              socket_nwrite(_socket)) >> group_close;
    ...
    if (parse_status == http::Parser::kFirstLineError) {
      xerror2(TSF"http head not receive yet,but socket closed, length:%0, nread:%_, nwrite:%_ ", recv_buf.Length(),
              socket_nread(_socket), socket_nwrite(_socket)) >> group_close;
      ...
      xerror2(TSF"parse http head failed, but socket closed, length:%0, nread:%_, nwrite:%_ ", recv_buf.Length(),
              socket_nread(_socket), socket_nwrite(_socket)) >> group_close;
...
void ShortLink::__OnResponse(ErrCmdType _errType, int _status, AutoBuffer& _body, AutoBuffer& _extension,
                             ConnectProfile& _conn_profile, bool _report) {
