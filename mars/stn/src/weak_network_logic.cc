...
void WeakNetworkLogic::OnConnectEvent(bool _is_suc, int _rtt, int _index) {
  ...
  if (!_is_suc) {
    if (is_curr_weak_ && last_mark_tick_.gettickspan() >= WEAK_LEAST_SPAN) {
      ...
      if (connect_after_weak_ <= 1 && first_mark_tick_.gettickspan() < SURE_WEAK_SPAN)
...
if (!ActiveLogic::Instance()->IsForeground())
  ...
  bool is_weak = false;
  if (_task_profile.transfer_profile.connect_profile.ip_index > 0
      && _task_profile.err_type != kEctOK
      && _task_profile.err_type != kEctEnDecode) {
  ...
  if(is_weak) {
    ...
    if (_task_profile.err_type == kEctOK
        && (_task_profile.end_task_time - _task_profile.start_task_time) < GOOD_TASK_SPAN) {
