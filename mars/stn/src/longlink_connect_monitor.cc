...
static int const reconnect_interval[7] = {
    // if modify this varible ,do not forget to modify varible: "kInternalMaxIndex"
...
static int __CurActiveState(const ActiveLogic& _activeLogic) {
  ...
  if (10 * 60 * 1000 <= ::gettickcount() - _activeLogic.LastForegroundChangeTime())
    return kForgroundActive;

  if (60 * 1000 <= ::gettickcount() - _activeLogic.LastForegroundChangeTime())
    return kForgroundTenMinute;
...
static unsigned long __Interval(int _type, const ActiveLogic& _activelogic) {
  ...
  if (__CurActiveState(_activelogic) == kInactive ||
      __CurActiveState(_activelogic) == kForgroundActive) {  // now - LastForegroundChangeTime>10min
...
LongLinkConnectMonitor::LongLinkConnectMonitor(ActiveLogic& _activelogic,
                                               LongLink& _longlink,
                                               MessageQueue::MessageQueue_t _id,
                                               bool _is_keep_alive)
    : asyncreg_(MessageQueue::InstallAsyncHandler(_id)),
      activelogic_(_activelogic),
      longlink_(_longlink),
      rebuild_alarm_(boost::bind(&LongLinkConnectMonitor::__OnAlarm, this, true), _id),
      wake_alarm_(boost::bind(&LongLinkConnectMonitor::__OnAlarm, this, false), _id), status_(LongLink::kDisConnected),
      last_connect_time_(0),
      last_connect_net_type_(kNoNet),
      thread_(boost::bind(&LongLinkConnectMonitor::__Run, this), XLOGGER_TAG"::con_mon"), conti_suc_count_(0),
      isstart_(false),
...
uint64_t LongLinkConnectMonitor::__IntervalConnect(int _type) {
  if (LongLink::kConnecting == longlink_.ConnectStatus()
      || LongLink::kConnected == longlink_.ConnectStatus()) {
  ...
  xinfo_trace(TSF"longlink_progress next connect interval: %_, process active state: %_, process foreground: %_",
              interval, activelogic_.IsActive(),
              activelogic_.IsForeground());

  xinfo2(TSF"next connect interval: %_, posttime: %_, buffer: %_, _type: %_",
         interval, posttime, buffer, _type);
  ...
  if (activelogic_.IsActive() || _type == kNetworkChangeConnect) {
    if ((posttime + buffer) >= interval) {
      ...
      xinfo2(TSF"process active made interval connect interval:%0, posttime:%_, newone:%_, connectstatus:%_, ret:%_",
             interval, posttime, newone, longlink_.ConnectStatus(), ret);
      return 0;
    } else {
  ...
  if (interval_final == 0) {
    bool newone = false;
    bool ret = longlink_.MakeSureConnected(&newone);
    xinfo2(TSF"rebuild now, connect interval:%0, posttime:%_, newone:%_, connectstatus:%_, ret:%_", interval, posttime,
           newone, longlink_.ConnectStatus(), ret);
    return 0;
  } else {
...
uint64_t LongLinkConnectMonitor::__AutoIntervalConnect() {
  ...
  xinfo_trace(
      TSF"longlink_progress set rebuild alarm, reason: rebuild longlink alarm, current time: %_, next rebuild time interval: %_",
      gettickcount(), remain);
#ifdef __ANDROID__
...
void LongLinkConnectMonitor::__OnSignalForeground(bool _isForeground) {
  ASYNC_BLOCK_START
        ...
        if (_isForeground) {
          xinfo2(TSF"longlink:%_ time:%_ %_ %_", longlink_.ConnectStatus(), tickcount_t().gettickcount().get(),
                 longlink_.GetLastRecvTime().get(),
                 int64_t(tickcount_t().gettickcount() - longlink_.GetLastRecvTime()));
...
void
LongLinkConnectMonitor::__OnLongLinkStatuChanged(LongLink::TLongLinkStatus _status, const std::string& _channel_id) {
...
void LongLinkConnectMonitor::__OnAlarm(bool _rebuild_longlink) {
  ...
  __AutoIntervalConnect();
  if (rebuild_longlink_) {
    xinfo_trace(TSF"longlink_progress onalarm log, reason: %_, alarm after %_ millsecond, spend time %_", alarm_reason,
                rebuild_alarm_.After(), rebuild_alarm_.ElapseTime());
  }
}
...
void LongLinkConnectMonitor::OnHeartbeatAlarmReceived(bool _is_noop_timeout) {
  xinfo_trace(TSF"longlink_progress on_heartbeat_alarm, time: %_, is_noop_timeout:%_", gettickcount(),
              _is_noop_timeout);
}
