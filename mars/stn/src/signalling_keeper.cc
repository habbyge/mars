...
SignallingKeeper::SignallingKeeper(const LongLink& _longlink, MessageQueue::MessageQueue_t _messagequeue_id,
                                   bool _use_UDP)
    : msgreg_(MessageQueue::InstallAsyncHandler(_messagequeue_id)), last_touch_time_(0), keeping_(false),
      longlink_(_longlink), port_(0), udp_client_(ip_, port_, this), use_UDP_(_use_UDP) {
  xinfo2(TSF"SignallingKeeper messagequeue_id=%_, handler:(%_,%_)", MessageQueue::Handler2Queue(msgreg_.Get()),
         msgreg_.Get().queue, msgreg_.Get().seq);
}
...
void SignallingKeeper::Keep() {
  xinfo2(TSF"start signalling, period:%0, keepTime:%1, use udp:%2, keeping_:%3", g_period, g_keepTime, use_UDP_,
         keeping_);
...
void SignallingKeeper::__SendSignallingBuffer() {
  if (use_UDP_) {
    ...
    if (ip_ != "" && port_ != 0) {
      ...
      AutoBuffer buffer;
      gDefaultLongLinkEncoder.longlink_pack(gDefaultLongLinkEncoder.signal_keep_cmdid(), 0, KNullAtuoBuffer,
                                            KNullAtuoBuffer, buffer, NULL);
