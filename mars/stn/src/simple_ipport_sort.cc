...
SimpleIPPortSort::SimpleIPPortSort()
    : hostpath_(mars::app::GetAppFilePath() + "/" + kFolderName), IPv6_ban_flag_(0), IPv4_ban_flag_(0), ban_v6_(false) {
...
void SimpleIPPortSort::__UpdateBanFlagAndTime(const std::string& _ip, bool _success) {
  ...
  xdebug2(TSF"ip is %_, success is %_ , current v6 flag %_ , current v4 flag %_", _ip, _success,
          DecToBin(IPv6_ban_flag_), DecToBin(IPv4_ban_flag_));
}
...
void SimpleIPPortSort::__SortbyBanned(std::vector<IPPortItem>& _items, bool _use_IPv6) const {
  ...
  auto find_lambda = [&](const IPPortItem& _v) {
    for (std::vector<BanItem>::const_iterator it_banned = _ban_fail_list_.begin();
         it_banned != _ban_fail_list_.end(); ++it_banned) {
  ...
  xassert2(_items.size() == items_history.size() + items_new.size(), TSF"_item:%_, history:%_, new:%_", _items.size(),
           items_history.size(), items_new.size());
  ...
  std::sort(items_history.begin(), items_history.end(),
            [&](const IPPortItem& _l, const IPPortItem& _r) {
              ...
              std::vector<BanItem>::const_iterator l = std::find_if(_ban_fail_list_.begin(), _ban_fail_list_.end(),
                                                                    boost::bind<bool>(find_lr_lambda, _1, _l));
              std::vector<BanItem>::const_iterator r = std::find_if(_ban_fail_list_.begin(), _ban_fail_list_.end(),
...
void SimpleIPPortSort::__PickIpItemRandom(std::vector<IPPortItem>& _items, std::deque<IPPortItem>& _items_history,
