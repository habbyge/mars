...
public:
    ...
    while (iter != socket_pool_.end()) {
      if (iter->IsSame(_item)) {
        ...
        xinfo2(TSF"get from cache: ip:%_, port:%_, host:%_, fd:%_, size:%_",
               _item.str_ip, _item.port, _item.str_host, fd, socket_pool_.size());

        return fd;
      }
      iter++;
    }
    xinfo2(TSF"can not find socket ip:%_, port:%_, host:%_, size:%_",
           _item.str_ip, _item.port, _item.str_host, socket_pool_.size());
  ...
  bool AddCache(CacheSocketItem& item) {
    ScopedLock lock(mutex_);
    xinfo2(TSF"add item to socket pool, ip:%_, port:%_, host:%_, fd:%_, size:%_", item.address_info.str_ip,
           item.address_info.port, item.address_info.str_host, item.socket_fd, socket_pool_.size());
  ...
  void CleanTimeout() {
    ...
    while (iter != socket_pool_.end()) {
      if (iter->HasTimeout()) {
        socket_close(iter->socket_fd);
        xinfo2(TSF"remove timeout socket: ip:%_, port:%_, host:%_, fd:%_", iter->address_info.str_ip,
               iter->address_info.port, iter->address_info.str_host, iter->socket_fd);
        iter = socket_pool_.erase(iter);
        continue;
      }
      iter++;
