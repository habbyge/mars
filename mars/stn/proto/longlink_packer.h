...
public:
   ...
   */
  std::function<void(uint32_t _cmdid, uint32_t _seq, const AutoBuffer& _body, const AutoBuffer& _extension,
                     AutoBuffer& _packed, longlink_tracker* _tracker)> longlink_pack;
   ...
   */
  std::function<int(const AutoBuffer& _packed, uint32_t& _cmdid, uint32_t& _seq, size_t& _package_len,
                    AutoBuffer& _body, AutoBuffer& _extension, longlink_tracker* _tracker)> longlink_unpack;

  //heartbeat signal to keep longlink network alive
  std::function<uint32_t()> longlink_noop_cmdid;
  std::function<bool(uint32_t _taskid, uint32_t _cmdid, uint32_t _recv_seq, const AutoBuffer& _body,
                     const AutoBuffer& _extend)> longlink_noop_isresp;
   ...
   */
  std::function<bool(uint32_t _cmdid, uint32_t _taskid, const AutoBuffer& _body,
                     const AutoBuffer& _extend)> longlink_ispush;
  std::function<bool(uint32_t _sent_seq, uint32_t _cmdid, uint32_t _recv_seq, const AutoBuffer& _body,
                     const AutoBuffer& _extend)> longlink_identify_isresp;
};
 ...
 * _packed: business send buffer + request header
 */
extern void (* longlink_pack)(uint32_t _cmdid, uint32_t _seq, const AutoBuffer& _body, const AutoBuffer& _extension,
                              AutoBuffer& _packed, longlink_tracker* _tracker);
 ...
 * return: 0 if unpackage succ
 */
extern int (* longlink_unpack)(const AutoBuffer& _packed, uint32_t& _cmdid, uint32_t& _seq, size_t& _package_len,
                               AutoBuffer& _body, AutoBuffer& _extension, longlink_tracker* _tracker);

//heartbeat signal to keep longlink network alive
extern uint32_t (* longlink_noop_cmdid)();

extern bool (* longlink_noop_isresp)(uint32_t _taskid, uint32_t _cmdid, uint32_t _recv_seq, const AutoBuffer& _body,
...
extern bool
(* longlink_identify_isresp)(uint32_t _sent_seq, uint32_t _cmdid, uint32_t _recv_seq, const AutoBuffer& _body,
                             const AutoBuffer& _extend);
