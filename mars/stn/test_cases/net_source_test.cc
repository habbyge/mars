...
m_testCaseInfo->
m_TestCaseName = "history record's num < 5, ReportLongIP false, successRate reduce";
...
netsource.
GetMixLongLinkItems(iPPortItemVector);
ASSERT_TRUE(!iPPortItemVector.
...
{
m_testCaseInfo->
m_TestCaseName = "history record's num < 5, ReportLongIP false three times, ip is baned";
UtilFunc::del_files(CONFIG_PATH);
...
{
m_testCaseInfo->
m_TestCaseName = "ReportLongIP true five times, ip will have history record";
UtilFunc::del_files(CONFIG_PATH);
...
PUBC_EXPECT_TRUE(iPPortItemVector
.

front()

.strIP == ipPortItem.
strIP&& iPPortItemVector
.

front()

.nPort == ipPortItem.nPort, &baseScene);
}
...
{
m_testCaseInfo->
m_TestCaseName = "history record's num >= 5, banlist is empty, ReportLongIP false, successRate reduce";
UtilFunc::del_files(CONFIG_PATH);
...
netsource.m_ipportstrategy.m_BanFailList.
...
PUBC_EXPECT_GT(1.0f, iPPortItemVector.

front()

.successRate, &baseScene);
}
...
{
m_testCaseInfo->
m_TestCaseName = "history record's num >= 5, banlist is empty, ReportLongIP true, successRate increase";
...
PUBC_EXPECT_LT(beforeRate, iPPortItemVector
.
...
{
m_testCaseInfo->
m_TestCaseName = "history record's num >= 5, banlist is empty, ReportLongIP false three times, ip is baned";
UtilFunc::del_files(CONFIG_PATH);
...
PUBC_EXPECT_EQ(iPPortItemVector
.
...
{
m_testCaseInfo->
m_TestCaseName = "history record's num >= 5, banlist is not empty, ReportLongIP false, successRate reduce";
UtilFunc::del_files(CONFIG_PATH);
...
{
m_testCaseInfo->
m_TestCaseName = "history record's num >= 5, banlist is not empty, ReportLongIP true, successRate increase";
UtilFunc::del_files(CONFIG_PATH);
...
PUBC_EXPECT_GT(iter
->successRate, beforeRate, &baseScene);
...
{
m_testCaseInfo->
m_TestCaseName = "history record's num >= 5, banlist is not empty, ReportLongIP false three times, ip is baned";
UtilFunc::del_files(CONFIG_PATH);
...
netsource.
SaveAuthLongPortList(portList);

netsource.SaveAuthShortIPList("Auth_test_host", iPList);
netsource.
SaveAuthShortPortList(portList);
...
netsource.
GetInitLongIPs(ipVector);
netsource.
GetInitShortIPs(ipVector);
