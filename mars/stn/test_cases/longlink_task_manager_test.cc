...
static unsigned long __FirstPkgTimeout(unsigned long _init_first_pkg_timeout, unsigned int _sendlen, int _send_count) {
  ...
  unsigned int rate = (kMobile != getNetInfo()) ? kWifiMinRate : kGPRSMinRate;
  unsigned int base_rw_timeout = (kMobile != getNetInfo()) ? kBaseFirstPackageWifiTimeout
...
static void
OnLongLinkNetworkError(int _line, ErrCmdType _eErrType, int _nErrCode, const std::string& _ip, unsigned int _port) {
...
static int CallBack(int _from, ErrCmdType _eErrType, int _nErrCode, const AutoBuffer& _cookies, int _nHashCode,
                    const CNetCmd& _cmd, unsigned int _taskcosttime) {
  printf("form:%d, errType:%d errCode:%d hashCode:%d, costTime:%u\n", _from, _eErrType, _nErrCode, _nHashCode,
         _taskcosttime);
...
public:
  bool Mine_SingleRespHandle(std::list<STCmdLongLinkTask>::iterator _it, ErrCmdType _eErrType,
                             int _nErrCode, const AutoBuffer& _cookies, unsigned int _respLength,
                             const LongLinkConnectionInfo& info);
...
bool (CDetour::* CDetour::Real_SingleRespHandle)(std::list<STCmdLongLinkTask>::iterator _it, ErrCmdType _eErrType,
                                                 int _nErrCode, const AutoBuffer& _cookies, unsigned int _respLength,
                                                 const LongLinkConnectionInfo& info) = (bool (CDetour::*)(
    std::list<STCmdLongLinkTask>::iterator _it, ErrCmdType _eErrType,
    int _nErrCode, const AutoBuffer& _cookies, unsigned int _respLength,
    const LongLinkConnectionInfo& info)) &CMMLongLinkTaskManager::__SingleRespHandle;

static bool (CDetour::* Mine_Target1)(std::list<STCmdLongLinkTask>::iterator _it, ErrCmdType _eErrType,
                                      int _nErrCode, const AutoBuffer& _cookies, unsigned int _respLength,
                                      const LongLinkConnectionInfo& info) = &CDetour::Mine_SingleRespHandle;
...
m_testCaseInfo->
m_TestCaseName = "task default time out:60000ms";
...
m_testCaseInfo->
m_WorkerExpectTimeCost = 60 * 1000;
m_testCaseInfo->
m_WorkerRealTimeCost = after - before;

BaseScene baseScene;
baseScene.
eventName = "use default expectfinishtime";
PUBC_EXPECT_TRUE(sg_taskCostTime<65000 && sg_taskCostTime>
6000, &baseScene);
...
{
m_testCaseInfo->
m_TestCaseName = "task user define task timeout:30000ms";
...
networkType = 3;
cmd.
cmduser_expectfinishtime = 30 * 1000;
...
m_testCaseInfo->
m_WorkerExpectTimeCost = 30 * 1000;
m_testCaseInfo->
m_WorkerRealTimeCost = after - before;

BaseScene baseScene;
baseScene.
eventName = "set expectfinishtime 30*1000ms";
PUBC_EXPECT_TRUE(sg_taskCostTime<35000 && sg_taskCostTime>
3000, &baseScene);
...
//appender_open(EAppednerSync, "C:\\log", "GTEST");
m_testCaseInfo->
m_TestCaseName = "default first pkg timeout";
...
scene.
eventName = "respone delay 20s";
PUBC_EXPECT_TRUE(costTime
> __FirstPkgTimeout(0, 16, 0) &&
costTime<__FirstPkgTimeout(0, 16, 0) + 2000,& scene
);
}
...
//appender_open(EAppednerSync, "C:\\log", "GTEST");
m_testCaseInfo->
m_TestCaseName = "user define first pkg timeout";
...
cmduser_retrycount = 0;
cmd.
cmduser_firstpkgtimeout = 10 * 1000;
...
PUBC_EXPECT_TRUE(costTime
>
computerTimeout&& costTime<computerTimeout + 2000,
&scene);
...
m_testCaseInfo->
m_TestCaseName = "read-write timeout, use default first pkg timeout";
...
BaseScene scene;
scene.
eventName = "respone 16 byte every 4 seconds";
...
m_testCaseInfo->
m_TestCaseName = "read-write timeout, user define first pkg timeout";
