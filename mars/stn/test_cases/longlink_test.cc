...
public:
  ...
  virtual void OnResponse(ErrCmdType _type, int _errcode, int _cmdid, unsigned int _seq, AutoBuffer& _body,
                          const LongLinkConnectionInfo& _info) {}
...
void
__OnLongLinkNetworkError(int _line, ErrCmdType _eErrType, int _nErrCode, const std::string& _ip, unsigned int _port) {
  //printf("%s:%u, err(%d, %d), line:%d\n", _ip.c_str(), _port, _eErrType, _nErrCode, _line);
}

bool (CDetour::* CDetour::Real_GetLongLinkItems)(std::vector<IPPortItem>& _IPPortItems) = (bool (CDetour::*)(
    std::vector<IPPortItem>& _IPPortItems)) &CMMNetSource::GetLongLinkItems;

bool (CDetour::* CDetour::Real_GetMixLongLinkItems)(std::vector<IPPortItem>& _IPPortItems) = (bool (CDetour::*)(
    std::vector<IPPortItem>& _IPPortItems)) &CMMNetSource::GetMixLongLinkItems;

void (CDetour::* CDetour::Real_ReportLongIP)(bool _isSuccess, const std::string& _ip,
                                             unsigned int _port) = (void (CDetour::*)(bool _isSuccess,
                                                                                      const std::string& _ip,
                                                                                      unsigned int _port)) &CMMNetSource::ReportLongIP;
...
void test(TestCaseInfo& _info) {
  ...
  for (size_t i = 0; i < sg_longLinkScenes.at(sg_index).expectValues.size(); ++i) {
    ...
    EXPECT_EQ(expectValue.second, longlink.m_connectstatus) << "case name:" << _info.m_TestCaseName
                                                            << ", expectValue index:" << i << ", sleep:"
                                                            << expectValue.first << ", longlink expect:"
                                                            << expectValue.second << ", longlink status:"
                                                            << longlink.m_connectstatus;
  ...
  uint64_t diff = (sg_nowChangeTime - sg_lastChangeTime) > sg_longLinkScenes.at(sg_index).expectUseTime ?
                  (sg_nowChangeTime - sg_lastChangeTime) - sg_longLinkScenes.at(sg_index).expectUseTime :
                  sg_longLinkScenes.at(sg_index).expectUseTime - (sg_nowChangeTime - sg_lastChangeTime);
...
static void
(CDetour::* Mine_Target3)(bool _isSuccess, const std::string& _ip, unsigned int _port) = &CDetour::Mine_ReportLongIP;
...
DetourAttach(& (PVOID
&)CDetour::Real_GetLongLinkItems, *(PBYTE*)&Mine_Target1);
DetourAttach(& (PVOID
&)CDetour::Real_GetMixLongLinkItems, *(PBYTE*)&Mine_Target2);
DetourAttach(& (PVOID
&)CDetour::Real_ReportLongIP, *(PBYTE*)&Mine_Target3);
...
info.
m_TestCaseGroupName = "MMLongLink2_test";
test(info);
}
