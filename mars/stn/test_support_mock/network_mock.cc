...
ErrorLogReport::ErrorLogReport()
    : m_nLastBackgroundReportTime(0), m_pFile(NULL), m_ini("c:\\errlog.txt") {
...
CMMLongLinkSpeedTestItem::CMMLongLinkSpeedTestItem(const std::string& _ip, unsigned int _port)
    : m_ip(_ip), m_port(_port), m_socket(-1), m_state(ELongLinkSpeedTestConnecting), m_beforeConnectTime(0),
