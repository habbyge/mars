...
int onTaskEnd(const int errType, const int errCode, const char* const errMsg, const int hashCode,
              const AutoBuffer& cookies) { return 0; }
...
void reportFlowData(const int wifiRecvDataSize, const int wifiSendDataSize, int mobileRecvDataSize,
                    int mobileSendDataSize) {}
...
int
getLongLinkIdentifyCheckBuffer(AutoBuffer& identifyReqBuf, int& reqCmdId, int& respCmdId, AutoBuffer& hashCodeBuffer) {
  return 2;
}
