...
DEFINE_FIND_STATIC_METHOD(KXlog_appenderOpenWithMultipathWithLevel, KXlog, "appenderOpen",
                          "(Lcom/tencent/mars/xlog/Xlog$XLogConfig;)V")
...
XLogConfig config = {(TAppenderMode) mode, logdir_str, nameprefix_str, pubkey_str, (TCompressMode) compressmode,
                     compresslevel, cachedir_str, cachedays};
appender_open(config);
xlogger_SetLevel((TLogLevel)
level);
}

JNIEXPORT void JNICALL
Java_com_tencent_mars_xlog_Xlog_appenderClose(JNIEnv
*env, jobject) {
appender_close();
...
JNIEXPORT void JNICALL
Java_com_tencent_mars_xlog_Xlog_appenderFlush(JNIEnv
*env, jobject,
jboolean _is_sync
) {
if (_is_sync) {
...
DEFINE_FIND_STATIC_METHOD(KXlog_logWrite, KXlog, "logWrite",
                          "(Lcom/tencent/mars/xlog/Xlog$XLoggerInfo;Ljava/lang/String;)V")
JNIEXPORT void JNICALL
Java_com_tencent_mars_xlog_Xlog_logWrite
(JNIEnv
*env, jclass,
jobject _log_info, jstring
_log) {
...
if (!
xlogger_IsEnabledFor((TLogLevel)
level)) {
return;
}

jstring tag = (jstring)
JNU_GetField(env, _log_info,
...
gettimeofday(& xlog_info
.timeval, NULL);
xlog_info.
level = (TLogLevel) level;
xlog_info.
line = line;
xlog_info.
pid = (int) pid;
xlog_info.
tid = LONGTHREADID2INT(tid);
xlog_info.
maintid = LONGTHREADID2INT(maintid);;
...
xlog_info.
tag = tag_jstr.GetChar();
xlog_info.
filename = filename_jstr.GetChar();
xlog_info.
func_name = funcname_jstr.GetChar();

xlogger_Write(& xlog_info, log_jst
.
...
JNIEXPORT void JNICALL
Java_com_tencent_mars_xlog_Xlog_logWrite2
(JNIEnv
*env, jclass,
int _level, jstring
_tag,
...
.timeval, NULL);
xlog_info.
level = (TLogLevel) _level;
xlog_info.
line = (int) _line;
xlog_info.
pid = (int) _pid;
xlog_info.
tid = LONGTHREADID2INT(_tid);
xlog_info.
maintid = LONGTHREADID2INT(_maintid);
...
xlog_info.
tag = NULL == tag_cstr ? "" : tag_cstr;
xlog_info.
filename = NULL == filename_cstr ? "" : filename_cstr;
xlog_info.
func_name = NULL == funcname_cstr ? "" : funcname_cstr;

xlogger_Write(& xlog_info,
NULL == log_cstr ? "NULL == log" : log_cstr);

if (NULL != _tag) {
env->
ReleaseStringUTFChars(_tag, tag_cstr
);
}

if (NULL != _filename) {
env->
ReleaseStringUTFChars(_filename, filename_cstr
);
}

if (NULL != _funcname) {
env->
ReleaseStringUTFChars(_funcname, funcname_cstr
);
}

if (NULL != _log) {
env->
ReleaseStringUTFChars(_log, log_cstr
);
}
}
...
JNICALL Java_com_tencent_mars_xlog_Xlog_getLogLevel
    (JNIEnv * , jobject) {
...
JNIEXPORT void JNICALL
Java_com_tencent_mars_xlog_Xlog_setLogLevel
(JNIEnv
*, jclass,
jint _log_level
) {
...
JNIEXPORT void JNICALL
Java_com_tencent_mars_xlog_Xlog_setAppenderMode
(JNIEnv
*, jclass,
jint _mode
) {
appender_setmode((TAppenderMode)
_mode);
}

DEFINE_FIND_STATIC_METHOD(KXlog_setConsoleLogOpen, KXlog, "setConsoleLogOpen", "(Z)V")
JNIEXPORT void JNICALL
Java_com_tencent_mars_xlog_Xlog_setConsoleLogOpen
(JNIEnv
*env, jclass,
jboolean _is_open
) {
...
JNIEXPORT void JNICALL
Java_com_tencent_mars_xlog_Xlog_setMaxFileSize
(JNIEnv
*env, jclass,
jlong _maxSize
) {
...
JNIEXPORT void JNICALL
Java_com_tencent_mars_xlog_Xlog_setMaxAliveTime
(JNIEnv
*env, jclass,
jlong _maxTime
) {
