...
EXPECT_EQ(strlen(dump1), calc_dump_required_length(1)
+ head_bytes + 1);    // 1 for '\n'
...
EXPECT_EQ(strlen(dump1), calc_dump_required_length(121)
+ head_bytes + round * 2 - 1);

dump1 = xlogger_memory_dump(srcbuffer, 128);
head_bytes = snprintf(dummybuf, sizeof(dummybuf), "\n%zu bytes:\n", 128);
round = 128 / 32;
EXPECT_EQ(strlen(dump1), calc_dump_required_length(128)
+ head_bytes + round * 2 - 1);

EXPECT_GT(calc_dump_required_length(4096),
4096);

dump1 = xlogger_memory_dump(srcbuffer, 4096);
EXPECT_LT(strlen(dump1),
4096);
