...
bool LogBaseBuffer::GetPeriodLogs(const char* _log_path, int _begin_hour, int _end_hour, unsigned long& _begin_pos,
  ...
  while (!feof(file) && !ferror(file)) {
    ...
    if (LogCrypt::GetHeaderLen() != fread(header_buff, 1, LogCrypt::GetHeaderLen(), file)) {
      snprintf(msg, sizeof(msg), "fread(buff.Ptr(), 1, __GetHeaderLen(), file) error:%s, before_len:%ld.",
    ...
    if (!LogMagicNum::MagicStartIsValid(start)) {
      ...
      if ((long) (ftell(file) + len + sizeof(magic_end)) > file_size) {
        ...
        if (0 != fseek(file, len, SEEK_CUR)) {
          snprintf(msg, sizeof(msg), "fseek(file, len, SEEK_CUR):%s, before_len:%ld, len:%u.", strerror(ferror(file)),
                   before_len, len);
          break;
        }
        char end;
        if (1 != fread(&end, 1, 1, file)) {
          snprintf(msg, sizeof(msg), "fread(&end, 1, 1, file) err:%s, before_len:%ld, len:%u.", strerror(ferror(file)),
                   before_len, len);
          break;
        }
        if (end != magic_end) {
    ...
    if (fix) {
      if (0 != fseek(file, before_len + 1, SEEK_SET)) {
        snprintf(msg, sizeof(msg), "fseek(file, before_len+1, SEEK_SET) err:%s, before_len:%ld.",
                 strerror(ferror(file)), before_len);
        break;
      }
      continue;
    ...
    if (!LogCrypt::GetLogHour(header_buff, LogCrypt::GetHeaderLen(), begin_hour, end_hour)) {
      snprintf(msg, sizeof(msg), "__GetLogHour(buff.Ptr(), buff.Length(), beginHour, endHour) err, before_len:%ld.",
               before_len);
      break;
    }
...
bool LogBaseBuffer::Write(const void* _data, size_t _length) {
  ...
  log_crypt_->CryptAsyncLog((char*) buff_.Ptr() + before_len, write_len + remain_nocrypt_len_, out_buffer,
                            remain_nocrypt_len_);
