...
void log_formater(const XLoggerInfo* _info, const char* _logbody, PtrBuffer& _log) {
  ...
  if (NULL != _info) {
...
#else
      snprintf(temp_time, sizeof(temp_time), "%d-%02d-%02d +%.3s %02d:%02d:%02d.%.3d", 1900 + tm.tm_year, 1 + tm.tm_mon,
               tm.tm_mday,
               gmt.c_str(), tm.tm_hour, tm.tm_min, tm.tm_sec, _info->timeval.tv_usec / 1000);
#endif
    }
    ...
    int ret = snprintf((char*) _log.PosPtr(), 1024,
                       "[%s][%s][%" PRIdMAX ", %" PRIdMAX "%s][%s][%s, %s, %d][",  // **CPPLINT SKIP**
