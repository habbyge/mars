...
static void
__get_filenames_by_prefix(const std::string& _logdir, const std::string& _fileprefix, const std::string& _fileext,
...
static void __get_filepaths_from_timeval(const timeval& _tv, const std::string& _logdir, const char* _prefix,
                                         const std::string& _fileext, std::vector<std::string>& _filepath_vec) {
...
static void
__make_logfilename(const timeval& _tv, const std::string& _logdir, const char* _prefix, const std::string& _fileext,
...
static void
__move_old_files(const std::string& _src_path, const std::string& _dest_path, const std::string& _nameprefix) {
...
void xlogger_appender(const XLoggerInfo* _info, const char* _log) {
  ...
  if (2 <= (int) recursion.Get() && NULL == s_recursion_str.get()) {
    ...
    snprintf(recursive_log, sizeof(recursive_log), "ERROR!!! xlogger_appender Recursive calls!!!, count:%d",
             (int) recursion.Get());
...
void appender_open(const XLogConfig& _config) {
  ...
  if (!_config.cachedir_.empty()) {
    ...
    Thread(boost::bind(&__move_old_files, _config.cachedir_, _config.logdir_,
                       std::string(_config.nameprefix_))).start_after(3 * 60 * 1000);
  }
  ...
  if (!sg_log_close) {
    __writetips2file("appender has already been opened. _dir:%s _nameprefix:%s", _config.logdir_.c_str(),
  ...
  snprintf(mmap_file_path, sizeof(mmap_file_path), "%s/%s.mmap3",
           sg_cache_logdir.empty() ? _config.logdir_.c_str() : sg_cache_logdir.c_str(), _config.nameprefix_.c_str());

  bool use_mmap = false;
  if (OpenMmapFile(mmap_file_path, kBufferBlockLength, sg_mmmap_file)) {
    if (_config.compress_mode_ == kZstd) {
      sg_log_buff = new LogZstdBuffer(sg_mmmap_file.data(), kBufferBlockLength, true, _config.pub_key_.c_str(),
    ...
    if (_config.compress_mode_ == kZstd) {
      sg_log_buff = new LogZstdBuffer(buffer, kBufferBlockLength, true, _config.pub_key_.c_str(),
                                      _config.compress_level_);
    } else {
