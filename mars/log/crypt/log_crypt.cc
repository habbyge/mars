...
void LogCrypt::SetHeaderInfo(char* _data, bool _is_async, char _magic_start) {
  ...
  memcpy(_data + sizeof(_magic_start) + sizeof(seq_) + sizeof(hour) * 2 + sizeof(len), client_pubkey_,
...
void LogCrypt::CryptAsyncLog(const char* const _log_data, size_t _input_len, AutoBuffer& _out_buff,
  ...
  memcpy((char*) _out_buff.Ptr() + _input_len - _remain_nocrypt_len, _log_data + _input_len - _remain_nocrypt_len,
         _remain_nocrypt_len);
#endif
}
