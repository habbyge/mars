 ...
 */
  static SOCKET
  makeNonBlockSocket(SocketSelect& sel, const std::string& strIp, int port, unsigned int timeoutInMs, int& errCode) {
    xverbose_function();
    ...
    if (fsocket == INVALID_SOCKET) {
      ...
      xerror2(TSF
      "socket create error, socket_errno:%0", socket_strerror(errCode));
      return INVALID_SOCKET;
    }

    if (0 != socket_ipv6only(fsocket, 0)) {
      xwarn2(TSF
      "set ipv6only failed. error %_", strerror(socket_errno));
    }
    ...
    if (ret != 0) {
      errCode = socket_errno;
      xerror2(TSF
      "nobio:%0", socket_strerror(errCode));
    ...
    if (connectRet < 0 && !IS_NOBLOCK_CONNECT_ERRNO(socket_errno)) {
      errCode = socket_errno;
      xerror2(TSF
      "connect error, socket_errno:%0", socket_strerror(errCode));
      ::socket_close(fsocket);
      return INVALID_SOCKET;
    }
    ...
    if (connectRet != 0) {
      ...
      while (true) {
        ...
        if (selectRet == 0) {
          errCode = socket_errno;
          xerror2(TSF
          "connect timeout, use time:%0 ms to connect", (gettickcount() - lastTime));
          ::socket_close(fsocket);
          return -2;
        } else if (selectRet < 0) {
          errCode = socket_errno;
          xerror2(TSF
          "select errror, ret:%0, socket_errno:%1, use time:%2 ms",
        ...
        if (sel.Exception_FD_ISSET(fsocket)) {
          ...
          if (ret == 0) {
            errCode = error;
            xerror2(TSF
            "select socket exception error:%0", strerror(errCode));
          } else
            errCode = errno;
          xerror2(TSF
        ...
        if (!sel.Write_FD_ISSET(fsocket)) {
          errCode = errno;
          xerror2(TSF
          "select return but not set, return:%0, errno:%1", selectRet, errno);
          ::socket_close(fsocket);
          xassert2(false);
          return INVALID_SOCKET;
        }
        ...
        xdebug2(TSF
        "use time:%0 ms to connect", (gettickcount() - lastTime));
        ...
        if (ret != 0) {
          ...
          if (ret == 0) {
            errCode = error;
            xerror2(TSF
            "connect error:%0", socket_strerror(errCode));
          } else
            xerror2(TSF
          "getsockopt error");
          ::socket_close(fsocket);
   ...
   *              0  --- suc
     *          others --- error
   */
  static TcpErrCode writenWithNonBlock(int fdSocket, SocketSelect& sel,
    ...
    xverbose_function();
    xinfo2(TSF
    "writenWithNonBlock with Socket:%0, timeoutMs:%1, unSize:%2", fdSocket, timeoutMs, unSize);
    xassert2(unSize > 0);
    if (unSize == 0) {
      xwarn2(TSF
      "writen size == 0");
    ...
    while (true) {
      ...
      if (ret == -1) {
        errcode = errno;
        xerror2(TSF
        "select return -1, error:%0", strerror(errcode));
      ...
      if (sel.Write_FD_ISSET(fdSocket)) {
        ...
        xdebug2(TSF
        "sendWithNonBlock ::send return:%0", nwrite);
        if (nwrite == 0 || (0 > nwrite && !IS_NOBLOCK_SEND_ERRNO(socket_errno))) {
          xerror2(TSF
          "sendWithNonBlock send <= 0, errno:%0", strerror(errcode));
          return kSndRcvErr;
        }
      ...
      xerror2(TSF
      "select return but none is select");
      xassert2(false);
      return kAssertErr;
    }
    return kTcpSucc;
   ...
   *              1  ------ suc and socket close
     *          others ------ error
   */
  static TcpErrCode readnWithNonBlock(int fdSocket, SocketSelect& sel, unsigned int timeoutMs,
                                      AutoBuffer& recvBuf, unsigned int unSize, int& errcode) {

    xverbose_function();
    xdebug2(TSF
    "readnWithNonBlock socket:%0, timeoutMs:%1", fdSocket, timeoutMs);
    if (unSize == 0)
      return kTcpSucc;
    ...
    while (true) {
      ...
      if (sel.Read_FD_ISSET(fdSocket)) {
        ...
        xdebug2(TSF
        "readnWithNonBlock recv :%0", nrecv);
        if (nrecv < 0) {
          xerror2(TSF
          "readnWithNonBlock readn nrecv < 0, errno:%0", strerror(errcode));
          return kSelectErr;
        } else if (nrecv == 0) {
          xinfo2(TSF
          "nrecv==0, socket close:%0", errno);
          return kTcpNonErr;
        }
        ...
        if ((recvBuf.Length() - oldLength) >= unSize) {
          xdebug2(TSF
          "recvBuf.Length()=%0, oldLength=%1, unSize=%2", recvBuf.Length(), oldLength, unSize);
          return kTcpNonErr;
        }
      }
      costMs += (gettickcount() - startMs);
