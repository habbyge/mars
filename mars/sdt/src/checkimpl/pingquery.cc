...
static int Sendto(int fd, const void* ptr, size_t nbytes, int flags, const struct sockaddr* sa, socklen_t salen) {
  ...
  if ((len = (int) sendto(fd, ptr, nbytes, flags, sa, salen)) != (ssize_t) nbytes) {
    xerror2(TSF"sendto: uncomplete packet, len:%_, nbytes:%_, errno:%_(%_)", len, nbytes, socket_errno,
...
void PingQuery::proc_v4(char* _ptr, ssize_t _len, struct msghdr* _msg, struct timeval* _tvrecv) {
  ...
  xdebug2(TSF"before ntohl tvsend sec=%_, nsec=%_; tvrecv sec=%_, usec=%_", tvsend->tv_sec, tvsend->tv_usec,
          _tvrecv->tv_sec, _tvrecv->tv_usec);

  tvsend->tv_sec = ntohl(tvsend->tv_sec);
  tvsend->tv_usec = ntohl(tvsend->tv_usec);

  xdebug2(TSF"tvsend sec=%_, nsec=%_; tvrecv sec=%_, usec=%_", tvsend->tv_sec, tvsend->tv_usec, _tvrecv->tv_sec,
          _tvrecv->tv_usec);
  ...
  if (rtt < 10000.0 && rtt > 0.0) {
    ...
    xerror2(TSF"rtt = %0 is illegal.receive %1 bytes from %2", rtt, icmplen,
...
int PingQuery::RunPingQuery(int _querycount, int _interval/*S*/, int _timeout/*S*/, const char* _dest,
  ...
  if (_packet_size >= ICMP_MINLEN && _packet_size <=
                                     MAXBUFSIZE/*4096*/) {  // packetSize is the length of ICMP packet, include ICMP header,but not include IP header
...
int PingQuery::GetPingStatus(struct PingStatus& _ping_status) {
  ...
  memset(temp, 0, 1024);
  snprintf(temp, 1024, " MaxRTT=%f ms, MinRTT=%f ms, AverageRTT=%f ms", _ping_status.maxrtt, _ping_status.minrtt,
           _ping_status.avgrtt);
  pingresult_.append(std::string(temp));
  _ping_status.res = pingresult_;
  return 0;
...
int doPing(const std::string& _destaddr, std::string& _real_pingip, std::string& _resultstr, unsigned int _packet_size,
           int _pingcount, int _interval/*s*/, int _timeout/*s*/) {
  PingQuery pingObj;
