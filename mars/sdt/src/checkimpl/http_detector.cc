...
#else
  snprintf(temp_time, sizeof(temp_time), "%d-%02d-%02d %+.1f %02d:%02d:%02d.%.3d", 1900 + tm.tm_year, 1 + tm.tm_mon,
           tm.tm_mday,
           tm.tm_gmtoff / 3600.0, tm.tm_hour, tm.tm_min, tm.tm_sec, timeval.tv_usec / 1000);
#endif
...
HTTPDetector::HTTPDetector(const HTTPDetectReq& _req)
    : req_(_req), worker_thread_(boost::bind(&HTTPDetector::__Run, this), XLOGGER_TAG "::HTTPDetector"),
      callback_(NULL) {
...
void HTTPDetector::__Detect() {
  ...
  RequestLine::THttpMethod method = (0 == strncasecmp(req_.http_method_.c_str(), "POST", 4) ? RequestLine::kPost
                                                                                            : RequestLine::kGet);
  ...
  for (std::map<std::string, std::string>::const_iterator iter = req_.extra_header_.begin();
       iter != req_.extra_header_.end(); ++iter) {
  ...
  if (req_.prior_ip_.empty()) {
    ...
    if (!is_host_ipaddr) {
      ...
      bool ret = dns_.GetHostByName(host, vec_ip,
                                    kDefaultDNSTimeout < remain_timeout ? kDefaultDNSTimeout : remain_timeout,
                                    &dns_breaker_);
      ...
      if (ret && !vec_ip.empty()) {
        ...
        if (result_.dns_cost_ >= remain_timeout) {
          result_.dns_errmsg_.append(
              std::string("GetHostByName timeout, set timeout:") + string_cast(remain_timeout).str() + "ms");
        } else {
      ...
      if (remain_timeout <= 0) {
        xwarn2(TSF"@%_ HTTPDetect timeout, dns_cost:%_, total_timeout:%_", this, result_.dns_cost_,
               req_.total_timeout_);
  ...
  if (INVALID_SOCKET == sock) {
    ...
    xerror2(TSF"tcp connect error, isbreak:%_, errno:%_(%_)", breaker_.IsBreak(), result_.tcp_connect_errno_,
            result_.tcp_connect_errmsg_);
    return;
  }

  if (remain_timeout <= 0) {
    xwarn2(TSF"@%_ HTTPDetect timeout, tcp_connect_cost_:%_, total_timeout:%_", this, result_.tcp_connect_cost_,
           req_.total_timeout_);
  ...
  int send_ret = block_socket_send(sock, (const unsigned char*) req_buffer.Ptr(), (unsigned int) req_buffer.Length(),
                                   breaker_, err_code);

  if (send_ret < 0) {
    xerror2(TSF"Send Request Error, ret:%_, errno:%_, nread:%_, nwrite:%_", send_ret, strerror(err_code),
            socket_nread(sock), socket_nwrite(sock));
  ...
  if (remain_timeout <= 0) {
    xwarn2(TSF"@%_ HTTPDetect timeout, http_send_req_cost_:%_, total_timeout:%_", this, result_.http_send_req_cost_,
           req_.total_timeout_);
  ...
  while (true) {
    ...
    if (remain_timeout <= 0) {
      xwarn2(TSF"@%_ HTTPDetect timeout, span:%_, total_timeout:%_", this, (int64_t) detect_tick.gettickspan(),
             req_.total_timeout_);
    ...
    if (recv_ret < 0) {
      xerror2(TSF"read block socket return false, error:%0, nread:%_, nwrite:%_", strerror(err_code),
              socket_nread(sock), socket_nwrite(sock));
      result_.tcp_rw_errno_ = err_code;
      result_.tcp_rw_errmsg_ = strerror(err_code);
      break;
    ...
    if (recv_ret == 0 && SOCKET_ERRNO(ETIMEDOUT) == err_code) {
      xerror2(TSF"read timeout error:(%_,%_), nread:%_, nwrite:%_ ", err_code, strerror(err_code), socket_nread(sock),
              socket_nwrite(sock));
      continue;
    }
    if (recv_ret == 0) {
    ...
    if (parse_status == http::Parser::kFirstLineError) {
      ...
      xerror2(TSF"parse http head failed, but socket closed, length:%_, nread:%_, nwrite:%_ ", recv_buf.Length(),
              socket_nread(sock), socket_nwrite(sock));
      ...
      if (status_code != 200) {
        xerror2(TSF"@%_, status_code != 200, code:%_, http dump:%_ \n headers size:%_", this, status_code,
                xdump(recv_buf.Ptr(), recv_buf.Length()), parser.Fields().GetHeaders().size());
      } else {
