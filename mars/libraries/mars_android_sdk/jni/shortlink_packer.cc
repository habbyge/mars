...
shortlink_tracker* (*shortlink_tracker::Create)() =  []() -> shortlink_tracker* {
  return new shortlink_tracker;
};

void (*shortlink_pack) (const std::string& _url,
    ...
    [](const std::string& _url,
       ...
       const AutoBuffer& _extension,
       AutoBuffer& _out_buff,
       shortlink_tracker* _tracker) -> void {
	...
	for (std::map<std::string, std::string>::const_iterator iter = _headers.begin();
