...
TrafficStatistics::TrafficStatistics()
    : report_timeout_(10 * 1000/*10s*/), report_size_threshold_(10 * 1024/*10KB*/), wifi_recv_data_size_(0),
      wifi_send_data_size_(0), mobile_recv_data_size_(0), mobile_send_data_size_(0),
...
TrafficStatistics::TrafficStatistics(unsigned long _report_tmo, unsigned int _report_size_threshold)
    : report_timeout_(_report_tmo), report_size_threshold_(_report_size_threshold), wifi_recv_data_size_(0),
      wifi_send_data_size_(0), mobile_recv_data_size_(0), mobile_send_data_size_(0),
      last_report_time_(gettickcount()) {}
...
void
TrafficStatistics::SetCallback(const boost::function<void(int32_t, int32_t, int32_t, int32_t)>& _func_report_flow) {
...
void TrafficStatistics::__ReportData() {
  if (func_report_flow_) {
    ...
    xdebug2(TSF"wifi:%_, r:%_, mobile:s:%_, r:%_", wifi_send_data_size_, wifi_recv_data_size_, mobile_send_data_size_,
            mobile_recv_data_size_);
  } else {
    xassert2(false, TSF"wifi:s:%_, r:%_, mobile:s:%_, r:%_", wifi_send_data_size_, wifi_recv_data_size_,
             mobile_send_data_size_, mobile_recv_data_size_);
  }
